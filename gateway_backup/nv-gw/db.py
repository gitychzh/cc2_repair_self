#!/usr/bin/env python3
"""PostgreSQL persistence for nv-uni metrics — R40.
...
- Two tables: nv_requests (1 row per request) + nv_tier_attempts (1 row per
...
- Disabled by default via NVU_DB_ENABLED env (opt-in per machine).
...
DB_ENABLED = os.environ.get("NVU_DB_ENABLED", "0") == "1"
DB_HOST = os.environ.get("NVU_DB_HOST", "logs_db")
DB_PORT = int(os.environ.get("NVU_DB_PORT", "5432"))
DB_USER = os.environ.get("NVU_DB_USER", "litellm")
DB_PASSWORD = os.environ.get("NVU_DB_PASSWORD", "")
DB_NAME = os.environ.get("NVU_DB_NAME", "hermes_logs")

FLUSH_INTERVAL_S = float(os.environ.get("NVU_DB_FLUSH_INTERVAL_S", "2"))
FLUSH_BATCH = int(os.environ.get("NVU_DB_FLUSH_BATCH", "50"))
QUEUE_MAX = int(os.environ.get("NVU_DB_QUEUE_MAX", "2000"))  # drop beyond this
    ...
    with _conn_lock:
        ...
        try:
            ...
            if now - _last_health_check > 60:
                print(f"[NV-DB] connect failed: {e}", flush=True)
    ...
    while not _worker_stop.is_set():
        ...
        while len(batch) < FLUSH_BATCH:
            try:
                ...
                break
        # R845: guard _flush_batch — it has internal try/except, but if any
        # exception escapes it the daemon worker thread dies and all DB writes
        # silently halt (the 02:33 UTC metrics gap root cause). Wrap to survive.
        try:
            _flush_batch(batch)
        except Exception as e:
...
def _flush_batch(batch):
    ...
    try:
            ...
            request_ids = execute_values(
                ...
                """INSERT INTO nv_requests
                   (request_id, ts, host_machine, proxy_role, request_model, mapped_model,
                    ...
                    fallback_actually_attempted, egress_ip, egress_route,
                    function_id, nvcf_reqid, caller)
                   VALUES %s
                   ON CONFLICT (request_id) DO UPDATE SET
                     ...
                     nv_key_idx=EXCLUDED.nv_key_idx,
                     ttfb_ms=EXCLUDED.ttfb_ms,
                     fallback_tiers_used=EXCLUDED.fallback_tiers_used,
                     finish_reason=EXCLUDED.finish_reason,
                     input_tokens=EXCLUDED.input_tokens,
                     output_tokens=EXCLUDED.output_tokens,
                     fallback_occurred=EXCLUDED.fallback_occurred,
                     function_id=EXCLUDED.function_id,
                     nvcf_reqid=EXCLUDED.nvcf_reqid,
                     egress_ip=EXCLUDED.egress_ip,
                     egress_route=EXCLUDED.egress_route
        ...
        global _last_health_check
        if now - _last_health_check > 60:
            print(f"[NV-DB] flush failed ({len(batch)} rows dropped): {e}", flush=True)
            _last_health_check = now
...
def enqueue_metrics(metrics):
    ...
    if not DB_ENABLED or not _HAS_PSYCOPG:
        return
    # R845: worker self-heal — if the daemon DB-writer thread has died (e.g. an
    # exception escaped _flush_batch in a prior version), restart it here so
    # metrics resume without a container restart. Cheap: only re-checks is_alive().
