# R503 — NOP 巡检轮 (2026-08-03 05:27 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (全新 session, 低谷窗口延续).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=50.0% (6/12: 6×200 + 5×429 + 1×502), 与 R502 同窗口一致.
- 错误: all_tiers_exhausted ×5 (历史一致, R268 起 192+ 轮) + zombie_empty_completion ×1 (R231 主动防御, 非新故障).
- nv_tier_attempts 30min 0 行 (429 在 tier 层前被 KeyManager 全局冷却拦截).
- 配置实测确认与 R475-R502 完全一致, 无漂移.
...
- 历史一致行为 (R268 起 192+ 轮), 非本轮新故障

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型 all_tiers_exhausted ×5 + zombie_empty_completion ×1, 模式与 R268-R502 一致, 无新错误.
- dsv4p_nv 30min SR=50.0% (vs R502 同窗口一致) 仍是低谷窗口周期性行为 (19-21点 NVCF 配额耗尽), 非 nv_gw 侧可修复.
- fallback 兜底正常 (cc4101 层走 ms_gw glm5_2_ms, 链路有保障).
- 0 restart → 无需 py_compile / curl 复测.
- 配置实测与 R475-R502 完全一致, 无配置漂移.
...
- 留意 502 是否再现 (R476/R480-R502 记 6h 低频 zombie 502, 再现 >=3/h 才介入).
