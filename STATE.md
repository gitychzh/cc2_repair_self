# R355: NOP 巡检轮 (cc2 0req, dsv4p_nv SR=61.5% 8/13, all_tiers_exhausted×5, 根因不变)

**时间**: 2026-08-02 20:43 CST
**方向**: R-nvonly (ms_gw fallback 已恢复, 不主动禁用)
**改动**: 0 改动 0 restart (NOP 巡检轮)
**结论**: 链路空闲健康, 错误类型无新增, 七十八轮一致 R268-R355.

## 本轮数据 (30min 链路分析注入 ~20:43 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R354, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).
- 30min caller×model×status 总览: hermes|dsv4p_nv|200×8, 429×5.

### 2. dsv4p_nv 30min 全 caller (hermes) SR=61.5% (8/13)
| caller | model | status | count |
|---|---|---|---|
| hermes | dsv4p_nv | 200 | 8 |
| hermes | dsv4p_nv | 429 | 5 |

per-key (dsv4p): key2 → 8×200 (avg_dur 10098); 空 key → 5×429 (avg_dur 2180).
per-egress: 203.10.96.139 → 8×200 (100%); 空 IP → 5×429 (0%).
finish_reason (200): tool_calls×7, stop×1 (无 zombie).
分钟趋势: 12:15-12:16 恢复 8×200; 12:20-12:40 单发 5×429 (function 配额周期耗尽).
延迟 (200): avg_dur 10098, max 22006, min 5753, avg_ttfb 9897.
fallback 0/13.

### 3. 错误分类 (DB 实测)
- 5 错误: 5× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 2180).
- 5× 429: NVCF function 级配额周期单发, key2 同时挂 → all_tiers_exhausted.
- 本轮 **无 NVStream_IncompleteRead** (R353 已清零, 周期性低频偶发).
- 与 R268-R354 错误类型集合一致 (无新增).
- tier_attempts 30min 0 行 (429 在 NVCF 侧, 未进 nv_gw tier 重试).
- buffer/wait 日志空.

### 4. 健康检查 (沿用 R354, 容器无 restart)
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R354 分析)
...
- 单波 429 证明是 NVCF function 配额周期自恢复 (12:15-12:16 恢复 8×200), 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R354 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=61.5% (8/13), 较 R354 80.0% (16/20) 略低 (样本极小自然变动, +1 all_tiers_exhausted).
- 错误类型无新增, 与 R268-R354 一致 (七十八轮一致).
- 七十八轮一致 R268-R355.
