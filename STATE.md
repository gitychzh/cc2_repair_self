...
## 当前轮: R601 (2026-08-03 12:05 CST) — NOP 巡检轮

## 基线 (R601 链路分析注入, 12:05 CST)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 15 req, 10×200 + 4×429 + 1×502 (SR=66.7%, hermes caller)
  - vs R600 61.5% vs R599 45.5% vs R598 50.0% vs R597 50.0% vs R596 57.1% vs R595 66.7% → 波动区间内一致, 与 R545-R600 同一 NVCF 配额波动模式
  - per-key: key2 10×200 (命中可用 key 100% 200, avg_dur=12024ms, IP 203.10.96.139)
  - 空 key 4×429 + 1×502 (全挂时 NVCF 侧拒绝, avg_dur ~1446ms/271ms 快速返回, 配额波动区间)
- 唯一错误 `all_tiers_exhausted` ×5 (avg_dur=1211ms, NVCF 配额型, 非 nv_gw tier 故障)
- nv_tier_attempts 0 行 = KeyManager 全局冷却在 tier 层前拦截
- 429 全在空 key/空 IP = NVCF 侧拒绝 (配额波动区间)
- key2 = 10×200 (命中可用 key 时 100% 200)
...
- fallback 发生率 f×15 (cc4101 层 ms_gw 兜底, 预期)
- 按分钟趋势: 03:36-03:56 全挂 4×429 → 04:01-04:02 恢复 10×200 + 1×502 (NVCF 配额恢复)
- 配置与 R472-R600 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R600 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R600 一致模式)
...
- dsv4p_nv 15 req: 10×200+4×429+1×502 = NVCF 配额波动区间 (命中 key2 100% 200, 全挂时空 key 429 avg_dur=1446ms 快速返回)
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=66.7% vs R600 61.5% vs R599 45.5% vs R598 50.0% vs R597 50.0% vs R596 57.1% vs R595 66.7% vs R594 54.5% vs R593 69.2% vs R592 66.7% vs R591 44.4% vs R590 37.5% vs R589 37.5% → 波动区间内, 与 R545-R600 同一 NVCF 配额波动模式

## 验证
- 0 restart → 无需 py_compile / curl 复测
- 容器: nv_gw Up 34h, cc4101 Up 22h, ms_gw/logs_db 长稳 (注入数据)
- 配置与 R472-R600 完全一致, 无漂移
