# R628 — NOP 巡检轮 (2026-08-03 14:00 CST)

## 基线 (R628 实测, 05:21-05:50 UTC 窗口)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本, 同 R626/R627)
- dsv4p_nv 30min (hermes caller): 31 req, 28×200 + 1×429 + 2×502 (SR=90.3%)
  - vs R627 90.3% / R626 88.5% / R625 85.0% / R624 82.4% / R623 76.5% / R622 64.3% / R621 57.1%
    → **连续 7 轮持平 R627, 反弹趋势已稳定在 R617-R618 正常波动区间 (75-91%) 上沿**
...
- 错误分类 1 类 (非新错误):
  - `all_tiers_exhausted` ×3 (avg_dur 24125ms: 429→2038ms 快速ABORT; 502→35433ms 长ABORT)
    → R621→R622→R623→R624→R625→R626→R627→R628: 6→5→4→3→3→3→3→3 持平
- 30min nv_tier_attempts: 0 行 (hermes caller 不走 buffer, KeyManager 层 ABORT, 同 R627)
- cc_requests stream_total_deadline 6h: 0 (deadline 链健康, 同 R627)

## 持续观察点 (沿用 R627, 不改码) — RemoteDisconnected 34s 长卡顿
- 本轮 2×502 duration≈35433ms, 同 R626/R627 (34646ms/35433ms) 模式
- 根因不变: NVCF 上游慢响应 (34s+ 主动 FIN) + peek-retry 单 key 语义不轮转, 非 timeout 配置
- 24h 反复低频模式 (R626-R627 已 DB 历史查证 10+ 次), 被 cc4101 fallback ms_gw 兜住, 用户侧无感
...
## 依据
- cc2 0 流量 → 无评估样本, 铁律1 cc2 视角不满足 (同 R626/R627)
- dsv4p_nv SR=90.3% 持平 R627, 连续 7 轮反弹后已稳定在正常波动区间上沿
- all_tiers_exhausted ×3 持平 R625-R627 (3→3→3→3→3), 与 SR 稳定同步, 无退化
- KeyManager 指数退避正确, 429 ABORT avg 2038ms (快速, vs R627 2038ms 完全持平)
...
- 配置无漂移 (env 全项匹配 R627 快照)
- curl /health: status ok, nv_num_keys=5, nv_default_model=glm5_2_nv

## 下一步
- dsv4p_nv SR 连续 7 轮反弹至 90.3% 后持平, 已稳固在正常波动区间上沿, 趋势反转确认稳定
- **升级标注解除** (R621 设定 SR<55% 或 exhausted>=8 → 切 PRIMARY 回 glm5_2_nv): 持续未触发
- **持续观察点: RemoteDisconnected 34-72s 长 ABORT** (24h 10+ 次, 同 R626-R627)
