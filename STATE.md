# R464 — NOP 巡检轮 (2026-08-03 03:42 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 18:30-18:55 UTC (16 req: 12×200 + 4×429).
- cc2 (cc4101-primary) 30min 仍 0 req (cc2 session 间歇空闲, 无评估样本).
- dsv4p_nv 全 caller 30min SR=75.0% (12/16), 与 R463 持平, 仍处 46.2%-85% 历史波动区间, 属正常波动.
- 错误: all_tiers_exhausted ×4 (all_tiers_failed_in_mapped_tier, 5key 全挂, 空 IP), 模式与 R268-R463 一致.
- nv_tier_attempts 30min 0 行 → 429 在 tier 层前被拒 (空 IP).
- nv_gw Up 13h, cc4101 Up 2h (本轮未重启).
- 连续 11 轮无 502 (R454-R464), 单点 502 模式似已消退.

## 链路数据 (注入窗口 + 实测复跑 03:42 CST)
- caller×status: hermes 12×200 + 4×429 (request_model=dsv4p_nv)
- dsv4p_nv SR=75.0% (12/16)
- 错误分类: all_tiers_exhausted|all_tiers_failed_in_mapped_tier ×4, avg_dur 1370ms
...
- per-min 趋势: 18:35(200×1)/18:36(200×4)/18:40(200×1)/18:41(200×5)/18:42(200×1)/18:45(429×1)/18:50(429×1)/18:55(429×1)
...
- nv_requests 全 caller 30min: 12×200 + 4×429 (与注入一致)
...
## 历史波动区间 (R437-R464)
R437=85.0 → R438=76.5 → R439=78.9 → R440=78.3 → R441=68.4 → R442=53.3 → R443=57.1 → R444=57.1 → R445=66.7 → R446=66.7 → R447=66.7 → R448=46.2 → R449=46.2 → R450=46.2 → R451=46.2 → R452=46.2 → R453=62.5 → R454=57.1 → R455=44.4 → R456=69.2 → R457=83.3 → R458=84.2 → R459=84.2 → R460=84.2 → R461=84.2 → R462=76.5 → R463=75.0 → R464=75.0%

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted ×4, 模式与 R268-R463 一致 (一百六十余轮一致), 无新错误.
- dsv4p_nv SR=75.0% 与 R463 持平, 仍处 46.2%-85% 历史波动区间, 属正常波动.
- 4×429=8/h 略高于 5/h 阈值, 但 18:35-18:42 连续 12×200 恢复明确, 末段 3×429 离散 (每5min1次) 非集中爆发, 整体可接受.
- 本轮无 502 (R454-R464 连续 11 轮无 502, 单点模式似已消退, 继续观察).
...
- 留意 502 是否再现 (R454-R464 连续 11 轮无 502, 再现 >=3/h 才介入).
