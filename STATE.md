...
## 当前轮基线 (2026-08-02 18:03 CST, R317 NOP 巡检轮)
- 本仓 master: 上轮 R316 (c27d9a0) 已 push. 主仓 hm2 侧 R316 (3db3dd3) 已 push.
- 主仓: R317 (1bef2a7) 已 push origin main (round 文件 + STATE).
- **架构 (主仓 1bef2a7)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R317 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=85.0% (17/20), 失败 3 全 all_tiers_exhausted
  (09:35+09:45+09:50 三波 429 NVCF function 配额周期, 5key 同时挂, 自恢复).
...
## 本轮关键数据 (30min 实时链路分析注入 ~18:03 CST, DB 校验一致)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R316, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
- 容器: nv_gw, cc4101, ms_gw, logs_db 全部 Up (nv_gw/cc4101 Up 4h, ms_gw/logs_db Up 3d).
- env 实测: NVU_DISABLE_MS_FALLBACK=0, UPSTREAM_TIMEOUT=90, NVU_BUFFER_MAX_RETRIES=5,
  TIER_COOLDOWN_S=180, KEY_COOLDOWN_S=30.

## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R316 分析)
...
- 本轮 09:35+09:45+09:50 三波 429 证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R317 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=85.0% (17/20) (窗口命中三波 429, NVCF function 配额周期; 总 req 20 少, 故 SR 数值偏低但根因不变).
- 错误类型无新增 (全 all_tiers_exhausted), 与 R268-R316 一致.
- 五十轮一致 R268-R317.
