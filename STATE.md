...
## 当前轮基线 (2026-08-02 19:30 CST, R339 NOP 巡检轮)
- 本仓 master: 上轮 R338 (b2b3da2) 已 push. 主仓 hm2 侧 R338 (8bd0d3e) 已 push, 本轮 R339.
- **架构 (主仓 8bd0d3e)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R339 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=91.2% (31/34), 失败 3 = 3× all_tiers_exhausted
  (2×429 NVCF function 配额周期 + 1×502 RemoteDisconnected transport hang, 11:16 一波, 自恢复).
  本轮无 NVStream_IncompleteRead. 错误类型无新增, 与 R268-R338 一致.
...
## 本轮关键数据 (30min 实时链路分析注入 ~19:28 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R338, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).
- 30min caller×model×status 总览: hermes|dsv4p_nv|200×30, 429×2, 502×1; openclaw|dsv4p_nv|200×1.

### 2. dsv4p_nv 30min 全 caller SR=91.2% (31/34)
| caller | model | status | count |
|---|---|---|---|
| hermes | dsv4p_nv | 200 | 30 |
| hermes | dsv4p_nv | 429 | 2 |
| hermes | dsv4p_nv | 502 | 1 |
| openclaw | dsv4p_nv | 200 | 1 |

per-key (dsv4p): key2 → 30×200 (avg_dur 11056); key3 → 1×200 (3310);
空 key → 2×429 (1261) + 1×502 (34101, transport-level hang, 空IP/0%).
per-egress: 203.10.96.139 → 30×100%; 134.195.101.194 → 1×100%; 空 IP → 3×0.
finish_reason (200): tool_calls×27 + stop×4 (无 zombie).
分钟趋势: 11:00-11:27 持续 30×200; 11:16 2×429 + 11:27 1×502 (一波 function 级降级).
延迟 (200): avg_dur 10806, max 25900, min 3047, avg_ttfb 10403.
fallback 0/34.

### 3. 错误分类 (DB 实测)
- 3 错误: 3× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 12208).
- 429 (2×): NVCF function 级配额周期, 11:16 一波, key2 同时挂 → all_tiers_exhausted.
- 502 (1×, dur 34101ms): 同 R336-R338 RemoteDisconnected transport-level hang (单发, 偶发).
- 本轮无 NVStream_IncompleteRead.
- 与 R268-R338 错误类型集合一致.
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R338 分析)
...
- 502 RemoteDisconnected transport hang: 同 R336-R338, NVCF 单 function 偶发 transport 级断连, peer 也降级时返回 502 (设计). 偶发不达介入阈值.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R338 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=91.2% (31/34), 与 R338 同窗口数据一致 (命中同一段 30min).
- 错误类型无新增, 与 R268-R338 一致.
- 六十一轮一致 R268-R339.
