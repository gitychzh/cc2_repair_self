...
## 当前轮: R591 (2026-08-03 11:10 CST) — NOP 巡检轮

## 基线 (R590 实测, 11:08 CST 链路分析注入)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 9 req, 4×200 + 5×429 (SR=44.4%, 全 hermes caller)
  - vs R590: SR 37.5% → 44.4% (略升, 在 20-55% 波动区间内)
  - per-key: key2 3×200 (命中可用 key 100% 200, avg_dur=10675ms, IP 203.10.96.139)
  - per-key: key3 1×200 (命中可用 key 100% 200, avg_dur=3274ms, IP 134.195.101.194)
  - 空 key 5×429 (全挂时 NVCF 侧拒绝, 配额波动区间)
- 唯一错误 `all_tiers_exhausted` ×5 (avg_dur=1394ms, NVCF 配额型, 非 nv_gw 故障)
- nv_tier_attempts 0 行 = KeyManager 全局冷却在 tier 层前拦截
- 429 全在空 key/空 IP = NVCF 侧拒绝 (配额波动区间)
- key2 = 3×200 + key3 = 1×200 (命中可用 key 时 100% 200)
- 无 buffer/wait 日志 (30min 无 buffer 触发), 无 stream_total_deadline, 无 zombie
- finish_reason: stop×2 + tool_calls×2 (健康, 无 zombie stop)
...
## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R590 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R590 一致模式)
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=44.4% vs R590 37.5% vs R589 37.5% vs R588 37.5% vs R587 37.5% vs R586 37.5% vs R585 37.5% vs R584 71.4% vs R583 71.4% vs R582 58.3% → 波动区间内, 与 R545-R590 同一 NVCF 配额波动模式
