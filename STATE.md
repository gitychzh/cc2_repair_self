# R558 — NOP 巡检轮 (2026-08-03 09:00 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (延续 R525-R557 间歇空闲窗口).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无 cc2 评估样本,
  铁律1 cc2 视角不满足 → 不动码).
- dsv4p_nv 30min: 6 req SR=0.0% (0×200 + 6×429), 全 `hermes` caller.
  与 R544-R557 完全一致模式 (R544=37.5%, R545-R557=0%, 6 req 样本落在
  NVCF 配额波动区间低端, 非 nv_gw 侧新故障, k2 在本轮窗口未命中配额空隙).
- 唯一错误类型 `all_tiers_exhausted` × 6 (avg_dur=1185ms, 与 R557 的 1185ms /
  R556 的 1247ms / R555 的 1247ms / R554 的 1270ms 一致量级 — KeyManager 全局
  冷却在 tier 层前拦截, tier_attempts 0 行, 历史一致).
- NV-GLOBAL-COOLDOWN tier=dsv4p_nv 周期性 429 仍在 (每 5min 1 次, 共 6 次,
  00:15/00:20/00:25/00:30/00:35/00:40, 与 R268-R557 完全一致), NVCF 侧配额波动.
