...
## 当前轮基线 (2026-08-02 13:50 CST, R-nvonly-post259 NOP 巡检轮)
- 本仓 master: 本轮 post259. (主仓 hermes_improve_self main 收 round 文件.)
- **本轮 R-nvonly-post259 (hm2_cc2)**: NOP 巡检轮. cc2 30min 0 req (session 轮前无流量产生, 无数据可判 SR).
  ...
  0 改动, 0 重启.
  hermes 打 dsv4p_nv 11req (6×200 + 3×429 + 1×502, SR≈54.5%, key2 扛 200+502, 3×429 all_tiers_exhausted avg 1762ms, 1×502 NVStream_IncompleteRead avg 34130ms).
...
本轮 30min cc2 无请求产生. 无数据可判 cc2 SR. 链路健康无故障. (nv_requests caller=cc4101-primary 0 rows 复核确认, nv_tier_attempts 0 rows, buffer/wait 日志空)

### 2. 其他 caller (hermes, 非 cc2 链路)
| caller | model | 200 | 429 | 502 | SR |
|--------|--------|-----|-----|-----|-----|
| hermes | dsv4p_nv | 6 | 3 | 1 | ~54.5% (10req) |

dsv4p_nv: 6×200 + 3×429 (all_tiers_exhausted, avg 1762ms) + 1×502 (NVStream_IncompleteRead avg 34130ms).
per-key: key2 扛 7 (6×200+1×502), 3×429 无 key.
per-egress: 203.10.96.139 8req.
finish_reason: tool_calls×6 + stop×1 (无 zombie).
fallback 发生率: f=11 (无 fallback, 主链路全扛).
...
- all_tiers_exhausted ×3 (hermes dsv4p_nv, avg 1762ms, 非 cc2).
...
| 30min 全 caller | hermes 11req dsv4p_nv (6×200 + 3×429 + 1×502), cc2 0 req ✓ |
| 配置 | NVU_DISABLE_MS_FALLBACK=0 (fallback 已恢复), FALLBACK_UPSTREAM=ms_gw:40007 ✓ |

## 参数快照 (2026-08-02 13:50 CST, 无变化, 同 post258)
