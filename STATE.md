...
> 当前轮: **R1064 (NOP 巡检轮/不改码 — 主链 111/112=99.1%, 唯一 scoped bad = R1060 瞬时 ec39dd9b 仍落 lookback 尾部已自愈, 非新缺陷; fresh 5min 100% 干净; main fid 281478d0 零错误; fallback 0 次)**
...
> **R1060 已溯源的瞬时**: 3 连 SSLEOFError (3 key/IP) + ms_gw 同步 down, **已自愈** (现处 30min lookback 尾部 ~8min 后滑出, 非新错误);
> fresh 5min = **20/20 全 200, 0 错误**, 链路当前完全健康;
> nv_tier_attempts main fid **281478d0** 109×pexec_success, **0 错误**;
> 5×NVCFPexecRemoteDisconnected 全命中坏 fid **52e1ddb6** → hermes 越界宿主, 非主链;
> hermes 另 bads (zombie×2/IncompleteRead×1) 同为越界宿主, 非 cc2 范围;
> 30min cc_requests fallback = **0 次 / 0.0%**;
> buffer 日志本轮窗口全 attempt1 success (7~12s), 无 fail/WAIT/KEYMGR;
> 容器 (/health 复核): nv_gw Up 16h, cc4101 Up 15h, /health 40006/4101 全 200
> 上轮: R1063 (NOP, 主链 103/104=99.0%)

## 本轮 (R1064) 改动 + 依据 + 验证

### 改动: 无 (NOP。唯一 scoped bad 为上轮 R1060 已溯源瞬时 ec39dd9b 仍落 lookback 尾部, 非新缺陷; 主链 fid 281478d0 全 pexec_success; 无参数可调)

### 依据 (注入轮前链路分析 19:22 CST + DB/容器日志复核 2026-08-07)

- 30min cc4101-primary (主 nv_gw:40006) = **111/112 = 99.1% SR, 1 scoped bad** (DB 复核, 较轮前注入 109 略增为 111, 窗口滑动所致)。
...
- 唯一 scoped 502 根因 (链路铁证): req **ec39dd9b**, buffer_exhausted, 58949ms,
  **DB created_at=11:02:02 UTC = 19:02 CST** (UTC-8)。即 **R1060 轮首即详细溯源的瞬时**:
  ...
  其 nv_tier_attempts 本轮已 0 行 (旧 attempt 记录已过期), 进一步证明为历史瞬时。
  **它本轮仍落在 30min lookback 尾部 (19:02 距今 ~8min), 非新缺陷**; 再 ~8min 彻底滑出后即消失。
- 主链 main fid **281478d0** nv_tier_attempts (30min) = **109 × pexec_success (全 5 key), 0 错误**。
  另 5 条 NVCFPexecRemoteDisconnected (k0×1/k1×2/k2×1/k3×1) 全命中**坏 fid 52e1ddb6** → hermes 越界宿主 (request_id JOIN), 非主链。
- hermes (越界宿主, 非 cc2 范围) 30min bads (DB 复核): zombie_empty_completion×2 (59bd8bb6/20982d20) + NVStream_IncompleteRead×1 (36b31ca1)。
- 30min cc_requests fallback = **0 次 / 0.0%**。
- buffer 日志本轮窗口 (19:23~19:24) 全 attempt=1/5 success (success_tool_call, elapsed 7~12s), 无任何 fail/WAIT/KEYMGR。
...
| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **111/112 = 99.1% SR, 1 scoped bad** | ⚠️(R1060 遗留瞬时尾部) |
...
| nv_tier_attempts 5×RemoteDisconnected | 全命中坏 fid 52e1ddb6 (hermes 宿主) | ✅(非主链) |
| 30min cc_requests | fallback 0 次 (0.0%) | ✅ |
