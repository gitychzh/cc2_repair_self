...
## 当前轮基线 (2026-08-02 13:06 CST, R-nvonly-post243 NOP 巡检轮)
- 本仓 master: 本轮 post243. (主仓 hermes_improve_self main 收 round 文件.)
- **本轮 R-nvonly-post243 (hm2_cc2)**: NOP 巡检轮. cc2 30min 0 req (session 轮前无流量产生, 无数据可判 SR).
  ...
  0 改动, 0 重启.
  hermes 打 dsv4p_nv 24req (22×200/1×429/1×other, SR=95.8%, all_tiers_exhausted×1, NVCF 配额限流, key2 单 key 扛 22×200).
  **与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv).
  glm5_2_nv 连续 post100-post243 (140 轮) 无 dsv4p 故障扩散.
...
|--------|--------|-----|-----|-----|
| hermes | dsv4p_nv | 22 | 1 | 95.8% (24req) |
| openclaw | dsv4p_nv | 1 | 0 | 100% (1req) |

hermes→dsv4p_nv SR=95.8% (24req): 22×200 + 1×429, all_tiers_exhausted×1 (NVCF 配额限流).
per-key: key2 扛 22×200 (单 key 健康, avg_dur 9776ms), key3 1×200, 1×429 来自无 key 映射 (empty key).
per-egress: 203.10.96.139 扛 22×200 (100% SR), 1×429 来自无 egress (空 IP).
finish_reason: tool_calls×18, stop×5 (无 zombie).
按分钟趋势: 04:40 1×429 (限流), 04:45-05:06 23×200 (恢复).
**与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv).

### 3. 30min 错误分类 (全 caller)
- all_tiers_exhausted (hermes→dsv4p_nv): 1× (avg_dur 1281ms), 全为 NVCF 配额限流, 非 cc2 链路.
...
| 30min 全 caller | hermes 24req dsv4p_nv (22×200/1×429 限流), openclaw 1req, cc2 0 req ✓ |
| 配置 | NVU_DISABLE_MS_FALLBACK=0 (fallback 已恢复), FALLBACK_UPSTREAM=ms_gw:40007 ✓ |

## 参数快照 (2026-08-02 13:06 CST, 无变化, 同 post241)
