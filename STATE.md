...
## 当前轮基线 (2026-08-02 16:58 CST, R299 NOP 巡检轮)
- 本仓 master: 本轮 R299 (push 待办). 主仓 hermes_improve_self main `f02a83e` R298 round.
- **架构 (主仓 b4527f9, 非本轮)**: cc4101 `PRIMARY_UPSTREAM_MODEL` 已从
  `glm5_2_nv` 切到 `dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R299 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=87.5% (14/16), 失败 2 全 `all_tiers_exhausted`
  (08:30/08:40 两波 429, NVCF function 配额周期 → 08:35-08:36/08:45-08:56 恢复 200, 自恢复).
...
## 本轮关键数据 (30min 实时 DB + 链路分析注入 ~16:58 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R298, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait/keymanager 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).

### 2. dsv4p_nv 30min 全 caller SR=87.5% (14/16)
| status | count | 备注 |
|---|---|---|
| 200 | 14 | key2 egress 203.10.96.139 健康 (avg_dur 12158, max 30071) |
| 429 | 2 | function 级配额边界 (08:30/08:40) |

per-key: key2 → 14×200 + 2×429. per-egress: 203.10.96.139 → 14×100.
finish_reason: tool_calls×10, stop×4 (无 zombie).
分钟趋势: 08:30 429 → 08:35-08:36 恢复 200 → 08:40 429 → 08:45-08:56 多波 200 恢复.
典型 NVCF function 配额周期自恢复 (429 → cooling → decay → reset → 200).

### 3. 错误分类 (DB 实测)
- 全 `all_tiers_exhausted` (2 条), 无新错误类型, 与 R268-R298 一致.
- fallback 0/16.
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R298 分析)
...
- 08:30/08:40 429 → 08:35-08:36/08:45-08:56 恢复 200,
  证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R299 验证), 对 function 级 429 是已知盲区.
