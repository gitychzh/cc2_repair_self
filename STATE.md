# R474 — NOP 巡检轮 (2026-08-03 04:50 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 19:20-19:50 UTC (04:48 CST 实测).
- cc2 (cc4101-primary) 30min 0 req (cc2 session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=0.0% (0/6, 全 429), 与 R473 同窗口一致.
- 6h 视角 dsv4p_nv SR=75.0% (159/212), 处历史波动区间 46-85%.
- 2h 视角: 18:00-18:40 连续高 200 桶, 18:50+ 进入低谷 (离散 429).
- 错误: all_tiers_exhausted ×6 (唯一类型, 模式与 R268-R473 一致, 无新错误).
- nv_tier_attempts 30min 0 行 (429 在 tier 层前被拒, 空 IP, 与 R470-R473 一致).
- 6h stream_total_deadline = 0 (deadline 链无越界).
- nv_gw Up 13h, cc4101 Up 3h (本轮未重启).
- 配置实测确认与 R473 完全一致, 无漂移.

## 链路数据 (04:48 CST 实测)
### 30min 窗口 (hermes caller, 全 dsv4p_nv)
- 6×429, SR 0.0% (与 R473 同窗口, R467-R474 七轮低谷锁定)
- 错误分类: all_tiers_exhausted ×6 (仅 hermes caller, cc4101-primary 0 req)
- nv_tier_attempts: 0 行 (429 未进入 tier 尝试, 空 IP, 与 R470-R473 一致)
- per-min 趋势: 19:10-19:40 每5min1次离散 429 (非集中爆发)

### 6h 视角 (dsv4p_nv)
- 159×200 + 49×429 + 4×502 = SR 75.0% (212req)
- 与 R473 完全一致 (159/212 vs 160/213, 差异为窗口滚动 1req)

### 2h 按桶 (10min)
- 17:50(2×429)→18:00(6×200/2×429/1×502)→18:10(2×429)→18:20(4×200/1×429)
  →18:30(5×200/1×429)→18:40(7×200/1×429)→18:50(2×429)
  →19:00(4×200/1×429)→19:10+(全 429)
- 18:00-18:40 连续高 200 桶, 18:50 起进入低谷, 符合 dsv4p_nv 周期性配额模式.
...
## 历史波动区间 (R437-R474)
R437=85.0 → ... → R467=44.4 → R468-R472=44.4 → R473=0.0% → R474=0.0% (低谷, 6h 视角 75.0%)

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted ×6, 模式与 R268-R473 一致 (170+ 轮), 无新错误.
...
- 6×429/30min ≈ 12/h 高于 5/h 阈值, 但 per-min 趋势呈"每5min1次离散 429", 非集中爆发.
  且 18:00-18:40 连续高 200 桶证明 NVCF 侧可用, 当前低谷是配额周期非链路故障.
- 本轮无 502 (2h 仅 1×502, 6h 4×502 低频, 单点模式继续消退).
- fallback 未触发 (ms_gw 已恢复但 dsv4p_nv 自恢复足够, 无需 fallback).
- 0 restart → 无需 py_compile / curl 复测.
- 配置实测与 R473 完全一致, 无配置漂移.
