# R374: NOP 巡检轮 (cc2 0req, dsv4p_nv SR=27.3% 3/11, all_tiers_exhausted×8[5×429+3×502 NV-TIER-SKIP 1ms] + stream_first_byte_timeout×1, 九十七轮一致)

## 当前轮基线 (2026-08-02 21:55 CST, R374 已完成, R375 待跑)
- 本仓 master: R373 已 push (03a9633). hermes 仓: R373 已 push (fcef737).
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R374 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=27.3% (3/11), 失败 8 = 8× all_tiers_exhausted
  (细分 5×429 avg 1705ms NVCF dsv4p function 配额瞬时空位 + 3×502 avg 1ms NV-TIER-SKIP 全 cooling 瞬拒, 均 0ms/1ms 快速失败非缓冲 caller)
  ...
  **cc2 是缓冲 caller (NVU_BUFFER_CALLERS), 走 buffer 5key 轮转路径, 不走 NV-TIER-SKIP, 不受影响.**
  错误类型无新增 (dsv4p 仍 all_tiers_exhausted, glm5_2_nv stream_first_byte_timeout, 九十七轮一致 R268-R374).
  cc2 无流量不受影响, 0 fallback 0 deadline. 0 改动 0 restart. **九十七轮一致 R268-R374**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~21:54 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).

### 2. dsv4p_nv 30min 全 caller SR=27.3% (3/11)
| caller | model | status | count | avg_dur |
|---|---|---|---|---|
| hermes | dsv4p_nv | 200 | 3 | 9700 |
| hermes | dsv4p_nv | 429 | 5 | 1705 |
| other | dsv4p_nv | 502 | 3 | 1 |
| other | glm5_2_nv | 200 | 2 | — |
| other | glm5_2_nv | 502 | 1 | 29952 |

per-key (dsv4p): key2 → 3×200 (9700); 空 key → 5×429 (1705) + 3×502 (1).
per-egress: 203.10.96.139 → 3× (100); 空 → 8× (0).
finish_reason (200): tool_calls×2, stop×1 (无 zombie).
分钟趋势: 13:25 429×1, 13:30 429×1, 13:33 502×3 (1ms NV-TIER-SKIP), 13:35 429×1, 13:40 200×1, 13:41 200×2, 13:45 429×1, 13:50 429×1.
延迟 (200): avg_dur 9700, max 11546, min 7774, avg_ttfb 8289, avg_in 0, avg_out 0.
fallback f×14 (全部 false, 0 fallback).

### 3. 错误分类 (30min)
- 8 dsv4p 错误: 8× all_tiers_exhausted (avg 1066ms) — 细分: 5×429 (1705ms, NVCF dsv4p function 配额瞬时空位) + 3×502 (1ms, NV-TIER-SKIP 全 cooling 瞬拒).
...
- dsv4p 错误类型集合与 R268-R373 一致 (all_tiers_exhausted, 无新增).
