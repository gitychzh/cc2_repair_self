...
## 当前轮基线 (2026-08-02 19:57 CST, R345 NOP 巡检轮 已完成, R346 待跑)
- 本仓 master: 上轮 R345 (本地 modified) 待 commit. hermes 仓: R345 (fc8f85b) 已 push.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R345 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller (hermes) SR=33.3% (3/9), 失败 6 = 6× all_tiers_exhausted
  (NVCF function 级 429 配额周期多波). 本轮无 NVStream_IncompleteRead; 502 1× 偶发 transport-level.
  错误类型无新增, 与 R268-R345 一致. cc2 无流量不受影响, 0 fallback 0 deadline.
  0 改动 0 restart. **六十八轮一致 R268-R345**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~19:57 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R345, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).
- 30min caller×model×status 总览: hermes|dsv4p_nv|200×3, 429×5, 502×1.
...
per-key (dsv4p): key2 → 3×200 (avg_dur 8590); 空 key → 5×429 (1919) + 1×502 (34101, transport-level hang).
per-egress: 203.10.96.139 → 3×100%; 空 IP → 6×0%.
finish_reason (200): tool_calls×2, stop×1 (无 zombie).
分钟趋势: 11:27 1×502; 11:30/11:35/11:40/11:45 持续 429; 11:55-11:56 恢复 3×200.
延迟 (200): avg_dur 8590, max 11298, min 6928, avg_ttfb 7954.
fallback 0/9.

### 3. 错误分类 (DB 实测)
- 6 错误: 6× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 7283).
- 429 (5×): NVCF function 级配额周期, 多波, key2 同时挂 → all_tiers_exhausted.
- 502 (1×, dur 34101ms): 同 R336-R345 RemoteDisconnected transport-level hang (单发, 偶发).
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R345 分析)
...
- 多波 429 证明是 NVCF function 配额周期自恢复 (11:55-11:56 恢复 3×200), 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.
- 502 RemoteDisconnected transport hang: 同 R336-R345, NVCF 单 function 偶发 transport 级断连, peer 也降级时返回 502 (设计). 偶发不达介入阈值.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R345 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=33.3% (3/9), 与 R344 同窗口数据漂移 (6/12 → 3/9, 窗口滚动 + 样本极小 (9req) 的自然变动, 429 波次略增).
