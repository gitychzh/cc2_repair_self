...
## 当前轮: R605 (2026-08-03 12:26 CST) — NOP 巡检轮

## 基线 (R605 链路分析注入, 12:26 CST)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 27 req, 22×200 + 2×429 + 2×502 (SR=85.2%, hermes caller)
  - vs R604 84.6% vs R603 84.6% vs R602 76.2% vs R601 66.7% vs R600 61.5% vs R599 45.5% vs R598 50.0% → 波动区间, NVCF 配额型
  - per-key: key2 23×200 (命中可用 key 100% 200, avg_dur=11400ms, IP 203.10.96.139)
...
- 配置与 R472-R604 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R604 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R604 一致模式)
...
- dsv4p_nv 27 req: 22×200+2×429+2×502 = NVCF 配额波动区间 (命中 key2 100% 200, 全挂时空 key 429 avg_dur=1470ms 快速返回)
...
- all_tiers_exhausted avg_dur 9482ms (vs R604 9482ms) 持平, 仍 NVCF 配额型 (全挂时 KeyManager 等冷却到期再 abort), 非 nv_gw tier 故障
- 502 avg_dur 17494ms ×2 = peer-fb-skip 慢速返回 (dsv4p_nv 跳 peer fb), 全挂时 NVCF 侧慢拒绝
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=85.2% vs R604 84.6% vs R603 84.6% vs R602 76.2% vs R601 66.7% vs R600 61.5% vs R599 45.5% vs R598 50.0% vs R597 50.0% vs R596 57.1% vs R595 66.7% → 波动区间内, 与 R545-R604 同一 NVCF 配额波动模式
...
- 容器 (链路分析注入):
  ...
  - cc4101: Up 22 hours
  - ms_gw/logs_db: 长稳
- /health: ok (R604 实测), 5 keys, 3 models (kimi_nv/dsv4p_nv/glm5_2_nv), port 40006
- 配置与 R472-R604 完全一致, 无漂移
