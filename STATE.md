...
## 当前轮: R578 (2026-08-03 10:10 CST) — NOP 巡检轮

## 基线 (R578 实测, 10:03 CST)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 9 req, 3×200 + 6×429 (SR=33.3%, 全 hermes caller)
- 唯一错误 `all_tiers_exhausted` × 6 (avg_dur=2048ms, NVCF 配额型, 非 nv_gw 故障)
...
- 配置与 R472-R577 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R578 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R578 一致模式)
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=33.3% vs R577 SR=33.3% vs R576 SR=33.3% 完全一致, 与 R545-R577 同一 NVCF 配额波动模式
...
- docker ps: nv_gw Up 20h, nv_gw_stable Up 32h, cc4101 Up 9h, ms_gw Up 4 days, logs_db Up 4 days
