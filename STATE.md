...
## 当前轮基线 (2026-08-02 12:55 CST, R-nvonly-post235 NOP 巡检轮)
- 本仓 master: 本轮 post235. (主仓 hermes_improve_self main 收 round 文件.)
- **本轮 R-nvonly-post235 (hm2_cc2)**: NOP 巡检轮. cc2 30min 0 req (session 轮前无流量产生, 无数据可判 SR).
  ...
  0 改动, 0 重启.
  hermes 打 dsv4p_nv 11req (7×200/4×429, SR≈70%, all_tiers_exhausted ×3, NVCF 配额限流, key2 单 key 扛 7×200).
...
|--------|--------|-----|-----|-----|
| hermes | dsv4p_nv | 7 | 4 | 70% (11req) |

hermes→dsv4p_nv SR=70% (11req): 7×200 + 4×429, all_tiers_exhausted ×3 (NVCF 配额限流).
per-key: key2 扛 7×200 (单 key 健康, avg_dur 9011ms), 4×429 来自无 key 映射 (empty key).
per-egress: 203.10.96.139 扛 8×200 (100% SR), 4×429 来自无 egress (空 IP).
finish_reason: tool_calls×6, stop×2 (无 zombie).
按分钟趋势: 04:30-04:40 间 3×429 (限流), 04:45-04:55 7×200 (恢复).
**与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv).
...
| docker ps | nv_gw/cc4101 Up 11h, ms_gw/logs_db Up ✓ |
...
## 参数快照 (2026-08-02 12:55 CST, 无变化, 同 post233)
