# R433 — NOP 巡检轮 (2026-08-03 01:25 CST)
...
- dsv4p_nv 全 caller 30min SR=85.0% (17/20), 3×429 all_tiers_exhausted (avg 2909ms),
  在历史波动区间内 (R420=86.4% → R429=69.2% → R430=63.6% → R433=85.0%, 回升).
- per-key (dsv4p 200): k2×16, k3×1 = 17×200; 3×429 无 key 归属 (空 IP).
- per-egress-IP: 203.10.96.139 16/16=100%, 134.195.101.194 1/1=100%, 空 IP 3req 全 429.
- 200 延迟 avg 9883ms, avg_ttfb 9391ms (正常波动).
- 30min buffer/wait/keymanager 日志: 无 (buffer caller 本轮 0 req 触发).
- 错误类型无新增, 与 R268-R432 一致 (一百五十余轮一致).
- glm5_2_nv 30min 0 req → 切 PRIMARY_UPSTREAM_MODEL 不满足"改前必有数据"铁律, 暂不切.

## 验证
- 容器健康: nv_gw (11h), cc4101 (39min), ms_gw, logs_db Up.
- curl /health: status=ok, nv_num_keys=5, nv_model_tiers 正常.
