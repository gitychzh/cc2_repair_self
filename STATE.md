...
## 当前轮: R607 (2026-08-03 12:33 CST) — NOP 巡检轮

## 基线 (R607 链路分析注入, 12:32 CST)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 26 req, 22×200 + 2×429 + 2×502 (SR=84.6%, hermes caller)
  - vs R606 85.2% vs R605 85.2% vs R604 84.6% vs R603 84.6% vs R602 76.2% vs R601 66.7% vs R600 61.5% vs R599 45.5% vs R598 50.0% → 波动区间, NVCF 配额型
...
- key2 = 21×200 (命中可用 key 时 100% 200)
...
- fallback 发生率 f×25 (cc4101 层 ms_gw 兜底, 预期)
- 按分钟趋势: 04:02-04:08 502+200 → 04:11-04:12 200 → 04:16/04:21 又 429 (NVCF 配额波动) → 04:26-04:32 恢复 200
- per-egress-IP: 203.10.96.139 21×100 (命中 IP 100% 200), 空 IP 4×0
- 配置与 R472-R606 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R606 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R606 一致模式)
...
- all_tiers_exhausted avg_dur 9482ms (vs R606 9482ms) 持平, 仍 NVCF 配额型 (全挂时 KeyManager 等冷却到期再 abort), 非 nv_gw tier 故障
...
- 容器 (链路分析注入 + 实测):
  - nv_gw: Up 22 hours (R606 时 22h, 持平)
  - cc4101: Up 12 hours (R606 时 12h, 持平)
