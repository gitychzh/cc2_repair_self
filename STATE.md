...
- dsv4p_nv 30min: 8 req, 3×200 + 5×429 (SR=37.5%, 全 hermes caller)
  - vs R584: SR 71.4% → 37.5% (下滑, 仍在 6h 波动区间 20-55% 内)
  - per-key: key2 3×200 (命中可用 key 100% 200, avg_dur=11798ms, IP 203.10.96.139)
  - 空 key 5×429 (全挂时 NVCF 侧拒绝, 配额波动区间)
- 唯一错误 `all_tiers_exhausted` ×5 (avg_dur=1512ms, NVCF 配额型, 非 nv_gw 故障)
...
- 配置与 R472-R584 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R585 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R585 一致模式)
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=37.5% vs R584 71.4% vs R583 71.4% vs R582 58.3% vs R581 58.3% vs R580 58.3% vs R579 66.7% → 波动区间内, 与 R545-R584 同一 NVCF 配额波动模式
