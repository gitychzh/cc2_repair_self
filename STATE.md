# R469 — NOP 巡检轮 (2026-08-03 03:25 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 18:50-19:20 UTC (实测复跑 03:25 CST 确认).
- cc2 (cc4101-primary) 30min 仍 0 req (cc2 session 间歇空闲, 无评估样本).
- dsv4p_nv 全 caller 30min SR=44.4% (4/9), 与 R467/R468 完全一致, 三轮锁定.
- 错误: all_tiers_exhausted ×5 (唯一类型, 模式与 R268-R468 一致, 无新错误).
- nv_tier_attempts 30min 0 行 → 429 在 tier 层前被拒 (空 IP).
- 6h stream_total_deadline = 0 (deadline 链无越界).
- nv_gw Up 13h, cc4101 Up 3h (本轮未重启).
- 连续 16 轮无 502 (R454-R469), 单点 502 模式似已消退.
- 配置实测确认与 R468 完全一致, 无漂移.

## 链路数据 (注入窗口 + 实测复跑 03:25 CST)
- 全 caller 30min: 4×200 + 5×429 = SR 44.4% (9req, 与注入一致)
- caller×model×status (注入): hermes|dsv4p_nv 3×200 + 5×429; openclaw|dsv4p_nv 1×200
- 错误分类: all_tiers_exhausted ×5 (唯一类型, 与注入一致)
- cc4101-primary 30min: 0 rows (实测复跑 0 rows 确认)
- per-key (实测): nv_key_idx=2 3×200; nv_key_idx=3 1×200; NULL key 5×429
...
- per-min 趋势 (实测): 18:55(429)/19:00(429)/19:05(200×2)/19:06(200×2)/19:10(429)/19:15(429)/19:20(429)
  - 19:05-19:06 连续 200 恢复明确, 其余离散 429 (每5min1次) 非集中爆发
- buffer/wait 日志: 30min 无 BUFFER-/WAIT- 行 (cc4101-primary 0 req, 无 buffer 触发样本)

## keymgr 日志关键观察 (03:20:50)
- 单次 429 即触发 NV-GLOBAL-COOLDOWN: "all keys 429. Marking all cooling 180s (TIER_COOLDOWN)"
- 429 count decay 机制工作正常 (>300s since last 429 → reset)
- 单次 429 → 全 key 180s 冷却 → 后续请求 all_tiers_exhausted
- 这是历史一致行为 (R268 起 160+ 轮), 非本轮新故障
- 429 resp 无 retry-after 头 (headers: ratelimit/retry=(none)), 故依赖 TIER_COOLDOWN_S 兜底

## 历史波动区间 (R437-R469)
R437=85.0 → ... → R467=44.4 → R468=44.4 → R469=44.4% (三轮锁定 44.4%)

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted ×5, 模式与 R268-R468 一致, 无新错误.
- dsv4p_nv SR=44.4% 与 R467/R468 完全一致, 三轮锁定, 仍处 46.2%-85% 历史波动区间, 属正常波动.
- 5×429/30min ≈ 10/h 略高于 5/h 阈值, 但 per-min 趋势呈"19:05-19:06 连续 200 + 其余离散 429"恢复模式, 非集中爆发, 整体可接受.
- 本轮无 502 (R454-R469 连续 16 轮无 502, 单点模式似已消退, 继续观察).
- fallback 未触发 (ms_gw 已恢复但 dsv4p_nv 自恢复足够, 无需 fallback).
- 0 restart → 无需 py_compile / curl 复测 (健康检查已做).
- 配置实测与 R468 完全一致, 无配置漂移.
...
- 留意 502 是否再现 (R454-R469 连续 16 轮无 502, 再现 >=3/h 才介入).
