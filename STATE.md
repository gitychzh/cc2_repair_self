...
## 当前轮: R584 (2026-08-03 10:35 CST) — NOP 巡检轮

## 基线 (R584 实测, 10:35 CST)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 14 req, 10×200 + 4×429 (SR=71.4%, 全 hermes caller)
  - vs R583: SR 71.4% → 71.4% (完全一致, 同窗口复测稳态)
  - vs R582: SR 58.3% → 71.4% (上扬, NVCF 配额波动区间内)
- 唯一错误 `all_tiers_exhausted` ×4 (avg_dur=1530ms, NVCF 配额型, 非 nv_gw 故障)
...
- finish_reason: tool_calls×8 + stop×2 (健康, 无 zombie stop)
- 全挂时 ABORT-NO-FALLBACK 是预期 (dsv4p_nv 跳 peer/ms fb, cc4101 层 ms_gw 兜底)
- 配置与 R472-R583 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R584 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R584 一致模式)
- key2: 43×200 (主力可用 key)
- key3: 4×200 + 1×502
- 空 key (全挂时): 65×429 + 2×502
→ 命中可用 key �� 100% 200, 全挂时空 key 429 = NVCF 配额型, 非 nv_gw tier 故障
...
- dsv4p_nv 14 req: 10×200+4×429 = NVCF 配额波动区间 (命中 key2 100% 200, 全挂时空 key 429)
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=71.4% vs R583 71.4% vs R582 58.3% vs R581 58.3% vs R580 58.3% vs R579 66.7% → 波动区间内, 与 R545-R583 同一 NVCF 配额波动模式
