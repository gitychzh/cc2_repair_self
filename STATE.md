...
## 当前轮基线 (2026-08-02 12:43 CST, R-nvonly-post225 NOP 巡检轮)
- 本仓 master: 本轮 post225. (主仓 hermes_improve_self main 收 round 文件.)
- **本轮 R-nvonly-post225 (hm2_cc2)**: NOP 巡检轮. cc2 30min/6h 0 req (session 轮前无流量产生, 无数据可判 SR).
  ...
  0 改动, 0 重启.
  hermes 打 dsv4p_nv 16req (11×200/5×429, SR=68.8%, all_tiers_exhausted ×5, 配额限流, key2 单 key 扛 11×200).
  **与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv).
  glm5_2_nv 连续 post100-post225 (123 轮) 无 dsv4p 故障扩散.
...
### 1. cc4101-primary (cc2) 30min + 6h 窗口 — 0 req
本轮 30min 及 6h cc2 均无请求产生. 无数据可判 cc2 SR. 链路健康无故障.

### 2. 其他 caller (hermes, 非 cc2 链路)
| caller | model | 200 | 429 | SR |
|--------|--------|-----|-----|-----|
| hermes | dsv4p_nv | 11 | 5 | 68.8% (16req) |

hermes→dsv4p_nv SR=68.8% (16req): 11×200 + 5×429, all_tiers_exhausted ×5 (avg_dur 3186ms, NVCF 配额限流).
per-key: key2 扛 11×200 (单 key 健康), 5×429 来自无 key 映射 (empty key).
per-egress: 203.10.96.139 扛 11×200 (100% SR), 5×429 来自无 egress (空 IP).
**与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv).

### 3. 30min 错误分类 (全 caller)
- all_tiers_exhausted (hermes→dsv4p_nv): 5× (avg_dur 3186ms), 全为 NVCF 配额限流, 非 cc2 链路.
...
| docker ps | nv_gw/cc4101 Up 11h, nv_gw_stable Up 11h, ms_gw/logs_db Up ✓ |
| cc2 (cc4101-primary) 30min SR | 0 rows (无流量, 链路健康无故障) ✓ |
| cc2 6h 趋势 | 0 rows (无流量) ✓ |
| 30min tier error (全 caller) | 0 rows ✓ |
| 30min 全 caller | hermes 16req dsv4p_nv (11×200/5×429 限流), cc2 0 req ✓ |
| 配置 | NVU_DISABLE_MS_FALLBACK=0 (fallback 已恢复), FALLBACK_UPSTREAM=ms_gw:40007 ✓ |

## 参数快照 (2026-08-02 12:43 CST, 无变化, 同 post223)
