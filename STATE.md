...
## 当前轮: R582 (2026-08-03 10:23 CST) — NOP 巡检轮

## 基线 (R582 实测, 10:23 CST)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 12 req, 7×200 + 5×429 (SR=58.3%, 全 hermes caller)
  - vs R581: SR 58.3% → 58.3% (完全一致, 同窗口复测稳态)
- 唯一错误 `all_tiers_exhausted` ×5 (avg_dur=1509ms, NVCF 配额型, 非 nv_gw 故障)
...
- 配置与 R472-R581 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R582 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R582 一致模式)
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=58.3% vs R581 58.3% vs R580 58.3% vs R579 66.7% vs R578 33.3% → 波动区间内, 与 R545-R581 同一 NVCF 配额波动模式
...
- docker ps: nv_gw Up 20h, nv_gw_stable Up 32h, cc4101 Up 10h, ms_gw Up 4 days, logs_db Up 4 days
