...
> 当前轮: R716 (NOP 巡检, 2026-08-03 21:00 CST)
> 上轮: R715 (NOP, cc2 16req全200 SR100% fb6.3% dsv4p93.1% glm5_2_nv0% NVCF上游退化)

## 本轮 (R716) 改了什么 + 依据 + 验证

### 改动: 不改码 (NOP)

### 依据 (30min 窗口 ~20:30-21:00 CST, 注入数据)
...
- **nv_gw 全量 30min** (hermes/openclaw caller, 非 cc2):
  ...
  - glm5_2_nv: 2×502 = SR **0%** (0/2) — NVCF 上游持续退化 (~10h+, 自 R713 起未恢复)
- **错误分类 (30min)**:
  - all_tiers_exhausted × 4 — 5key 全挂, glm5_2_nv NVCF 上游配额副作用
  - (无新错误类型)
- **per-key (dsv4p)**: k0 6×200, k1 7×200, k2 5×200, k3 6×200, k4 5×200 — 均衡健康
- **per-egress-IP (dsv4p)**: 5 US IP 全 100% (5~7 req 各) — IP 轮转健康
- **dsv4p 200 延迟**: avg 9.9s, max 28.7s, ttfb 9.4s, finish_reason tool_calls×24/stop×3/length×2 (无 zombie)
- **fallback**: f×33 (全非 cc2, hermes/openclaw 流量)
- **buffer/wait/keymanager 日志**: 无 (无 buffer 触发, 链路直接 fallback 到 dsv4p)
- **根因**: glm5_2_nv NVCF 上游持续退化 (~10h+), cc4101 fallback → dsv4p 兜底, cc2 本窗口零流量

### 验证: NOP 无需 restart
- `curl /health`: nv_gw ok(5keys, glm5_2_nv/dsv4p_nv/kimi_nv) + cc4101 ok(primary=glm5_2_nv) + dsv4p_nv40066 ok(5keys) — 全 ok
- `docker ps`: nv_gw Up 4h, cc4101 Up 5h, dsv4p_nv40066 Up 4h, nv_gw_stable Up 42h, logs_db Up 4d — 全 Up
- 配置零漂移 (R661 baseline):
