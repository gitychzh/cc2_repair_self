# R629 — NOP 巡检轮 (2026-08-03 13:57 CST)

## 基线 (R629 实测, 05:21-05:55 UTC 窗口)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本, 同 R626/R627/R628)
- dsv4p_nv 30min (hermes caller): 30 req, 26×200 + 2×429 + 2×502 (SR=86.7%)
  - vs R628 90.3% / R627 90.3% / R626 88.5% / R625 85.0% / R624 82.4% / R623 76.5% / R622 64.3% / R621 57.1%
    → **本轮略降 90.3%→86.7%, 仍在 R617-R618 正常波动区间上沿 (75-91%), 反弹趋势已稳定**
  - per-key: k2=26×200 全 first-attempt 命中
  - per-egress-IP: 203.10.96.139 26×200 / 4 fail (单 IP 主力, 同 R628)
  - finish_reason: tool_calls×23 + stop×3 (健康, 无 zombie)
  - fallback_occurred=f ×30 (hermes caller 直连 nv_gw)
  - 200 avg_dur=12740ms (健康, 同 R628 完全持平)
- 错误分类 1 类 (非新错误):
  - `all_tiers_exhausted` ×4 (avg_dur 18372ms: 429→1311ms 快速ABORT; 502→35433ms 长ABORT)
    → R624→R625→R626→R627→R628→R629: 3→3→3→3→3→4 (略升 1, 仍在 3-4 正常区间)
- 30min nv_tier_attempts: 0 行 (hermes caller 不走 buffer, KeyManager 层 ABORT)
- cc_requests stream_total_deadline 6h: 0 (deadline 链健康)

## 持续观察点 (沿用 R628, 不改码) — RemoteDisconnected 34-35s 长卡顿
- 本轮 2×502 duration≈35433ms, 同 R626/R627/R628 (34646/35433/35433ms) 模式
...
## 依据
- cc2 0 流量 → 无评估样本, 铁律1 cc2 视角不满足 (同 R626/R627/R628)
- dsv4p_nv SR=86.7% 略降 R628 90.3%, 仍在 R617-R618 正常波动区间上沿 (75-91%), 趋势稳定
- all_tiers_exhausted ×4 略升 R628 ×3, 仍在 3-4 正常区间 (R624-R628: 3→3→3→3→3→4), 无退化
- KeyManager 指数退避正确, 429 ABORT avg 1311ms (快速, vs R628 2038ms 略快仍健康)
