...
## 当前轮: R604 (2026-08-03 12:22 CST) — NOP 巡检轮

## 基线 (R604 链路分析注入, 12:22 CST)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 26 req, 22×200 + 2×429 + 2×502 (SR=84.6%, hermes caller)
  - vs R603 84.6% vs R602 76.2% vs R601 66.7% vs R600 61.5% vs R599 45.5% vs R598 50.0% → 波动区间, NVCF 配额型
  - per-key: key2 22×200 (命中可用 key 100% 200, avg_dur=11405ms, IP 203.10.96.139)
  - 空 key 2×429 + 2×502 (全挂时 NVCF 侧拒绝, 429 avg_dur=1470ms, 502 avg_dur=17494ms, 配额波动区间)
...
- 按分钟趋势: 03:56 全挂 429 → 04:01-04:12 恢复 22×200 + 2×502 → 04:16 又 429 (NVCF 配额波动)
- per-egress-IP: 203.10.96.139 22×100 (命中 IP 100% 200), 空 IP 4×0
- 配置与 R472-R603 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R603 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R603 一致模式)
...
- all_tiers_exhausted avg_dur 9482ms (vs R603 9380ms) 持平, 仍 NVCF 配额型 (全挂时 KeyManager 等冷却到期再 abort), 非 nv_gw tier 故障
- 502 avg_dur 17494ms ×2 = peer-fb-skip 慢速返回 (dsv4p_nv 跳 peer fb), 全挂时 NVCF 侧慢拒绝
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=84.6% vs R603 84.6% vs R602 76.2% vs R601 66.7% vs R600 61.5% vs R599 45.5% vs R598 50.0% vs R597 50.0% vs R596 57.1% vs R595 66.7% vs R594 54.5% vs R593 69.2% vs R592 66.7% vs R591 44.4% vs R590 37.5% vs R589 37.5% → 波动区间内, 与 R545-R603 同一 NVCF 配额波动模式
