...
## 当前轮基线 (2026-08-02 20:16 CST, R348 NOP 巡检轮 已完成, R349 待跑)
- 本仓 master: R348 (bc321aa) 已 push. hermes 仓: R348 (8e1a41b) 已 commit+push.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R349 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller (hermes) SR=93.1% (27/29), 失败 2 = 1× all_tiers_exhausted
  (NVCF function 级 429 配额周期单发, 11:55-12:16 自恢复 27×200) + 1× NVStream_IncompleteRead (502,
  transport-level 偶发单发, R345/R347/R348 出现 R346 清零, 周期低频不达介入阈值).
  错误类型无新增, 与 R268-R348 一致. cc2 无流量不受影响, 0 fallback 0 deadline.
  0 改动 0 restart. **七十二轮一致 R268-R349**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~20:16 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R348, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).
- 30min caller×model×status 总览: hermes|dsv4p_nv|200×27, 429×1, 502×1.

### 2. dsv4p_nv 30min 全 caller (hermes) SR=93.1% (27/29)
| caller | model | status | count |
|---|---|---|---|
| hermes | dsv4p_nv | 200 | 27 |
| hermes | dsv4p_nv | 429 | 1 |
| hermes | dsv4p_nv | 502 | 1 |

per-key (dsv4p): key2 → 27×200 (avg_dur 9919) + 1×502 (35151); 空 key → 1×429 (1290).
per-egress: 203.10.96.139 → 28 (含 27×200 + 1×502); 空 IP → 1×429.
finish_reason (200): tool_calls×24, stop×3 (无 zombie).
分钟趋势: 11:50 单发 429; 11:55-12:01 恢复 8×200; 12:02 单发 502; 12:06-12:16 恢复 19×200.
延迟 (200): avg_dur 9919, max 24137, min 4097, avg_ttfb 9665.
fallback 0/29.

### 3. 错误分类 (DB 实测)
- 2 错误: 1× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 1290) + 1× NVStream_IncompleteRead (avg_dur 35151, 502).
- 1× 429: NVCF function 级配额周期单发, key2 同时挂 → all_tiers_exhausted.
- 1× NVStream_IncompleteRead (502): transport-level 偶发单发 (R345/R347/R348 出现→R346 清零→R349 再单发), 不达介入阈值.
- 与 R268-R348 错误类型集合一致.
- tier_attempts 30min 0 行 (429 在 NVCF 侧, 未进 nv_gw tier 重试).
- buffer/wait 日志空.

### 4. 健康检查 (沿用 R348, 容器无 restart)
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R348 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R348 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.
- NVStream_IncompleteRead 偶发单发不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=93.1% (27/29), 较 R348 86.4% (19/22) 回升 (429 波次减, 样本极小自然变动).
- 错误类型无新增, 与 R268-R348 一致 (七十二轮一致).
- 七十二轮一致 R268-R349.
