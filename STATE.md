...
> 当前轮: **R1213 (NOP — SR 100%, 0 错误, fallback 0%, 防御链按设计工作)**
...
> 最新窗口: 30min cc2-primary 200|109, cc_requests 109 全 200 fallback 0%, dsv4f0731_nv 全量 172/172 SR 100%
...
- **30min cc2-primary (nv_requests)**: `200|109`, **无 502/4xx** → SR=**100% (109/109)**
  (活查确认, 注入快照 107 已随窗口滚动增至 109)。R1206 残留 `7f34c956` 早已滑出滚动窗口。
...
- **容器健康**: nv_gw /health ok (5 keys + pexec_models 含 dsv4f0731_nv, fid 281478d0-f307),
  cc4101 ok (primary=dsv4f0731_nv), ms_gw ok, logs_db ok。无重配置迹象, 参数与 R1212 一致 → 非配置回归。

### 验证
活查 30min cc2-primary 109/109 (0 错误), dsv4f0731_nv 全量 172/172 SR 100%、fallback 0%;
容器 health ok、参数无漂移。k2/k3 各 1 次 attempt 级瞬时抖动被重试自愈到 200, 无任何净新增。
→ 无改码条件, NOP。

## 参数快照 (nv_gw + cc4101, 与 R1212 一致)
...
## 上轮
R1212 (NOP — SR 100%, 0 错误, fallback 0%) → R1213: 维持 SR 100%, fallback 0%,
