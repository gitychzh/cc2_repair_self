...
> 当前轮: R748 (NOP 巡检, 2026-08-05 05:31 CST)
> 上轮: R747 (NOP, cc2 30min SR 100%/fb 0%, 第 13 连续 100%)

## 本轮 (R748) 改了什么 + 依据 + 验证

### 改动: 不改码 (NOP)

### 依据 (实测 ~05:31 CST, 30min 窗, created_at 实测核验)
- **cc2 (cc4101-primary) glm5_2_nv: nv_requests 75×200 (SR=100%) + cc_requests 75×200 (fb=0, SR=100%)** — 连续第 14 轮 100%
- 注入的 "f|88" 在 fallback 发生率段 → created_at 实测: 75 req / 0 fb (ts 列时区 bug 口径, 沿 R730/R742-R747 实证)
- per-key pexec_success 实测: k0=15, k1=16, k2=15, k3=16, k4=14 = 总 76, 与 cc2 75×200 完全一致 (差 1 边界) — 本轮最干净, 全 5key 仅 pexec_success 无任何错误穿透
- 注入的 NVCFPexecRemoteDisconnected=15 / 529_nv_overloaded=3 / empty_200=1 / all_tiers_exhausted=7 / NVStream_IncompleteRead=2 全部来自 hermes→dsv4f0731_nv 上游 NVCF 容量, 不在 cc2 路径

### 验证 (NOP 无需 restart)
- `/health`: nv_gw ok (5 keys, glm5_2_nv default) + cc4101 ok (primary=glm5_2_nv) — 全 ok
- `docker ps`: nv_gw Up 3h, cc4101 Up 4h, dsv4p_nv40066 Up 9h, dsvf0731_nv40666 Up 2h, nv_gw_stable Up 3d, logs_db Up 5d — 全 Up
- env 沿 R747, 无漂移

## 判稳结论
- **cc2 nv_gw 链路连续 14 轮 (R735~R748) SR 100%, fb 0%** — 全面达标 (目标 SR 99%+/fb <10%)
- 本轮 per-key 全 pexec_success 无任何 cc2 错误穿透 — 最干净一轮
...
| R748 | 100% (75 nv / 75 cc) | 14th consecutive, fb=0 (created_at 实测), 注入 "f\|88" 是 ts 时区 bug, 最干净一轮 |
