...
> 当前轮: **R1215 (NOP — SR 100%, 0 错误, fallback 0%, 防御链按设计工作)**
...
> 最新窗口: 30min cc2-primary 200|107, cc_requests 全 200 fallback 0%, dsv4f0731_nv 全量 172/172 SR 100%
...
- **30min cc2-primary (nv_requests)**: `200|107`, **无 502/4xx** → SR=**100% (107/107)**
  (aktiv 查询确认)。R1206 残留早已滑出滚动窗口。
- **30min 错误分类 (活查)**: 空 (0 rows) → 无 buffer_exhausted / stream_total_deadline / 其他。
- **dsv4f0731_nv 全量 (含 hermes 65)**: 172/172 SR=**100%** (注入快照: 107+65)。
...
- **per-key tier** (nv_tier_attempts): k2 NVCFPexecRemoteDisconnected×1, 其余全 pexec_success →
  均为 attempt 级瞬时抖动, 被重试自愈到 status 200, 非净新增 (与 R1212-R1214 同签名的
  链路单点瞬抖签名一致)。
...
- **容器健康**: nv_gw /health ok (5 keys + pexec_models 含 dsv4f0731_nv, fid 281478d0-f307),
  cc4101 ok, ms_gw ok, logs_db ok。参数与 R1214 一致 → 非配置回归。

### 验证
活查 30min cc2-primary 107/107 (0 错误), dsv4f0731_nv 全量 172/172 SR 100%、fallback 0%;
容器 health ok、参数无漂移。k2 1 次 attempt 级瞬时抖动被重试自愈到 200, 无任何净新增。
→ 无改码条件, NOP。

## 参数快照 (nv_gw + cc4101, 与 R1214 一致)
...
## 上轮
R1214 (NOP — SR 100%, 0 错误, fallback 0%) → R1215: 维持 SR 100%, fallback 0%,
全量 dsv4f0731_nv 172/172。R1206 SSLEOFError/Remote-end-closed 瞬时 egress 抖动统计影响持续闭合。
