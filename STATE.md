...
## 当前轮基线 (2026-08-02 ~19:05 CST, R334 NOP 巡检轮)
- 本仓 master: 上轮 R333 (2392e36) 已 push. 本仓 up to date.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R334 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=68.8% (11/16): 10:35-10:50 一波 429×4 (NVCF function 配额周期)
  → 10:55-10:56 自恢复 5×200 → 10:58 1×502(all_tiers_exhausted) → 11:00-11:01 恢复 5×200 → 11:05 1×200.
...
- top: all_tiers_exhausted × 5 (sub=all_tiers_failed_in_mapped_tier, avg_dur 15619).
  ...
  1× all_tiers_exhausted (key_idx 空, egress 空).
- 本轮 30min 注入窗口内无 NVStream_IncompleteRead.
  60min 窗口外 (10:06:55) 有 1× NVStream_IncompleteRead (key2, egress 203.10.96.139, 34s)
  — 历史已知偶发 mid-stream 软挂单发, 非新错误类型, 与 R325 同源.
- 与 R268-R333 错误类型集合一致.
- tier_attempts 30min 0 行 (429 在 NVCF 侧, 未进入 nv_gw tier 重试).
- buffer/wait 日志空.

### 4. 健康检查 (沿用 R333 实测, 本轮 0 改动 0 restart)
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R333 分析)
...
- 本轮 10:35-10:50 一波 429×4 → 10:55-11:05 恢复 11×200 证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R334 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=68.8% (11/16): 窗口命中 1 波 429×4 后自恢复 11×200, 根因不变 (NVCF function 配额周期).
- 错误类型无新增, 与 R268-R333 一致.
- 六十七轮一致 R268-R334.
