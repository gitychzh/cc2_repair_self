...
## 当前轮基线 (2026-08-02 17:20 CST, R305 NOP 巡检轮)
- 本仓 master: 本轮 R305 (push 待办). 上轮 R304 (42faa29) 已 commit.
- **架构 (主仓 38afea2)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R305 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=95.3% (41/43), 失败 2 全 all_tiers_exhausted
  (17:12 + 17:15 两波 429 NVCF function 配额周期, 5key 同时挂, 自恢复).
  错误类型无新增 (全 all_tiers_exhausted), 与 R268-R304 一致.
...
## 本轮关键数据 (30min 实时链路分析注入 ~17:19 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R304, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).

### 2. dsv4p_nv 30min 全 caller SR=95.3% (41/43)
...
per-key (dsv4p): key2 → 40×200 (avg_dur 10897), key3 → 1×200 (5781), 空 key → 2×429 (1508).
per-egress: 203.10.96.139 → 40×100, 134.195.101.194 → 1×100, 空 IP → 2×0 (429).
finish_reason: tool_calls×35, stop×6 (无 zombie).
分钟趋势: 08:50-09:11 持续多波 200, 09:12 一波 429, 09:15 一波 429 (配额周期命中两波).
延迟: avg_dur 10773, max 24567, min 3861, avg_ttfb 10479.
fallback 0/43.

### 3. 错误分类 (DB 实测)
- 2 错误: 全 all_tiers_exhausted (avg_dur 1508).
- 两波 429 → 5key 同 function 同时挂 → all_tiers_exhausted.
- 与 R268-R304 错误类型集合一致 (all_tiers_exhausted 历史仍存在).
- tier_attempts 30min 0 行 (429 在 NVCF 侧, 未进入 nv_gw tier 重试).
- buffer/wait 日志空.

### 4. KEYMGR 日志 (本轮 429 波铁证)
- 17:12:08 第一波 429 — k1/k2/k3/k4/k5 同时收 429, 各 count=1 decayed reset,
  cooldown k3=120s, k1/k2/k4/k5=180s (count decayed >300s 重置).
- 17:15:36 第二波 429 — k1/k2/k3/k4/k5 同时再 429, count 升至 2-4,
  k3 count=4 cooldown=480s (指数退避), 其他 count=2 cooldown=180s.
- 证实: 5key 全绑同一 NVCF function, function 级配额耗尽时 5key 同时挂.
- 这是 function 级配额周期, 非 key/IP 级故障, buffer 5key 轮转设计对此失效.

### 5. 健康检查
- /health 200, nv_num_keys=5, default glm5_2_nv, pexec_models=[kimi_nv, dsv4p_nv, glm5_2_nv].
- 容器: nv_gw 3h, cc4101 3h, nv_gw_stable 15h, ms_gw 3d, logs_db 3d 全部 Up.

## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R304 分析)
...
- 本轮 17:12+17:15 两波 429 证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R305 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=95.3% (窗口末尾命中两波 429, NVCF function 配额周期).
- 三十八轮一致 R268-R305.
...
- 容器: nv_gw 3h, cc4101 3h, nv_gw_stable 15h, ms_gw 3d, logs_db 3d 全 Up.
