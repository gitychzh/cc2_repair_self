# R390: NOP 巡检轮 (cc2 0req, dsv4p_nv SR=0.0% 0/6, all_tiers_exhausted×6[5×429+1×502 avg 3504ms], 一百一十四轮一致)

## 当前轮基线 (2026-08-02 22:52 CST, R389 已完成, R390 待跑)
- 本仓 master: R389 已 commit (39f2ec1). hermes 仓: R389 已 push (ca5d6b6).
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R390 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  ...
  **cc2 是缓冲 caller (NVU_BUFFER_CALLERS), 走 buffer 5key 轮转路径, 不走 mapped-tier 直接失败, 不受影响.**
  错误类型无新增 (dsv4p 仍 all_tiers_exhausted, 一百一十四轮一致 R268-R390).
  cc2 无流量不受影响, 0 fallback 0 deadline. 0 改动 0 restart. **一百一十四轮一致 R268-R390**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~22:52 CST)
...
- 6 dsv4p 错误: 6× all_tiers_exhausted (sub=`all_tiers_failed_in_mapped_tier`, avg 3504ms)
  ...
  - 本轮无 NV-TIER-SKIP (R389 0×, 本轮 0×, 自然波动).
...
- dsv4p 错误类型集合与 R268-R389 一致 (all_tiers_exhausted, 无新增).
- buffer/wait 日志空.

### 4. 健康检查 (沿用 R389, 容器未重启)
...
## 根因: NVCF dsv4p function 429/502 波 (非代码缺陷, 沿用 R353-R389 分析)
- **非 nv_gw 代码缺陷, 无需本轮改码**.
- all_tiers_exhausted (429/502): NVCF dsv4p function 配额瞬时空位/上游瞬时错误, 低频偶发,
  buffer + KeyManager 指数退避 + ProbeWorker + decayed reset 自恢复 (R268-R389 验证).
...
- 连续 3 轮下滑 (R388 72.2% → R389 14.3% → R390 0%) 但样本基数从 18→7→6 递减且全非缓冲 caller,
  无统计学意义, 错误类型集合不变, 仍判为 NVCF 配额波动而非链路退化.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=0.0% (0/6), 较 R389 14.3% (1/7) 继续下降 (样本极小 + NVCF 本窗口配额紧张,
  全部非缓冲 caller, cc2 不受影响). dsv4p 错误类型无新增, 与 R268-R389 一致 (一百一十四轮一致).
