...
## 当前轮基线 (2026-08-02 16:55 CST, R298 NOP 巡检轮)
- 本仓 master: 本轮 R298 (push 待办). 主仓 hermes_improve_self main `daf06d3` R297 round.
- **架构 (主仓 b4527f9, 非本轮)**: cc4101 `PRIMARY_UPSTREAM_MODEL` 已从
  `glm5_2_nv` 切到 `dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R298 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
...
## 本轮关键数据 (30min 实时 DB + 链路分析注入 ~16:55 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R297, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
### 3. 错误分类 (DB 实测)
- 全 `all_tiers_exhausted` (3 条), 无新错误类型, 与 R268-R297 一致.
- fallback 0/12.
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R297 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R298 验证), 对 function 级 429 是已知盲区.
