...
> 当前轮: R876 (NOP 巡检轮 — 近窗 cc4101-primary SR=100% (110×200) 零错误, 残留 all_tiers_exhausted×5 全为 caller=hermes 外部 cron (链路总览仅 hermes/502/5), fallback 0%, buffer 全 attempt1 一次成交, 不改码, 2026-08-07 ~05:54 CST)
> 上轮: R875 (NOP — 近窗 cc4101-primary SR=100% (114×200) 零错误, all_tiers_exhausted×6 全为 hermes 外部 cron (DB 复核 hermes/502/6), 不改码)

## 本轮 (R876) 改动 + 依据 + 验证

### 改动: 无 (巡检轮 — cc2 路径全净 110×200, hermes 周期 all_tiers_exhausted 与 cc2 无关)

### 本轮数据 (~05:54 CST, 轮前链路分析注入, DB UTC)

**最近 30min cc4101-primary (cc2 自己路径) SR = 100% (110×200, 零错误).**

| 指标 | 值 | 状态 |
|---|---|---|
| **最近 30min cc4101-primary SR** | **100% (110×200)** | ✅ |
| **primary 目标 tier** | **dsv4f0731_nv** (自适应轮转持有, /health 确认) | ✅ |
| **30min 按模型 SR** | dsv4f0731_nv = 95.7% (111/116, 注入快照); cc4101-primary 110×200 | cc2 路径 100% |
| **error 归属** | all_tiers_exhausted×5 avg~179s 全为 caller=hermes (链路总览仅 hermes/502/5, 非 cc4101) | ✅ 与 cc2 无关 |
| **非 200 归属** | 仅 `hermes/502/5` (链路总览), cc4101-primary 0 错误 | ✅ |
| **fallback 触发率** | 0 (116 请求 0 fallback) | ✅ |
| **buffer** | 无 buffer/wait 日志 (近窗全 attempt1 一次成交, 无 multi-attempt 退化) | ✅ |
| **per-key nv_tier_attempts** | dsv4f0731_nv 5key 均 22 次 pexec_success (共 ~110 成功), NVCFPexecRemoteDisconnected 瞬态 3~5/key + Timeout + 529_nv_overloaded 被跨 key 吸收 | ✅ |
| **三容器 health** | nv_gw / cc4101 / dsv4p 均 ok, primary=dsv4f0731_nv, 5 keys | ✅ |

### 关键判断: cc2 路径全净, hermes 周期 all_tiers_exhausted 非本链路问题

30min 窗口链路总览: cc4101-primary **110×200 零错误**; 非 200 (502×5) 全部 caller=hermes
(外部客户端, 非 cc4101): all_tiers_exhausted×5, avg ~179s ≈ buffer deadline 特征 —— 属 hermes
外部 cron 周期性请求(沿用 R853-R876 判定), 而非本链路退化.

buffer 近窗全 attempt1 一次成交, 无 multi-attempt 退化, 证明 KeyManager/跨 key round-robin
修复链健康. cc2 自身路径 110×200 零错误, fallback 0%, 无 buffer/wait 退化, 链路/KeyManager 无退化. 不改码.
