# R538 — NOP 巡检轮 (2026-08-03 07:34 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (延续 R525-R537 间歇空闲窗口).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无 cc2 评估样本, 铁律1 cc2 视角不满足 → 不动码).
- dsv4p_nv 30min: 15 req SR=66.7% (10×200 + 4×429 + 1×502), 全 `hermes` caller (1× openclaw 200).
  SR 较 R537 的 62.5% 略升 +4.2pp, 仍在 NVCF 侧配额波动区间 (40-65% band, R530-R537).
- 唯一错误类型 `all_tiers_exhausted` × 5 (avg 12883ms = 12.9s), 无新错误类型.
- 1× 502 = peer-fb-skip 已知路径 (23:17 UTC), R530-R537 同源, 非新错误.
- NV-GLOBAL-COOLDOWN tier=dsv4p_nv 周期性 429 仍在 (23:05/23:10/23:20/23:26 UTC),
  与 R268-R537 完全一致, NVCF 侧配额波动.
...
- dsv4p_nv 15 req: 10×200 (k2=9 + k3=1) + 4×429 (每 5min GLOBAL-COOLDOWN 180s) + 1×502 (peer-fb-skip)
- 唯一错误类型: `all_tiers_exhausted` × 5, avg_dur=12883ms (全 NVCF 配额型, 非 nv_gw 故障)
- per-key (dsv4p): k2=9×200 (8970ms avg), k3=1×200 (2408ms), 其余 key 全 429
- per-egress-IP: 203.10.96.139=9×100% SR, 134.195.101.194=1×100% SR, 其余 0 success
- 200 finish_reason: tool_calls × 8, stop × 2 (无 zombie)
- fallback_occurred=f × 15 (nv_gw 层 dsv4p_nv 不 fallback, cc4101 层兜底)
- 全局冷却模式: 每 5min 1 次 NV-GLOBAL-COOLDOWN tier=dsv4p_nv all keys 429, 与 R268-R537 一致
...
- 502 (peer-fb-skip) 持续 >=6/h 且 cc2 流量恢复, 再评估 dsv4p_nv fallback 策略:
  ...
  (c) 增加 ring fallback
- k3 53s RemoteDisconnected 传输挂死若频次上升再评估 (本轮 0 次)
