# R486 — NOP 巡检轮 (2026-08-03 05:00 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 20:00-20:25 UTC (04:30 CST 注入, 05:00 psql 复测确认), 与 R475-R485 同低谷窗口高度一致 (连续 12 轮锁定 14.3%).
- cc2 (cc4101-primary) 30min 0 req (psql 05:00 复测 0 rows, cc2 session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=14.3% (1/7, 1×200 + 6×429), 与 R483/R484/R485 完全持平, 仍处低谷窗口.
- 错误: all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 与 R268-R485 (177+ 轮) 一致, 无新错误.
- nv_tier_attempts 30min 0 行 (429 在 tier 层前被 KeyManager 全局冷却拦截, 空 IP, 与 R470-R485 一致).
- fallback_to 30min 7 次空值 (429 未实际 fallback, 由 cc4101 层走 ms_gw glm5_2_ms 兜底).
- stream_total_deadline 6h 0 次 (deadline 链对齐稳, 复测 0 rows 确认).
- per-min 趋势: 每 5min 1 次离散 429 (20:00/20:05/20:11/20:15/20:20/20:25), 20:10 唯一一次 200 (dur=7760ms, tool_calls finish), 非集中爆发.
- nv_gw Up 14h, cc4101 Up 4h, nv_gw_stable Up 27h, ms_gw Up 3 days (本轮未重启).
- 配置实测确认与 R475-R485 完全一致, 无漂移.

## 链路数据 (04:30 CST 注入, 05:00 psql 复测确认)
...
- nv_tier_attempts: 0 行 (429 未进入 tier 尝试, 空 IP, 与 R470-R485 一致)
- fallback_to: 7 行空值 (429 在 nv_gw 层未实际 fallback, 由 cc4101 层走 ms_gw glm5_2_ms 兜底)
- per-min: 20:00|429, 20:05|429, 20:10|200, 20:11|429, 20:15|429, 20:20|429, 20:25|429 (离散, 非爆发)

### cc4101-primary 专属 (cc2 的请求)
- 30min 0 req (psql 05:00 复测 0 rows 确认, cc2 session 间歇空闲)

### 6h dsv4p_nv 全 caller SR 基线
- (R485=69.5% 持平, 仍处 46-85% 历史波动区间)
- 502 6h 低频 (与 R476/R480-R485 一致)

### nv_gw COOLDOWN 日志 (与 R485 一致, 30min 无 buffer/wait 日志)
...
- 这是历史一致行为 (R268 起 177+ 轮), 非本轮新故障.

## 历史波动区间 (R437-R486)
R437=85.0 → ... → R467=44.4 → R468-R472=44.4 → R473-R481=0.0% → R482-R486=14.3% (30min 低谷, 6h 视角 69.5%)

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 模式与 R268-R485 一致 (177+ 轮), 无新错误.
- dsv4p_nv 30min SR=14.3% (与 R483-R485 持平) 仍是低谷窗口, 6h SR=69.5% 仍处 46-85% 历史波动区间, 属稳态周期性行为.
...
- 留意 502 是否再现 (R476/R480-R485 记 6h 3-4×502 低频, 再现 >=3/h 才介入).
