...
## 当前轮基线 (2026-08-02 17:38 CST, R309 NOP 巡检轮)
- 本仓 master: 本轮 R309 (round 文件已写, 待 commit+push). 上轮 R308 (hm 3996cbf) 已 push.
- **架构 (主仓 3996cbf)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R309 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=64.7% (11/17), 失败 6 全 all_tiers_exhausted
  (09:12+09:15+09:20+09:25+09:30+09:35 六波 429 NVCF function 配额周期, 5key 同时挂, 自恢复).
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R308 分析)
...
- 本轮 09:12+09:15+09:20+09:25+09:30+09:35 六波 429 证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R309 验证), 对 function 级 429 是已知盲区.
