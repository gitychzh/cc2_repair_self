...
## 当前轮基线 (2026-08-02 19:49 CST, R344 NOP 巡检轮)
- 本仓 master: 上轮 R343 (2615fc0) 已 push. 本轮 R344.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R344 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller (hermes) SR=50.0% (6/12), 失败 6 = 6× all_tiers_exhausted
  (NVCF function 级 429 配额周期多波). 本轮无 NVStream_IncompleteRead; 502 1× 偶发 transport-level.
  错误类型无新增, 与 R268-R343 一致. cc2 无流量不受影响, 0 fallback 0 deadline.
  0 改动 0 restart. **六十六轮一致 R268-R344**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~19:49 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R343, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).
- 30min caller×model×status 总览: hermes|dsv4p_nv|200×6, 429×5, 502×1.

### 2. dsv4p_nv 30min 全 caller SR=50.0% (6/12)
...
per-key (dsv4p): key2 → 6×200 (avg_dur 11914); 空 key → 5×429 (1954) + 1×502 (34101, transport-level hang).
per-egress: 203.10.96.139 → 6×100%; 空 IP → 6×0%.
finish_reason (200): tool_calls×6 (无 zombie).
分钟趋势: 11:25-11:27 多波 200; 11:27 1×502; 11:30/11:35/11:40/11:45 持续 429.
延迟 (200): avg_dur 11914, max 19630, min 4856, avg_ttfb 11387.
fallback 0/12.

### 3. 错误分类 (DB 实测)
- 6 错误: 6× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 7312).
- 429 (5×): NVCF function 级配额周期, 多波, key2 同时挂 → all_tiers_exhausted.
- 502 (1×, dur 34101ms): 同 R336-R343 RemoteDisconnected transport-level hang (单发, 偶发).
- 本轮无 NVStream_IncompleteRead.
- 与 R268-R343 错误类型集合一致.
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R343 分析)
...
- 502 RemoteDisconnected transport hang: 同 R336-R343, NVCF 单 function 偶发 transport 级断连, peer 也降级时返回 502 (设计). 偶发不达介入阈值.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R343 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=50.0% (6/12), 与 R343 同窗口数据漂移 (7/14 → 6/12, 窗口滚动自然变动, 429 波次略减).
- 错误类型无新增, 与 R268-R343 一致.
- 六十六轮一致 R268-R344.
