# R380: NOP 巡检轮 (cc2 0req, dsv4p_nv SR=85.0% 17/20, all_tiers_exhausted×3[3×429 1877ms], 一百零四轮一致)

## 当前轮基线 (2026-08-02 22:14 CST, R379 已完成, R380 待跑)
- 本仓 master: R379 已 push (b0e52b6). hermes 仓: R380 待 push.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R380 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=85.0% (17/20), 失败 3 = 3× all_tiers_exhausted
  ...
  **cc2 是缓冲 caller (NVU_BUFFER_CALLERS), 走 buffer 5key 轮转路径, 不走 NV-TIER-SKIP, 不受影响.**
  错误类型无新增 (dsv4p 仍 all_tiers_exhausted, 一百零四轮一致 R268-R380).
  cc2 无流量不受影响, 0 fallback 0 deadline. 0 改动 0 restart. **一百零四轮一致 R268-R380**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~22:14 CST)
...
|---|---|---|---|---|
| hermes | dsv4p_nv | 200 | 17 | 11385 |
| hermes | dsv4p_nv | 429 | 3 | 1877 |

per-key (dsv4p): key2 → 17×200 (11385); 空 key → 3×429 (1877).
per-egress: 203.10.96.139 → 17× (100%); 空 → 3× (0%).
finish_reason (200): tool_calls×14, stop×3 (无 zombie).
分钟趋势: 13:45 429×1, 13:50 429×1, 13:55 429×1, 14:00 200×1, 14:01 200×2, 14:05 200×1, 14:06 200×4, 14:10 200×2, 14:11 200×5, 14:12 200×2.
延迟 (200): avg_dur 11385, max 18545, min 2759, avg_ttfb 10838, avg_in 0, avg_out 0.
fallback f×20 (全部 false, 0 fallback).

### 3. 错误分类 (30min)
- 3 dsv4p 错误: 3× all_tiers_exhausted (avg 1877ms) — 全 429 (NVCF dsv4p function 配额瞬时空位).
  - 本轮无 NV-TIER-SKIP (R379 0×, 本轮 0×, 自然波动).
  - 429: NVCF dsv4p function 配额瞬时空位, 低频偶发, buffer 5key 轮转 + KeyManager 指数退避自恢复.
- dsv4p 错误类型集合与 R268-R379 一致 (all_tiers_exhausted, 无新增).
- buffer/wait 日志空.

### 4. 健康检查 (沿用 R379, 容器未重启)
...
## 根因: NVCF dsv4p function 429 波 (非代码缺陷, 沿用 R353-R379 分析)

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**.
- all_tiers_exhausted (429): NVCF dsv4p function 配额瞬时空位, 低频偶发 (3/20=15%), buffer + KeyManager 指数退避 + ProbeWorker + decayed reset 自恢复 (R268-R379 验证).
- 本轮无 NV-TIER-SKIP 无 stream_first_byte_timeout, 错误面收敛.
- 当前 cc2 流量极低, 偶发错误罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=85.0% (17/20), 较 R379 81.3% (13/16) 微升 (样本极小自然波动,
  本轮成功 17 vs 13, 失败 3 vs 3, 全部非缓冲 caller, cc2 不受影响).
  dsv4p 错误类型无新增, 与 R268-R379 一致 (一百零四轮一致).
