# R480 — NOP 巡检轮 (2026-08-03 04:20 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 19:35-20:05 UTC (04:08 CST 注入 + 04:20 复测), 与 R475-R479 同低谷窗口高度一致 (连续 6 轮锁定 0%).
- cc2 (cc4101-primary) 30min 0 req (cc2 session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=0.0% (0/6, 全 429), 与 R475-R479 同窗口一致.
- 6h dsv4p_nv 全 caller SR=73.3% (200=151/429=51/502=4, 共 206), 与 R476-R479 的 ~75% 一致, 处 46-85% 历史波动区间.
- 错误: all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 与 R475-R479 一致.
- nv_tier_attempts 30min 0 行 (429 在 tier 层前被拒, 空 IP, 与 R470-R479 一致).
- 12h per-hour 趋势: 08-18 UTC 全程稳态 (SR ~80%), 19:00 UTC 转折低谷 (200=4/429=11), 20:00 UTC 429=2 (低谷持续).
- fallback f=6/30min (dsv4p 全挂走 ms_gw 兜底, 链路有保障).
- nv_gw Up 14h, cc4101 Up 3h, nv_gw_stable Up 26h, ms_gw Up 3 days (本轮未重启).
- 配置实测确认与 R475-R479 完全一致, 无漂移.

## 链路数据 (04:08 CST 注入 + 04:20 复测)
### 30min 窗口 (hermes caller, 全 dsv4p_nv)
- 6×429, SR 0.0% (与 R475-R479 同窗口, R467-R480 十三轮低谷锁定)
- 错误分类: all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier)
- nv_tier_attempts: 0 行 (429 未进入 tier 尝试, 空 IP, 与 R470-R479 一致)
...
- 200=151, 429=51, 502=4, 共 206 → SR=73.3% (R479 ~75%, 一致)
- 502 6h 4 次 (与 R476 一致, 低频)

### 12h per-hour 趋势 (dsv4p_nv)
- 08:00 UTC: 200=14, 429=6 (SR 70.0%)
- 09:00-18:00 UTC: 稳态, 200 22-44/h, 429 6-9/h (SR ~80%)
- 19:00 UTC: 200=4, 429=11 (SR 26.7%, 低谷转折点)
- 20:00 UTC: 429=2 (低谷持续, 无 200)
- 结论: 08-18 UTC 稳态可用, 19:00+ 进入配额低谷窗口, 与 R475-R479 一致.

### nv_gw COOLDOWN 日志 (04:20 复测, 与注入一致)
```
[03:40-04:05] 每 5min: [NV-COOLDOWN] tier=dsv4p_nv k3 marked cooling after 429
...
## 历史波动区间 (R437-R480)
R437=85.0 → ... → R467=44.4 → R468-R472=44.4 → R473-R480=0.0% (30min 低谷, 6h 视角 73-75%)

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier), 模式与 R268-R479 一致 (171+ 轮), 无新错误.
- dsv4p_nv 30min SR=0% 是低谷窗口, 6h SR=73.3% 仍处 46-85% 历史波动区间, 属稳态周期性行为.
...
- 配置实测与 R475-R479 完全一致, 无配置漂移.
- 注入数据 nv_gw "26 hours ago" 实为 nv_gw_stable Up 26h, nv_gw 本身 Up 14h (R479 14h, 0 增长, 无漂移).
...
- 留意 502 是否再现 (R476/R480 记 6h 4×502 低频, 再现 >=3/h 才介入).
