...
## 当前轮基线 (2026-08-02 17:48 CST, R312 NOP 巡检轮)
- 本仓 master: 本轮 R312 已 commit+push. 上轮 R311 (a11441f) 已 push.
- **架构 (主仓 3996cbf)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R312 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=61.5% (8/13), 失败 5 全 all_tiers_exhausted
  (09:20+09:25+09:30+09:35+09:45 五波 429 NVCF function 配额周期, 5key 同时挂, 自恢复).
  错误类型无新增 (全 all_tiers_exhausted), 与 R268-R311 一致.
...
## 本轮关键数据 (30min 实时链路分析注入 ~17:48 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R311, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
per-key (dsv4p): key2 → 8×200 (avg_dur 10112), 空 key → 5×429 (1629).
per-egress: 203.10.96.139 → 8×100, 空 IP → 5×0 (429).
finish_reason: tool_calls×7, stop×1 (无 zombie).
分钟趋势: 09:20/09:25/09:30/09:35/09:45 五波 429, 09:40-09:41 恢复 8×200 (配额周期自恢复).
延迟 (200): avg_dur 10112, max 19004, min 4669, avg_ttfb 9797.
fallback 0/13.

### 3. 错误分类 (DB 实测)
- 5 错误: 全 all_tiers_exhausted (avg_dur 1629, sub=all_tiers_failed_in_mapped_tier).
- 09:20/09:25/09:30/09:35/09:45 五波 429 → 5key 同 function 同时挂 → all_tiers_exhausted.
- 与 R268-R311 错误类型集合一致 (all_tiers_exhausted 历史仍存在).
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R311 分析)
...
- 本轮 09:20+09:25+09:30+09:35+09:45 五波 429 证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R312 验证), 对 function 级 429 是已知盲区.
