...
## 当前轮基线 (2026-08-02 ~18:55 CST, R331 NOP 巡检轮)
- 本仓 master: 上轮 R330 (e7329db) 已 push. 本仓 up to date.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R331 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=0.0% (0/6), 6 = 6× all_tiers_exhausted
  (10:25-10:50 一波 429 NVCF function 配额周期, 5key 同时挂, 自恢复).
  本轮无 NVStream_IncompleteRead (R325 有 1, R326-R331 均为 0).
  错误类型无新增, 与 R268-R330 一致.
...
## 本轮关键数据 (30min 实时链路分析注入 ~18:54 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R330, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
per-key (dsv4p): 空 key → 6×429 (KEYMGR 阶段即拦, 无 egress).
per-egress: 空 IP → 6×0.
分钟趋势: 10:25-10:50 一波 429×6 (每 5min 1 发) → all_tiers_exhausted.
fallback 0/6.

### 3. 错误分类 (DB 实测)
- 6 错误: 6× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 1470).
- 10:25-10:50 一波 429 → 5key 同 function 同时挂 → all_tiers_exhausted.
- 本轮无 NVStream_IncompleteRead (R325 有 1, 历史偶发 mid-stream 软挂单发, 非新错误类型).
- 与 R268-R330 错误类型集合一致.
- tier_attempts 30min 0 行 (429 在 NVCF 侧, 未进入 nv_gw tier 重试).
- buffer/wait 日志空.

### 4. 健康检查 (沿用 R330 实测, 本轮 0 改动 0 restart)
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R330 分析)
...
- 本轮 10:25-10:50 一波 429×6 证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R331 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=0.0% (0/6) (窗口命中 1 波 429×6, NVCF function 配额周期;
  总 req 6 少, 故 SR 数值偏低但根因不变).
- 错误类型无新增, 与 R268-R330 一致.
- 六十四轮一致 R268-R331.
