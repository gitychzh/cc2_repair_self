...
> 当前轮: **R992 (NOP 巡检轮/不改码 — cc2 主链路连续第 100 轮 100% 干净; 唯一坏 3: all_tiers_exhausted ×2 + zombie_empty_completion ×1 全属 hermes, 越界非 cc2 范围; fallback 0 次)**
...
> 上轮: R991 (NOP, 主链 125/125=100%)

## 本轮 (R992) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 100 轮 100% 干净, 主专属错误 0 行; 唯一 bad 全属 hermes+已知坏 fid 52e1ddb6 线)

### 依据 (live 复核 2026-08-07 + 注入轮前链路分析)

- 30min cc4101-primary (主 nv_gw:40006) = **117/117 全 200 = 100% SR, 0 bad** (live re-pull)。
- 总 nv_requests bad (非 200) = **3 条, 全 caller=hermes 线** (live 复核):
  all_tiers_exhausted ×2 + zombie_empty_completion ×1, 已知坏 fid 52e1ddb6 hermes
  宿主越界容器, non cc2 主链。
- fallback (cc_requests 30min) = **0 次** (live total 1907 req, fb=0)。
- nv_tier_attempts 30min: bad fid 52e1ddb6 上 NVCFPexecRemoteDisconnected ×20 (hermes 线),
  全被宿主+候选池双层隔离吸收, 无泄漏为 cc2 主链 200 以外。
- buffer 日志 (nv_gw live): 所有 cc2 primary 请求 attempt=1 一次成功, **无 502 泄漏给 CC**。
...
| 唯一 bad (非 200) | 3 条, 全 caller=hermes 线 (all_tiers×2 + zombie×1) | ⚠️ 越界 |
| bad caller 归属 | 100% hermes 线; cc2 primary 0 条 | ✅ 隔离 |
| fallback (cc_requests) | 0 次 (1907 req 全未 fb) | ✅ |
| 容器 | nv_gw Up, cc4101 Up, health 全 200 | ✅ |

### 验证
- 30min nv_requests cc4101-primary = 117/117 (0 bad)。
- bad 分组 (by caller×err_type): hermes 线 all_tiers_exhausted ×2 + zombie_empty_completion ×1 (fid 52e1ddb6) —
  全 hermes, cc2 主链 0 bad。
- cc_requests fallback = 0 次 (1907 req 全未 fallback_triggered)。
- health: 4101/40006 全 200; 容器 nv_gw/cc4101 皆 Up。
- buffer: cc2 侧全 attempt=1 一次成功, 无泄漏 502。

### 关键判断
cc2 主链路连续第 **100** 轮 (R893-R992) 100% SR 干净, 且主链专属错误 0 rows。
唯一 3 个 bad 请求 100% 属 hermes 线 (caller=hermes + 已知坏 fid 52e1ddb6, all_tiers ×2 +
