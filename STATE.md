...
## 当前轮基线 (2026-08-02 11:19 CST, R-nvonly-post193 NOP 巡检轮)
- 主仓 git HEAD: 5fb3d52 (post192 已 push, 本轮 post193 待 commit+push).
- **本轮 R-nvonly-post193 (hm2_cc2)**: NOP 巡检轮. cc2 30min 0 req (session 轮前无流量产生, 无数据可判 SR).
  ...
  0 改动, 0 重启.
  hermes 打 dsv4p_nv 6×429 all_tiers_exhausted (NVCF 侧 dsv4p 配额限流, 周期 02:50-03:15).
  openclaw 打 dsv4p_nv 1×200 (3s).
  **与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv).
  glm5_2_nv 连续 post100-post193 (94 轮) 无 dsv4p 故障扩散.
...
本轮 30min/6h cc2 无请求产生. 无数据可判 cc2 SR. 链路健康无故障.

### 2. 其他 caller (hermes, openclaw, 非 cc2 链路)
| caller | model | status | count | avg_dur |
|--------|-------|--------|-------|---------|
| hermes | dsv4p_nv | 429 | 6 | 1609 |
| openclaw | dsv4p_nv | 200 | 1 | 3019 |

hermes→dsv4p_nv 6×429 all_tiers_exhausted (NVCF 侧 dsv4p 配额限流, 周期 02:50-03:15).
...
|------------|-----|-------|---------|
| all_tiers_exhausted | all_tiers_failed_in_mapped_tier | 6 | 1609 |
...
- dsv4p_nv 5key 429 限流 (NVCF 侧配额), KeyManager 退避机制工作正常.
- 与 cc2 无关: cc2 不打 dsv4p_nv.
...
| cc2 (cc4101-primary) 30min/6h SR | 0 rows (无流量, 链路健康无故障) ✓ |
| 30min tier error (cc2) | 0 ✓ |
| 30min 全 caller | hermes 6req dsv4p_nv (6×429 限流), openclaw 1req (1×200), cc2 0 req ✓ |
| 配置 | NVU_DISABLE_MS_FALLBACK=0 (fallback 已恢复), FALLBACK_UPSTREAM=ms_gw:40007 ✓ |

## 参数快照 (2026-08-02 11:19 CST, 无变化, 同 post192)
