...
## 当前轮基线 (2026-08-02 16:33 CST, R292 NOP 巡检轮)
- 本仓 master: 本轮 R292. (主仓 hermes_improve_self main `a6baad3` R292 round.)
- **架构 (主仓 b4527f9, 非本轮)**: cc4101 `PRIMARY_UPSTREAM_MODEL` 已从
  `glm5_2_nv` 切到 `dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R292 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=5.9% (1/17), 失败 16 全 `all_tiers_exhausted`.
  glm5_2_nv 30min SR=100.0% (11/11) 健康输出.
...
## 本轮关键数据 (30min 实时 DB + 链路分析注入 ~16:33 CST)
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R291 分析)
...
### 为何 hermes/other 边界 429/502 常态, cc2 buffer_exhausted 罕见
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R292 验证), 对 function 级 429 是已知盲区.
