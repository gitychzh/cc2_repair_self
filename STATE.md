...
> 当前轮: **R1061 (NOP 巡检轮/不改码 — 主链 99.1% (105/106), 唯一 scoped bad = R1060 已溯源瞬时 ec39dd9b 仍留 lookback, 非新缺陷; fresh 5min 100% 干净; main fid 281478d0 零错误; fallback 0 次)**
> cc4101-primary (主 nv_gw:40006) 实测 30min = **105/106 = 99.1% SR, 1 scoped bad**;
> 唯一 scoped 502 = req ec39dd9b (buffer_exhausted, 58949ms, created_at 11:02:02 UTC = 19:02 CST) =
> **R1060 已溯源的瞬时**: 3 连 SSLEOFError (3 key/IP) + ms_gw 同步 down, **已自愈** (当前 lookback 内遗留, 非新错误);
> fresh 5min = **19/19 = 100%**, 链路当前完全健康;
> nv_tier_attempts main fid **281478d0** = k0-k4 全 pexec_success (22/19/22/20/21), **0 错误**;
> 3×NVCFPexecRemoteDisconnected (k0/k1/k2) 全命中坏 fid **52e1ddb6** → hermes 越界宿主, 非主链;
> 30min cc_requests fallback = **0 次 / 0.0%**, SR **100%** (106/106);
> buffer 日志本轮窗口全 attempt1 success, 无 fail;
> 容器 (/health 复核): nv_gw Up 16h, cc4101 Up 15h, /health 40006/4101 全 200
> 上轮: R1060 (NOP, 主链 101/102=99.0%)

## 本轮 (R1061) 改动 + 依据 + 验证

### 改动: 无 (NOP。唯一 scoped bad 为上轮 R1060 已溯源瞬时 ec39dd9b 仍落 lookback, 非新缺陷; 主链 fid 281478d0 全 pexec_success; 无参数可调)

### 依据 (注入轮前链路分析 19:10 CST + DB/容器日志复核 2026-08-07)

- 30min cc4101-primary (主 nv_gw:40006) = **105/106 = 99.1% SR, 1 scoped bad**。
...
- 唯一 scoped 502 根因 (链路铁证): req **ec39dd9b**, buffer_exhausted, 58949ms,
  **DB created_at=11:02:02 UTC = 19:02 CST** (UTC-8)。即 **R1060 轮首即详细溯源的瞬时**:
  attempt1/2/3 (19:01:37/19:01:47/19:02:02) 于不同 key 不同出口 IP 连续 3 连 SSLEOFError
  (UNEXPECTED_EOF_WHILE_READING, dist :7899/:7901/:7894) → 3 consecutive all_keys_exhausted →
  AKE fail-fast (≥3 CLOSED) 正确跳过 WaitQueue → ms_gw 同步 down → 才 502。19:02:27 起 200 自愈。
  **它本轮只是仍落在 30min lookback 内, 非新缺陷**; offset 后即消失。
- 主链 main fid **281478d0** nv_tier_attempts (30min) = k0 22 / k1 19 / k2 22 / k3 20 / k4 21, **0 错误**。
  另 3 条 NVCFPexecRemoteDisconnected (k0/k1/k2) 全命中**坏 fid 52e1ddb6** → hermes 越界宿主 (request_id JOIN), 非主链。
- 30min cc_requests fallback = **0 次 / 0.0%**, SR **100%** (106/106 全 200)。
- buffer 日志本轮窗口 (19:09~19:11) 全 attempt=1/5 success (success_text / success_tool_call), 无任何 fail。
...
| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **105/106 = 99.1% SR, 1 scoped bad** | ⚠️(R1060 遗留瞬时) |
| fresh 5min | **19/19 = 100%, 0 错误** | ✅ 当前健康 |
| 主链 scoped 错误 | 1 (ec39dd9b buffer_exhausted 58949ms, 19:02 CST) — R1060 历史瞬时已自愈 | ✅ 非新缺陷 |
| 主链 main fid 281478d0 nv_tier_attempts | k0-k4 全 pexec_success (22/19/22/20/21), 0 错误 | ✅ |
| nv_tier_attempts 3×RemoteDisconnected | 全命中坏 fid 52e1ddb6 (hermes 宿主) | ✅(非主链) |
| 30min cc_requests | fallback 0 次 (0.0%), 总 106 全 200 | ✅ |
...
## 参数快照 (2026-08-07, 与上轮 R1060 一致, 未动)
