# R640 — NOP 巡检轮 (2026-08-03 14:40 CST)

## 基线 (R640 实测, 06:00-06:40 UTC 窗口)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本, 同 R626-R639)
- dsv4p_nv 30min (hermes caller): 17 req, 12×200 + 3×429 + 2×502 (SR=70.6%)
  - vs R639 61.5% / R638 58.3% / R637 37.5% / R636 37.5% / R635 37.5% / R634 37.5% / R633 64.7% / R632 73.7% / R631 76.2% / R630 80.8% / R629 86.7% / R628 90.3%
    → **本轮持续回升 +9.1pp vs R639 (61.5%), +33.1pp vs R634-R637 连续 4 轮 37.5%, 仍在 R617-R639 正常波动区间 (37-91%) 上沿, 配额恢复中, 趋势向好**
  ...
  - 200 avg_dur=12639ms (max 34819, min 3654, avg_ttfb=12162), 略高于 R639 10841ms (含 1×34819ms 慢响应拉高均值)
- 错误分类 1 类 (非新错误):
  - `all_tiers_exhausted` ×5 (avg_dur 9293ms)
    - 含 429×3 avg 1925ms 快速 ABORT + 502×2 avg 20346ms 慢响应 (NVCF 慢, peer-fb skip)
    → R624→...→R633→R634→R635→R636→R637→R638→R639→R640: 3→...→3→3→3→4→5→5→5→5→6→5→5→5→5→5 (持平正常区间 3-6, 本轮持平 R634-R639)
...
## 6h dsv4p_nv SR 趋势 (小时��, 实测 R639 记录)
...
- 本轮 06:00 回升 53.8%→70.6% (vs R639 06:00 61.5%) → 配额持续恢复, SR 向好

## 持续观察点 (沿用 R639, 不改码) — dsv4p_nv 配额型 429 全挂
- 6h SR 波动 29-87%, 04:00 回升 86.8%, 06:00 回升 53.8-70.6% → 配额型故障模式确认, 非 nv_gw 侧可改
- 01:00 单小时 SR=29.4% 触及 R621 升级阈值 (SR<55%), 但:
  - cc2 视角 0 样本 (铁律1 不满足)
  - 非持续 (04:00 回升至 86.8%, 06:00 回升至 70.6%)
...
## 依据
- cc2 0 流量 → 无评估样本, 铁律1 cc2 视角不满足 (同 R626-R639)
- dsv4p_nv SR=70.6% 持续回升 +9.1pp vs R639 (61.5%), +33.1pp vs R634-R637 连续 4 轮 37.5%, 仍在 R617-R639 正常波动区间 (37-91%) 上沿, 趋势向好
- 6h SR 趋势 29-87% 波动, 04:00 回升 86.8%, 06:00 回升 53.8-70.6% → 配额型故障模式确认
- all_tiers_exhausted ×5 (R624-R640: 3→...→6→5→5→5→5→5→5), 持平 R634-R639 仍在正常波动区间 3-6, 无退化
- KeyManager 指数退避正确, 429 ABORT avg 1925ms (快速, 持平 R639 1725ms 量级)
...
- 配置无漂移 (env 全项匹配 R639 快照)
- stream_total_deadline 6h=0 (deadline 链铁证)
- KeyManager 指数退避正确工作 (沿用 R639 日志铁证: 429 count decay >300s → reset → 180s)

## 下一步
- dsv4p_nv SR 持续回升 70.6% (vs R634-R637 37.5%), 仍在正常波动区间上沿, 配额恢复中, 趋势向好, 无恶化
- **升级标注解除** (R621 设定 SR<55% 或 exhausted>=8 → 切 PRIMARY 回 glm5_2_nv):
  - 01:00 单小时触及 SR<55% (29.4%), 但非持续 (04:00 回升 86.8%, 06:00 回升 70.6%), cc2 视角 0 样本 → 不触发
