# R523 — NOP 巡检轮 (2026-08-03 06:33 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (延续 R522 间歇空闲窗口).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=37.5% (3/8: 3×200 + 5×429), 与 R510-R522 同窗口周期一致.
...
- 配置实测确认与 R475-R522 完全一致, 无漂移.

## 链路数据 (06:32 CST 实测, 注入)
### 30min 窗口 (全 caller, dsv4p_nv)
- 8 req: 3×200 (avg_dur=9568ms, nv_key_idx=2, egress=203.10.96.139), 5×429 (空 idx, 空 IP, avg_dur=1643ms) → SR=37.5%
...
- 按分钟趋势: 22:05-22:11 429+200 交替, 22:15-22:30 4×429, 22:10 后 200 回升

### cc4101-primary 专属 (cc2 的请求)
- 30min 0 req (session 间歇空闲, 实测 0 rows, 已二次确认)

### KeyManager 行为 (与 R522 一致)
- 三波 5min 周期全局 429 (06:15/06:20/06:25), k3 累计最快 (count=5→600s 顶), 其余 180s 基础冷却
...
- 错误类型 all_tiers_exhausted ×5, 模式与 R268-R522 一致, 无新错误
- dsv4p_nv 30min SR=37.5% (与 R510-R522 同窗口周期性行为, 低谷窗口)
- fallback 兜底正常 (cc4101 层走 ms_gw glm5_2_ms, 链路有保障)
- 0 restart → 无需 py_compile / curl 复测
- 配置实测与 R475-R522 完全一致, 无配置漂移
...
- 留意 502 是否再现 (R476/R480-R522 记 6h 低频 zombie 502, 再现 >=3/h 才介入)
