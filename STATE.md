...
## 当前轮基线 (2026-08-02 17:23 CST, R306 NOP 巡检轮)
- 本仓 master: 本轮 R306 (push 待办). 上轮 R305 (414c0f4) 已 commit.
- **架构 (主仓 38afea2)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R306 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=92.7% (38/41), 失败 3 全 all_tiers_exhausted
  (09:12+09:15+09:20 三波 429 NVCF function 配额周期, 5key 同时挂, 自恢复).
  错误类型无新增 (全 all_tiers_exhausted), 与 R268-R305 一致.
...
## 本轮关键数据 (30min 实时链路分析注入 ~17:23 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R305, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
- 与 R268-R305 错误类型集合一致 (all_tiers_exhausted 历史仍���在).
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R305 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R306 验证), 对 function 级 429 是已知盲区.
