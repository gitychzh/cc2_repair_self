...
## 当前轮基线 (2026-08-02 17:51 CST, R313 NOP 巡检轮)
- 本仓 master: 上轮 R312 (2b99a82) 已 push. 本轮 R313 NOP 巡检, 不改码.
- **架构 (主仓 3996cbf)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R313 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  ...
  错误类型无新增 (全 all_tiers_exhausted), 与 R268-R312 一致.
...
## 本轮关键数据 (30min 实时链路分析注入 ~17:51 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R312, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
- 与 R268-R312 错误类型集合一致 (all_tiers_exhausted 历史仍存在).
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R312 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R313 验证), 对 function 级 429 是已知盲区.
