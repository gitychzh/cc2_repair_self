...
> 当前轮: **R1008 (NOP 巡检轮/不改码 — cc2 主链路连续第 116 轮 100% 干净; 主链专属错误 0 rows; fallback 0)**
...
> 上轮: R1007 (NOP, 主链 105/105=100%)

## 本轮 (R1008) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 116 轮 100% 干净, 主专属错误 0 rows; 本轮 window 内 nv_requests 的 5 条 bad 全属 hermes)

### 依据 (live 复核 2026-08-07 15:49 CST + 注入轮前链路分析)

- 30min cc4101-primary (主 nv_gw:40006) = **107/107 全 200 = 100% SR, 0 bad** (live re-pull)。
...
- fallback (cc_requests 30min) = **0 次** (107 total, fb=0; primary 107/107 全 200)。
- buffer 日志: 全 attempt=1 一次成功 (2-13s), verdict 全 success_text/success_tool_call, 无 BUFFER-/WAIT- 停滞。
...
| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **107/107 = 100% SR, 0 bad** (live re-pull) | ✅ |
| 主链专属错误 (caller=cc4101-primary) | **0 rows** | ✅ |
| nv_requests 总 bad (非 200) | 5 条, 全属 hermes (dsv4f0731_nv), 主链 0 | ✅(主链) |
| 30min cc_requests primary | 107, fallback 0 次 | ✅ |
| buffer/wait | 全 attempt=1 成功, 无停滞 | ✅ |
| 容器 | nv_gw Up 17h, cc4101 Up 12h, dsv4p health 200 | ✅ |

### 验证
- 30min nv_requests cc4101-primary = 107/107 (0 bad)。
- 主链专属错误分组 = 空 (0 rows)。
- 5 条非 200 经 scoped 分组判定属 hermes (caller=hermes), 非 cc2 主链。
- cc_requests primary 107, fallback 0 次。
- buffer 日志: 全 attempt=1 成功, 无 BUFFER-/WAIT- 停滞。
- health: 4101/40006/40066 全 200; 容器 nv_gw/cc4101 皆 Up。

### 关键判断
cc2 主链路连续第 **116** 轮 (R893-R1008) 100% SR 干净, 主链专属错误 0 rows。
本轮 5 条 bad (dsv4f0731_nv 502) 归属全属 hermes 越界宿主 (fid 52e1ddb6 泄漏) — 与主链 host
分离保持, 主链 107/107 全 200。fallback 0 次, 无新 cc2 主链错误类, 无持久 key 疲劳。
