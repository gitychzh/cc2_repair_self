...
> 当前轮: **R969 (NOP 巡检轮/不改码 — cc2 主链路连续第 77 轮 100% 干净; 坏请求 3: 502 zombie_empty_completion ×2 + 502 all_tiers_exhausted ×1 全属 hermes 线 + fid 52e1ddb6, 双重归属非 cc2 范围; fallback 0 次)**
> cc4101-primary (主 nv_gw:40006) 实时 30min = **126/126 = 100% SR, 0 bad** (live 复核);
...
### 改动: 无 (NOP。cc2 主链路连续 77 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes+已知坏 fid 非 cc2)

### 依据 (注入轮前链路分析 ≈2026-08-07 12:51 CST + live 复核 bad 归属/fallback)

- 30min cc4101-primary (主 nv_gw:40006) = **126/126 全 200 = 100% SR, 0 bad** (live re-pull)。
- 总 nv_requests 30min = 152 req, dsv4f0731_nv SR=98.0% (149/152)。
...
- fallback (cc_requests 30min) = **0 次** (全窗口 1918 req, 1903 OK, SR 99.2%;
  主链 151 req 全未 fallback)。
- nv_tier_attempts 30min (per-key): pexec_success 26/25/24/24/26 (k0-k4) +
  NVCFPexecRemoteDisconnected k0×4,k1×2,k2×4,k3×4,k4×4 + 529_nv_overloaded k1×1 +
...
| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **126/126 = 100% SR, 0 bad** (live re-pull) | ✅ |
...
| fallback (cc_requests) | 0 次 (1918 req, 1903 OK; 主链 151 全未 fb) | ✅ |
| tier 瞬态错误 | RemoteDisconnected 18 / 529 1 / empty_200 1, 全被吸收 | ✅ |
| 全局 nv_requests SR | 149/152 = 98.0% (dsv4f0731_nv) | ✅ |

### 验证
- 30min nv_requests cc4101-primary = 126/126 (0 bad)。
- bad 分组 (caller+fid 归属): 502 zombie_empty_completion ×2 + 502 all_tiers_exhausted ×1
  均 caller=hermes + fid=52e1ddb6, cc2 主链 0 bad。
- cc_requests fallback = 0 次 (全窗口 1918 req 全未 fallback_triggered)。
...
### 关键判断
cc2 主链路连续第 **77** 轮 (R893-R969) 100% SR 干净, 且主链专属错误 0 rows。
