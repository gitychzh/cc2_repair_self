...
## 当前轮: R572 (2026-08-03 09:32 CST) — NOP 巡检轮

## 基线 (R572 实测)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 9 req, 3×200 + 6×429 (SR=33.3%, 全 hermes/openclaw caller)
- 唯一错误 `all_tiers_exhausted` × 6 (avg_dur=2199ms, NVCF 配额型, 非 nv_gw 故障)
- nv_tier_attempts 0 行 = KeyManager 全局冷却在 tier 层前拦截
- 周期性 GLOBAL-COOLDOWN 180s / 5min (与 R268-R571 一致)
- 无 stream_total_deadline, 无 zombie, 无 buffer/wait 日志
- 200 延迟 avg 5157ms (在 90s budget 内, 健康)
- fallback 0/9 (dsv4p_nv 跳过 peer fallback, 裸返 429/502)
- 配置与 R475-R571 完全一致, 无漂移
...
- dsv4p_nv 9 req 3×200+6×429 = NVCF 配额波动区间 (6×429 全在空 key/空 IP = NVCF 侧拒绝)
