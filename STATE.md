...
> 当前轮: **R965 (NOP 巡检轮/不改码 — cc2 主链路连续第 73 轮 100% 干净; 坏请求 3: 502 all_tiers_exhausted ×2 + 502 zombie_empty_completion ×1 全属 hermes 线 + fid 52e1ddb6, 双重归属非 cc2 范围; fallback 0 次)**
...
### 改动: 无 (NOP。cc2 主链路连续 73 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes+已知坏 fid 非 cc2)

### 依据 (live DB 30min 实拉 ≈2026-08-07 12:36 CST)

- 30min cc4101-primary (主 nv_gw:40006) = **126/126 全 200, 0 bad (100% SR)** (live re-pull)。
- 总 nv_requests 30min = 145 req, dsv4f0731_nv SR=97.9% (142/145)。
...
- nv_tier_attempts 30min (per-key): pexec_success 26/23/27/24/26 (k0-k4) +
  NVCFPexecRemoteDisconnected k0×1,k1×1,k2×2,k3×5,k4×5 + 529_nv_overloaded k1×1 +
  NVCFPexecTimeout k0×1,k4×1 + empty_200 k0×1,k2×2
...
| tier 瞬态错误 | RemoteDisconnected 14 / 529 1 / Timeout 2 / empty_200 3, 全被吸收 | ✅ |
| 全局 nv_requests SR | 142/145 = 97.9% (dsv4f0731_nv) | ✅ |
...
### 关键判断
cc2 主链路连续第 **73** 轮 (R893-R965) 100% SR 干净, 且主链专属错误 0 rows。
...
③多 key round-robin (dsv4f0731_nv 为首, 当前 cc4101 PRIMARY_UPSTREAM_MODEL=dsv4f0731_nv)
+ func_health 健康选择已达稳态, hermes 的 all_tiers_exhausted/zombie 由 hermes 自身 key pool
疲劳 + 坏 fid 所致, 不泄漏进 cc2。
