# R618 — NOP 巡检轮 (2026-08-03 13:13 CST)

## 基线 (R618 实测, 04:36-05:06 CST 窗口, 与 R617 同窗口延续)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 20 req, 15×200 + 3×502 + 2×429 (SR=75.0%, hermes caller)
  - vs R617 75.0% / R616 77.3% / R615 82.6% / R614 88.5% / R613 91.3% / R612 85.0%
...
- 错误分类 2 类 (均非新错误):
  - `all_tiers_exhausted` ×4 (avg_dur 10276ms, all_tiers_failed_in_mapped_tier, ABORT-NO-FALLBACK)
...
## 根因分析 (与 R617 一致)
日志铁证: 13:01:14.5 / 13:06:05.8 / 13:11:05.2 三次 `NV-GLOBAL-COOLDOWN tier=dsv4p_nv all keys 429,
...
- dsv4p_nv 75.0% SR 在 R612-R617 历史波动区间 (75-91%), 非新低, 属 NVCF 账户配额波动
- all_tiers_exhausted 全部 nv_key_idx 空 + tier_attempts 0 行 → KeyManager 层 ABORT, 非 buffer/tier 路径
- 日志铁证 3 次 GLOBAL-COOLDOWN + retry-after 缺失 = NVCF 账户级配额耗尽, nv_gw 无法靠改码解决
- KeyManager 指数退避正确, ABORT 路径快速 (2-5s), 无退化
- NVStream_IncompleteRead ×1: 30min 单次瞬态, 与 R612-R617 同模式
- 容器健康, 配置无漂移
- 无介入必要
...
- all_tiers_exhausted 次数 R613×1 → R614×2 → R615×3 → R616×4 → R617×4 → R618×4 持平:
