...
per-key (dsv4p): key2 → 26×200 (avg_dur 10176) + 1×502 (33960); 空 key → 2×429 (4337).
per-egress: 203.10.96.139 → 27×96, 空 IP → 2×0 (429).
finish_reason (200): tool_calls×23, stop×3 (无 zombie).
分钟趋势: 10:00-10:20 恢复 26×200 (配额周期自恢复), 10:21 一波 429×2 + 502×1.
延迟 (200): avg_dur 10176, max 22797, min 3038, avg_ttfb 9880.
fallback 0/29.

### 3. 错误分类 (DB 实测)
- 3 错误: 2× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 4337)
...
- nv_gw: NVU_DISABLE_MS_FALLBACK=0, UPSTREAM_TIMEOUT=90, TIER_COOLDOWN_S=180,
  ...
  NVU_FORCE_STREAM_UPGRADE_TIMEOUT=150, TIER_TIMEOUT_BUDGET_S=180,
