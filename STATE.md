# R642 — NOP 巡检轮 (2026-08-03 14:47 CST)

## 基线 (R642 实测, 06:20-06:45 UTC 窗口)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本, 同 R626-R641)
- dsv4p_nv 30min (hermes caller): 19 req, 15×200 + 2×429 + 2×502 (SR=78.9%)
  - vs R641 70.6% / R640 70.6% / R639 61.5% / R638 58.3% / R634-R637 37.5%
    → **本轮 +8.3pp vs R641 (70.6%), +17.4pp vs R639 (61.5%), +41.4pp vs R634-R637 连续 4 轮 37.5%,
       仍在 R617-R641 正常波动区间 (37-91%) 上沿, 配额持续恢复, 趋势向好, 无恶化**
  - per-key: k2=15×200 全 first-attempt 命中, 空 key=2×429+2×502
  - per-egress-IP: 203.10.96.139 主力 (15×200 全命中), 空 IP=4×失败
  - finish_reason: tool_calls×13 + stop×2 (健康, 无 zombie)
  - fallback_occurred=f ×19 (hermes caller 直连 nv_gw)
  - 200 avg_dur=12191ms (max 34819, min 3654, avg_ttfb=11725), 持平 R641 12639ms 量级
- 错误分类 1 类 (非新错误):
  - `all_tiers_exhausted` ×4 (avg_dur 11321ms)
    - 含 429×2 快速 ABORT + 502×2 avg 20346ms 慢响应 (NVCF 慢, peer-fb skip)
    → R624→...→R634→R635→R636→R637→R638→R639→R640→R641→R642: 3→...→3→4→5→5→5→5→5→5→4
       (持平 R634-R641 正常波动区间 3-6, 本轮 -1 vs R641, 趋势向好)
...
## 6h dsv4p_nv SR 趋势 (小时桶, 沿用 R641 记录 + 本轮)
- 23:00 52.2% → 00:00 42.1% → 01:00 29.4% → 02:00 59.1% → 03:00 56.5% → 04:00 86.8% → 05:00 76.3% → 06:00 53.8%(部分)→06:45 78.9%(本轮)
- 波动 29-87%, 模式不变: NVCF 账户级配额耗尽 (429 无 retry-after, 全 5key 429 → TIER_COOLDOWN 180s)
- 04:00 回升 86.8% (高时段配额刷新) → 配额型故障模式确认, 非 nv_gw 侧可改
- 本轮 06:00 78.9% (vs R641 06:40 70.6%) → 配额持续恢复, SR 稳定向好

## 持续观察点 (沿用 R641, 不改码) — dsv4p_nv 配额型 429 全挂
- 6h SR 波动 29-87%, 04:00 回升 86.8%, 06:00 本轮 78.9% → 配额型故障模式确认, 非 nv_gw 侧可改
- 01:00 单小时 SR=29.4% 触及 R621 升级阈值 (SR<55%), 但:
  - cc2 视角 0 样本 (铁律1 不满足)
  - 非持续 (04:00 回升至 86.8%, 06:00 本轮 78.9%)
  - → 不触发回切 PRIMARY glm5_2_nv
- KeyManager 指数退避正确 (429 count decay >300s → reset → 180s, 全 key 429 → TIER_COOLDOWN 180s)
...
- 502×2 (avg 20346ms 慢) = NVCF 慢响应 (other 非 429 非 timeout), peer-fb skip
...
## 依据
- cc2 0 流量 → 无评估样本, 铁律1 cc2 视角不满足 (同 R626-R641)
- dsv4p_nv SR=78.9% +8.3pp vs R641 (70.6%), +17.4pp vs R639 (61.5%), +41.4pp vs R634-R637 连续 4 轮 37.5%,
  仍在 R617-R641 正常波动区间 (37-91%) 上沿, 趋势向好, 无恶化
- 6h SR 趋势 29-87% 波动, 04:00 回升 86.8%, 06:00 本轮 78.9% → 配额型故障模式确认
- all_tiers_exhausted ×4 (R624-R642: 3→...→6→5→5→5→5→5→5→5→4), -1 vs R641, 仍在正常波动区间 3-6, 无退化
- KeyManager 指数退避正确, 429 ABORT avg (持平 R641 1925ms 量级)
...
- 配置无漂移 (env 全项匹配 R641 快照)
- stream_total_deadline 6h=0 (deadline 链铁证)
- KeyManager 指数退避正确工作 (沿用 R641 日志铁证: 429 count decay >300s → reset → 180s)

## 下一步
- dsv4p_nv SR 78.9% (+8.3pp vs R641 70.6%), 仍在正常波动区间上沿, 配额持续恢复, 趋势向好, 无恶化
- **升级标注解除** (R621 设定 SR<55% 或 exhausted>=8 → 切 PRIMARY 回 glm5_2_nv):
  - 01:00 单小时触及 SR<55% (29.4%), 但非持续 (04:00 回升 86.8%, 06:00 本轮 78.9%), cc2 视角 0 样本 → 不触发
