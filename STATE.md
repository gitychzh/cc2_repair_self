...
## 当前轮基线 (2026-08-02 17:29 CST, R307 NOP 巡检轮)
- 本仓 master: 本轮 R307 (push 已完成 8ba0050). 上轮 R306 (8fa5a82) 已 push.
- **架构 (主仓 8ba0050)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R307 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=89.2% (33/37), 失败 4 全 all_tiers_exhausted
  (09:12+09:15+09:20+09:25 四波 429 NVCF function 配额周期, 5key 同时挂, 自恢复).
  错误类型无新增 (全 all_tiers_exhausted), 与 R268-R306 一致.
...
## 本轮关键数据 (30min 实时链路分析注入 ~17:28 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R306, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).

### 2. dsv4p_nv 30min 全 caller SR=89.2% (33/37)
...
per-key (dsv4p): key2 → 32×200 (avg_dur 11022), key3 → 1×200 (5781), 空 key → 4×429 (1562).
per-egress: 203.10.96.139 → 32×100, 134.195.101.194 → 1×100, 空 IP → 4×0 (429).
finish_reason: tool_calls×29, stop×4 (无 zombie).
分钟趋势: 09:00-09:11 持续多波 200, 09:12/09:15/09:20/09:25 四波 429 (配额周期命中).
延迟: avg_dur 10863, max 24567, min 3861, avg_ttfb 10614.
fallback 0/37.

### 3. 错误分类 (DB 实测)
- 4 错误: 全 all_tiers_exhausted (avg_dur 1562, sub=all_tiers_failed_in_mapped_tier).
- 09:12/09:15/09:20/09:25 四波 429 → 5key 同 function 同时挂 → all_tiers_exhausted.
- 与 R268-R306 错误类型集合一致 (all_tiers_exhausted 历史仍存在).
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R306 分析)
...
- 本轮 09:12+09:15+09:20+09:25 四波 429 证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R307 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=89.2% (窗口末尾命中四波 429, NVCF function 配额周期).
- 错误类型无新增 (全 all_tiers_exhausted), 与 R268-R306 一致.
- 四十轮一致 R268-R307.
