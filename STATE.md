...
## 当前轮基线 (2026-08-02 18:08 CST, R318 NOP 巡检轮)
- 本仓 master: 上轮 R317 (ebd4edc) 已 push. 主仓 hm2 侧 R317 (1bef2a7) 已 push.
- **架构 (主仓 1bef2a7)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R318 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=88.0% (22/25), 失败 3 = 2× all_tiers_exhausted
  (09:45+09:50 两波 429 NVCF function 配额周期, 5key 同时挂, 自恢复)
  + 1× NVStream_IncompleteRead (502, mid-stream 软挂单��, 历史偶发).
  错误类型无新增, 与 R268-R317 一致.
...
## 本轮关键数据 (30min 实时链路分析注入 ~18:08 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R317, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
- 与 R268-R317 错误类型集合一致 (all_tiers_exhausted + NVStream_IncompleteRead 历史仍存在).
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R317 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R318 验证), 对 function 级 429 是已知盲区.
