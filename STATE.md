...
## 当前轮: R592 (2026-08-03 11:17 CST) — NOP 巡检轮

## 基线 (R592 实测, 11:17 CST 链路分析注入 + 复测)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 12 req, 8×200 + 4×429 (SR=66.7%, 全 hermes caller)
  - vs R591: SR 44.4% → 66.7% (波动区间内上升, R545-R591 同一 NVCF 配额模式)
  - per-key: key2 7×200 (命中可用 key 100% 200, avg_dur=10995ms, IP 203.10.96.139)
  - per-key: key3 1×200 (命中可用 key 100% 200, avg_dur=3274ms, IP 134.195.101.194)
  - 空 key 4×429 (全挂时 NVCF 侧拒绝, avg_dur=1677ms 快速返回, 配额波动区间)
- 唯一错误 `all_tiers_exhausted` ×4 (avg_dur=1677ms, NVCF 配额型, 非 nv_gw 故障)
- nv_tier_attempts 0 行 = KeyManager 全局冷却在 tier 层前拦截
- 429 全在空 key/空 IP = NVCF 侧拒绝 (配额波动区间)
- key2 = 7×200 + key3 = 1×200 (命中可用 key 时 100% 200)
- 无 buffer/wait 日志 (30min 无 buffer 触发), 无 stream_total_deadline, 无 zombie
- finish_reason: tool_calls×5 + stop×3 (健康, 无 zombie stop)
- 全挂时 ABORT-NO-FALLBACK 是预期 (dsv4p_nv 跳 peer/ms fb, cc4101 层 ms_gw 兜底)
- fallback 发生率 f×12 (cc4101 层 ms_gw 兜底, 预期)
- 配置与 R472-R591 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R591 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R591 一致模式)
...
- dsv4p_nv 12 req: 8×200+4×429 = NVCF 配额波动区间 (命中 key2/key3 100% 200, 全挂时空 key 429 avg_dur=1677ms 快速返回)
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=66.7% vs R591 44.4% vs R590 37.5% vs R589 37.5% vs R588 37.5% vs R587 37.5% vs R586 37.5% vs R585 37.5% vs R584 71.4% vs R583 71.4% vs R582 58.3% → 波动区间内, 与 R545-R591 同一 NVCF 配额波动模式
