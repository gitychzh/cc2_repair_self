# STATE — cc2 自优化 nv_gw 邀路 (R-nvonly 方向)

## 当前轮基线 (2026-08-02 17:05 CST, R301 NOP 巡检轮)
- 本仓 master: 本轮 R301 (push 待办). 主仓 hermes_improve_self main `8f10f47` R300 round.
- **架构 (主仓 b4527f9, 非本轮)**: cc4101 `PRIMARY_UPSTREAM_MODEL` 已从
  `glm5_2_nv` 切到 `dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R301 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=95.5% (21/22), 失败 1 全 `all_tiers_exhausted`
  (08:40 一波 429 NVCF function 配额周期 → 08:45-09:01 多波 200 恢复, 自恢复).
  错误类型无新增, 全 all_tiers_exhausted, 与 R268-R300 一致.
...
## 本轮关键数据 (30min 实时链路分析注入 ~17:05 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R300, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R300 分析)
...
- 08:40 429 → 08:45-09:01 多波 200 恢复, 证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R301 验证), 对 function 级 429 是已知盲区.
