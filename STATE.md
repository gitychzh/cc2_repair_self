...
## 当前轮基线 (2026-08-02 15:49 CST, R281 NOP 巡检轮)
- 本仓 master: 本轮 R281. (主仓 hermes_improve_self main 收 round 文件, commit 待 push.)
- **架构 (主仓 b4527f9, 非本轮)**: cc4101 `PRIMARY_UPSTREAM_MODEL` 已从
  `glm5_2_nv` 切到 `dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R281 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  hermes caller dsv4p_nv 30min 6×200 + 4×429 = SR 60% (10req), 4 失败全 hermes, 全整 5min
  边界点 (07:20/07:25/07:35/07:40 UTC), 全 `all_tiers_exhausted`, <3s 快速失败.
  07:30 三连 200 + 07:45/07:46 三连 200 印证 NVCF 5min 配额周期恢复窗口.
  边界点与 R279/R280 完全重合 (07:15-07:40) 非恶化而是配额周期稳态.
...
## 本轮关键数据 (30min 实时 DB 复查 ~15:49 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R280, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.

### 2. dsv4p_nv 30min SR=60% (6/10), 失败全 hermes 边界点
...
### 4. 健康检查
- `curl /health` → ok, nv_num_keys=5, nv_model_tiers=[kimi_nv,dsv4p_nv,glm5_2_nv].
- buffer/wait/keymanager 日志 30min 空 (无 buffer 流量, cc2 0 req).

## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R280 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R280 验证), 对 function 级 429 是已知盲区.
...
- 边界点与 R279/R280 完全重合 (07:15-07:40), 配额周期稳态, 非恶化.
