# R624 — NOP 巡检轮 (2026-08-03 13:36 CST)

## 基线 (R624 实测, 05:05-05:36 UTC 窗口)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 17 req, 14×200 + 3×429 (SR=82.4%, hermes caller)
  - vs R623 76.5% / R622 64.3% / R621 57.1% / R620 62.5% / R619 70.6% / R618 75.0%
    → **连续 3 轮反弹 (57.1→64.3→76.5→82.4), 已回到 R618 波动区间上沿**
  - per-key: k2 14×200; 空 key 3×429
  - per-egress: 203.10.96.139 14 req (100%) + 空 3 req (0%)
  - finish_reason: tool_calls×12 + stop×2 (健康, 无 zombie)
...
- 错误分类 1 类 (非新错误):
  - `all_tiers_exhausted` ×3 (avg_dur 1540ms, all_tiers_failed_in_mapped_tier, ABORT-NO-FALLBACK)
    → **all_tiers_exhausted R621×6 → R622×5 → R623×4 → R624×3 持续下降, 与 SR 反弹同步**
- 30min nv_tier_attempts: 0 行 (hermes caller 不走 buffer, 全挂 KeyManager 层 ABORT)

## 根因分析 (与 R623 一致)
按分钟趋势: 05:11/05:16/05:21 各 1×429 → 05:26-05:36 一波 14×200 (k2 first attempt)。
NVCF 配额周期性间歇 (5min 周期), 非永久挂; 全挂 180s 后 k2 自动恢复连续命中。
...
- dsv4p_nv SR=82.4% 连续 3 轮反弹, all_tiers_exhausted 同步下降 (6→3)
  → 证实 R621 "周期性配额耗尽 + 采样窗口偏移" 判断正确, 趋势已反转
...
- KeyManager 指数退避正确, ABORT avg 1540ms vs R623 1731ms↓ 持续下降无退化
- 容器健康, 配置无漂移
- 无介入必要
...
- 配置无漂移 (env 全项匹配 R623 快照)

## 下一步
- dsv4p_nv SR 从 R621 低点 57.1% 连续 3 轮反弹至 82.4%, 趋势已反转
