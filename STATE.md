...
## 当前轮基线 (2026-08-02 19:46 CST, R343 NOP 巡检轮)
- 本仓 master: 上轮 R342 (90dcaf2) 已 push. 本轮 R343.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R343 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller (hermes) SR=50.0% (7/14), 失败 7 = 7× all_tiers_exhausted
  (NVCF function 级 429 配额周期多波). 本轮无 NVStream_IncompleteRead; 502 1× 偶发 transport-level.
  错误类型无新增, 与 R268-R342 一致. cc2 无流量不受影响, 0 fallback 0 deadline.
  0 改动 0 restart. **六十五轮一致 R268-R343**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~19:46 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R342, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).
- 30min caller×model×status 总览: hermes|dsv4p_nv|200×7, 429×6, 502×1.

### 2. dsv4p_nv 30min 全 caller SR=50.0% (7/14)
...
per-key (dsv4p): key2 → 7×200 (avg_dur 11865); 空 key → 6×429 (1805) + 1×502 (34101, transport-level hang).
per-egress: 203.10.96.139 → 7×100%; 空 IP → 7×0%.
finish_reason (200): tool_calls×7 (无 zombie).
分钟趋势: 11:16/11:20/11:25-11:27 多波 200+429 混合; 11:27 1×502; 11:30/11:35/11:40 持续 429.
延迟 (200): avg_dur 11865, max 19630, min 4856, avg_ttfb 11309.
fallback 0/14.

### 3. 错误分类 (DB 实测)
- 7 错误: 7× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 6418).
- 429 (6×): NVCF function 级配额周期, 多波, key2 同时挂 → all_tiers_exhausted.
- 502 (1×, dur 34101ms): 同 R336-R342 RemoteDisconnected transport-level hang (单发, 偶发).
- 本轮无 NVStream_IncompleteRead.
- 与 R268-R342 错误类型集合一致.
...
- 容器全 Up: nv_gw 18h, cc4101 5h, nv_gw_stable 18h, ms_gw/logs_db 3 days 持续.

## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R342 分析)
...
- 502 RemoteDisconnected transport hang: 同 R336-R342, NVCF 单 function 偶发 transport 级断连, peer 也降级时返回 502 (设计). 偶发不达介入阈值.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R342 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=50.0% (7/14), 与 R342 同窗口数据漂移 (12/18 → 7/14, 窗口滚动自然变动, 429 波次增多).
- 错误类型无新增, 与 R268-R342 一致.
- 六十五轮一致 R268-R343.
