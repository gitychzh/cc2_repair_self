...
## 当前轮: R600 (2026-08-03 12:02 CST) — NOP 巡检轮

## 基线 (R600 链路分析注入, 12:02 CST)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 13 req, 8×200 + 5×429 (SR=61.5%, hermes caller)
  - vs R599 45.5% vs R598 50.0% vs R597 50.0% vs R596 57.1% vs R595 66.7% vs R594 54.5% vs R593 69.2% vs R592 66.7% → 波动区间内一致, 与 R545-R599 同一 NVCF 配额波动模式
  - per-key: key2 8×200 (命中可用 key 100% 200, avg_dur=13624ms, IP 203.10.96.139)
  - 空 key 5×429 (全挂时 NVCF 侧拒绝, avg_dur=1377ms 快速返回, 配额波动区间)
- 唯一错误 `all_tiers_exhausted` ×5 (avg_dur=1377ms, NVCF 配额型, 非 nv_gw tier 故障)
- nv_tier_attempts 0 行 = KeyManager 全局冷却在 tier 层前拦截
- 429 全在空 key/空 IP = NVCF 侧拒绝 (配额波动区间)
- key2 = 8×200 (命中可用 key 时 100% 200)
- 无 buffer/wait 日志 (30min 无 buffer 触发), 无 stream_total_deadline, 无 zombie
- finish_reason: tool_calls×7 + stop×1 (健康, 无 zombie stop)
- 全挂时 ABORT-NO-FALLBACK 是预期 (dsv4p_nv 跳 peer/ms fb, cc4101 层 ms_gw 兜底)
- fallback 发生率 f×13 (cc4101 层 ms_gw 兜底, 预期)
- 配置与 R472-R599 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R599 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R599 一致模式)
...
- dsv4p_nv 13 req: 8×200+5×429 = NVCF 配额波动区间 (命中 key2 100% 200, 全挂时空 key 429 avg_dur=1377ms 快速返回)
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=61.5% vs R599 45.5% vs R598 50.0% vs R597 50.0% vs R596 57.1% vs R595 66.7% vs R594 54.5% vs R593 69.2% vs R592 66.7% vs R591 44.4% vs R590 37.5% vs R589 37.5% → 波动区间内, 与 R545-R599 同一 NVCF 配额波动模式
