...
> 当前轮: R803 (NOP — R802 后置续净, tier 连续 6 轮零错再创历史新高, 2026-08-05 ~09:52 CST)
> 上轮: R802 (NOP — R801 后置续净 tier 连续 5 轮零错)

## 本轮 (R803) 改动 + 依据 + 验证

### 改动: NOP — 无源码 / 无 env 改动

R796 wait_queue 180s 改 env 后第 6 轮后置验证. 读数据判稳不动码.

### 验证 (实测 30min, 2026-08-05 ~09:52)

1. **cc4101 用户视角 SR 排 499 = 100%**: total=88, ok=86 (200), c499=2 (client_gone_mid_stream), fb=0 (0%). 全 200 成功, fallback 0% ≪ 10% 目标.
2. **glm5_2_nv tier 连续 6 轮零错** (R798/R799/R800/R801/R802/R803 — 刷新历史最长净窗口):
   30min 88 nv_tier_attempts **全 pexec_success**, 零错误. per-key fid 全 b1b22d03 均布 k0:19 k1:15 k2:20 k3:18 k4:16. 5key 全活.
3. **cc4101-primary nv_gw 视角 100%** (零 502 实时窗口): 30min 88 全 200. R796 死请求 f15fe5ef (buffer_exhausted) 已滚出 30min 窗口 (延续 7 轮 R797-R803 不再产生).
4. **buffer 全 attempt=1 success**: 最近 6 个请求一次过, 7-23s 延迟, 零 retry/WAIT/KEYMGR/BREAKER.
5. **容器健康**: nv_gw (Up 41min) + cc4101 (Up 8h) + logs_db (Up 6d) 都 ok.

### 数据修正

轮前注入分析中的 `NVCFPexecRemoteDisconnected` 计数 (k0:4 k1:3+1empty k2:4 k3:2+1empty k4:2) 是分析脚本把
nv_tier_attempts.error_type 字段误读 — 实测查询 88 条全 `pexec_success`/空 (=成功). tier 连续零错窗口挺进第 6 轮, 未被打断.

## 判稳结论

- cc4101 SR 排 499 = 100% ≥ 99% 阈值 → NOP 巡检轮
- glm5_2_nv tier 连续 6 轮零错 (R798-R803) — 历史最长净窗口 (刷新 R802 的 5 轮记录)
- fallback 0% ≪ 10% 目标
- 无新错误类型 / 无新错误模式
...
- `all_tiers_exhausted × 7` + dsv4f0731_nv SR 50% (6/12): hermes caller 的 dsv4f0731_nv 链路噪声, 属并行 R-dsv4f-fallback 自优化线 (R1025), 不穿透 cc2 (cc4101-primary).
