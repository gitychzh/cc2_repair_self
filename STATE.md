...
> 当前轮: **R925 (NOP 巡检轮/不改码 — cc2 主链路连续第 34 轮 100% 干净; 坏请求 all_tiers_exhausted ×5 + zombie_empty_completion ×1 (502) 全属 hermes 线, caller 列实拉铁证, 非 cc2 范围; fallback 0 次)**
> cc4101-primary (主 nv_gw:40006) 实时 30min = **114/114 = 100% SR, 0 bad** (实拉);
> cc4101-primary 专属错误 30min = **0 rows** (114 request 全 200);
> 容器: nv_gw Up 7h, cc4101 Up 6h, dsv4p_nv40066 Up 2d
> 上轮: R924 (NOP, 主链 119/119=100%)

## 本轮 (R925) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 34 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes 非 cc2)
...
- 30min 所有 bad (502) = `caller=hermes` ×6: `all_tiers_exhausted ×5` + `zombie_empty_completion ×1`。
- **caller 列实拉铁证**: 6 bad 全 caller=hermes, 0 个属于 cc2 主链 (host-separated)。
- 聚合 dsv4f0731_nv SR 拉低 (95.2%) 纯因 hermes 502, cc2 primary 自身 100% — 不作改码依据。
- fallback (cc_requests 30min total=115) = **0 次**。
- tier 层 NVCFPexecRemoteDisconnected ×19 + timeout ×8 全被多 tier round-robin 吸收, 未浮现为 cc2 primary bad。
- 容器 health: 4101/40006 全 ok (200); nv_gw Up 7h, cc4101 Up 6h, dsv4p_nv40066 Up 2d。
...
| scoped health | 4101/40006 全 ok (200), nv_gw Up 7h | ✅ |
...
### 关键判断
cc2 主链路连续第 **34** 轮 (R892-R925) 100% SR 干净, 且 30min 主链专属错误实拉 0 rows。
