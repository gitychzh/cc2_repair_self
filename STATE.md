# R652 — NOP 巡检轮 (2026-08-03 15:40 CST)

## 本轮改了什么 + 依据 + 验证
- **改动**: 无代码改动 (NOP). 仅数据巡检 + R651 既定阈值复核.
- **依据 (实测 30min DB 15:10-15:40 CST + 150min 回查)**:
  ...
  - **150min 回查 (R651 既定阈值复核)**:
    - `where error_type='NVAnthCollect_IncompleteRead'` → **仍只 1 行 (c1297569, hits=1)**
    - 与 R651 回查结果完全一致 → 同一请求跨窗口回放, 未出现第 2 个独立 request_id
    - **R651 阈值 (150min >= 2 个不同 request_id) 未触发, 不改 collect 重试逻辑**
  - dsv4p_nv (hermes caller) 30min: 18 req 14×200+4×429 (SR=77.8%, 配额型, 与 R651 的 70.6% 同区间)
  - cc4101-fallback 1 次 (dsv4p_nv 200) — primary 路径 1 次 502 触发 cc4101 侧 fallback, fallback 链路健康
...
- **deadline 链**: 6h stream_total_deadline=0 (健康铁证, 与 R651 一致)
- **验证**:
  - /health ok: nv_num_keys=5, nv_default_model=glm5_2_nv, nvcf_pexec_models=[kimi_nv,dsv4p_nv,glm5_2_nv]
  - 容器: nv_gw Up 19min, cc4101 Up 25min, dsv4p_nv40066 Up 19min, ms_gw/logs_db Up 4 days, nv_gw_stable Up 38h 备用
  - 配置无漂移 (与 R651 快照一致): nv_gw NVU_DISABLE_MS_FALLBACK=1, buffer 5key×90s=450s, TIER_COOLDOWN=180; cc4101 PRIMARY=glm5_2_nv, FALLBACK=http://dsv4p_nv40066:40066, STREAM_TOTAL=470

## 基线 (R652 实测, 15:10-15:40 CST)
- cc2 (cc4101-primary/glm5_2_nv) 30min: 16 req, 15×200+1×502 (SR=93.8%)
  - 对比 R651: 16 req 15×200+1×502 (SR=93.8%) → 完全持平, 同一 c1297569 跨窗口
- dsv4p_nv (hermes) 30min: 18 req 14×200+4×429 (SR=77.8%, 配额型, R651 为 70.6%)
...
## 保护 gap 分析 (R649 既定, R651 修订阈值, R652 复核未触发)
...
**R652 复核**: 150min 回查 IncompleteRead 仍只 1 行 (c1297569), 未出现第 2 个独立 request_id.
**阈值 (R651 修订, R652 沿用)**:
