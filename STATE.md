# R630 — NOP 巡检轮 (2026-08-03 14:00 CST)

## 基线 (R630 实测, 05:31-06:00 UTC 窗口)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本, 同 R626-R629)
- dsv4p_nv 30min (hermes caller): 26 req, 21×200 + 3×429 + 2×502 (SR=80.8%)
  - vs R629 86.7% / R628 90.3% / R627 90.3% / R626 88.5% / R625 85.0% / R624 82.4%
    → **本轮略降 86.7%→80.8%, 仍在 R617-R618 正常波动区间上沿 (75-91%), 反弹趋势已稳定**
  - per-key: k2=21×200 全 first-attempt 命中
  - per-egress-IP: 203.10.96.139 21×200 / 5 fail (单 IP 主力, 同 R629)
  - finish_reason: tool_calls×19 + stop×2 (健康, 无 zombie)
  - fallback_occurred=f ×26 (hermes caller 直连 nv_gw)
  - 200 avg_dur=11834ms (健康, 同 R629 12740ms 量级)
- 错误分类 1 类 (非新错误):
  - `all_tiers_exhausted` ×5 (avg_dur 15050ms: 429→1462ms 快速ABORT; 502→35433ms 长ABORT)
    → R624→R625→R626→R627→R628→R629→R630: 3→3→3→3→3→4→5 (略升1, 仍在 3-5 正常区间)
- 30min nv_tier_attempts: 0 行 (hermes caller 不走 buffer, KeyManager 层 ABORT)
- cc_requests stream_total_deadline 6h: 0 (deadline 链健康)

## 持续观察点 (沿用 R629, 不改码) — RemoteDisconnected 34-35s 长卡顿
- 本轮 2×502 duration≈35433ms, 同 R626/R627/R628/R629 (34646/35433/35433/35433ms) 模式
...
## 依据
- cc2 0 流量 → 无评估样本, 铁律1 cc2 视角不满足 (同 R626-R629)
- dsv4p_nv SR=80.8% 略降 R629 86.7%, 仍在 R617-R618 正常波动区间上沿 (75-91%), 趋势稳定
- all_tiers_exhausted ×5 略升 R629 ×4, 仍在 3-5 正常区间 (R624-R630: 3→3→3→3→3→4→5), 无退化
- KeyManager 指数退避正确, 429 ABORT avg 1462ms (快速, vs R629 1311ms 略慢仍健康)
