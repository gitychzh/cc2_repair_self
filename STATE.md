...
延迟 (200): avg_dur 10655, max 20440, min 3047, avg_ttfb 10295.
fallback 0/23.

### 3. 错误分类 (DB 实测)
- 5 错误: 5× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 7949).
- 429 (4×): NVCF function 级配额周期, 多波, key2 同时挂 → all_tiers_exhausted.
- 502 (1×, dur 34101ms): 同 R336-R340 RemoteDisconnected transport-level hang (单发, 偶发).
- 本轮无 NVStream_IncompleteRead.
- 与 R268-R340 错误类型集合一致.
...
- KEYMGR 日志证实 function 级 429 配额周期自恢复:
  19:30:37 k4/k5 同时 429 → 180s cooldown;
  19:35:39 k1/k2/k3/k4/k5 同时 429 → 120-180s cooldown (指数退避正常).
  证明是 NVCF function 配额周期, 非 nv_gw 代码缺陷.

## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R340 分析)
...
- 502 RemoteDisconnected transport hang: 同 R336-R340, NVCF 单 function 偶发 transport 级断连, peer 也降级时返回 502 (设计). 偶发不达介入阈值.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R340 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=78.3% (18/23), 与 R340 同窗口数据漂移 (26/30 → 18/23, 窗口滚动自然变动, 429 波次增多).
- 错误类型无新增, 与 R268-R340 一致.
- 六十三轮一致 R268-R341.
