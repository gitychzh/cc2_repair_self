# R386: NOP 巡检轮 (cc2 0req, dsv4p_nv SR=85.2% 23/27, all_tiers_exhausted×4[3×429+1×502 avg 3944ms], 一百一十轮一致)

## 当前轮基线 (2026-08-02 22:36 CST, R385 已完成, R386 待跑)
- 本仓 master: R385 已 commit (01add5c). hermes 仓: R385 已 push (8764c54).
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R386 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=85.2% (23/27), 失败 3 = 3× all_tiers_exhausted
  ...
  **cc2 是缓冲 caller (NVU_BUFFER_CALLERS), 走 buffer 5key 轮转路径, 不走 NV-TIER-SKIP, 不受影响.**
  错误类型无新增 (dsv4p 仍 all_tiers_exhausted, 一百一十轮一致 R268-R386).
  cc2 无流量不受影响, 0 fallback 0 deadline. 0 改动 0 restart. **一百一十轮一致 R268-R386**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~22:36 CST)
...
### 2. dsv4p_nv 30min 全 caller SR=85.2% (23/27)
| caller | model | status | count | avg_dur |
|---|---|---|---|---|
| hermes | dsv4p_nv | 200 | 23 | 12186 |
| hermes | dsv4p_nv | 429 | 3 | 1684 |
| hermes | dsv4p_nv | 502 | 1 | 10722 |

per-key (dsv4p): key2 → 23×200 (12186); 空 key → 3×429 (1684) + 1×502 (10722).
per-egress: 203.10.96.139 → 23× (100%); 空 → 4× (0%).
finish_reason (200): tool_calls×19, stop×4 (无 zombie).
...
### 3. 错误分类 (30min)
- 4 dsv4p 错误: 4× all_tiers_exhausted (avg 4559ms) — NVCF dsv4p function 配额瞬时空位/上游瞬时错误
  (tier 错误明细: 3×429 + 1×502, 502 为 NVCF 上游瞬时错误归入 all_tiers_exhausted 桶).
  - 本轮无 NV-TIER-SKIP (R385 0×, 本轮 0×, 自然波动).
  - 429/502: NVCF dsv4p function 配额瞬时空位/上游瞬时错误, 低频偶发 (4/27=15%), buffer 5key 轮转 + KeyManager 指数退避自恢复.
- dsv4p 错误类型集合与 R268-R385 一致 (all_tiers_exhausted, 无新增).
- buffer/wait 日志空.

### 4. 健康检查 (沿用 R385, 容器未重启)
...
## 根因: NVCF dsv4p function 429/502 波 (非代码缺陷, 沿用 R353-R385 分析)

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**.
- all_tiers_exhausted (429/502): NVCF dsv4p function 配额瞬时空位/上游瞬时错误, 低频偶发 (4/27=15%), buffer + KeyManager 指数退避 + ProbeWorker + decayed reset 自恢复 (R268-R385 验证).
- 本轮无 NV-TIER-SKIP 无 stream_first_byte_timeout, 错误面收敛.
- 当前 cc2 流量极低, 偶发错误罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=85.2% (23/27), 较 R385 90.0% (27/30) 略降 (样本极小自然波动,
  本轮成功 27 vs 30, 失败 4 vs 2, 全部非缓冲 caller, cc2 不受影响).
  dsv4p 错误类型无新增, 与 R268-R385 一致 (一百一十轮一致).
