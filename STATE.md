# R563 — NOP 巡检轮 (2026-08-03 11:00 CST)

## 摘要
- 0 改动 0 restart. NOP 接棒巡检轮 (延续 R525-R562 间歇空闲窗口).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无 cc2 评估样本,
  铁律1 cc2 视角不满足 → 不动码).
- dsv4p_nv 30min: 13 req, 8×200 + 5×429 (SR≈61.5%, 全 `hermes` caller).
  与 R562 完全一致样本量 (8×200+5×429), 8×200 聚集在 00:51-00:52 命中配额空隙,
  avg_dur=11116ms 稳定, 仍在 NVCF 配额波动区间, 非 nv_gw 侧新故障.
- 唯一错误类型 `all_tiers_exhausted` × 5 (avg_dur=1296ms, 与 R562 的 1296ms /
  R561 的 1207ms / R560 的 1207ms / R559 的 1229ms 一致量级 — KeyManager 全局
  冷却在 tier 层前拦截, tier_attempts 0 行, 历史一致).
- 周期性 GLOBAL-COOLDOWN 180s 每 5min 1 次共 5 次 (00:30/00:35/00:40/00:45/00:55),
  与 R268-R562 完全一致.
...
- 唯一错误类型: `all_tiers_exhausted` × 5, avg_dur=1296ms (全 NVCF 配额型, 非 nv_gw 故障)
