...
## 当前轮: R599 (2026-08-03 11:52 CST) — NOP 巡检轮

## 基线 (R599 链路分析注入, 11:52 CST)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 11 req, 5×200 + 6×429 (SR=45.5%, hermes caller)
  - vs R598 50.0% vs R597 50.0% vs R596 57.1% vs R595 66.7% vs R594 54.5% vs R593 69.2% vs R592 66.7% → 波动区间内一致, 与 R545-R598 同一 NVCF 配额波动模式
  - per-key: key2 5×200 (命中可用 key 100% 200, avg_dur=15547ms, IP 203.10.96.139)
  - 空 key 6×429 (全挂时 NVCF 侧拒绝, avg_dur=1332ms 快速返回, 配额波动区间)
- 唯一错误 `all_tiers_exhausted` ×6 (avg_dur=1332ms, NVCF 配额型, 非 nv_gw tier 故障)
- nv_tier_attempts 0 行 = KeyManager 全局冷却在 tier 层前拦截
- 429 全在空 key/空 IP = NVCF 侧拒绝 (配额波动区间)
- key2 = 5×200 (命中可用 key 时 100% 200)
- 无 buffer/wait 日志 (30min 无 buffer 触发), 无 stream_total_deadline, 无 zombie
- finish_reason: tool_calls×5 (健康, 无 zombie stop)
- 全挂时 ABORT-NO-FALLBACK 是预期 (dsv4p_nv 跳 peer/ms fb, cc4101 层 ms_gw 兜底)
- fallback 发生率 f×11 (cc4101 层 ms_gw 兜底, 预期)
- 配置与 R472-R598 完全一致, 无漂移
...
- dsv4p_nv 11 req: 5×200+6×429 = NVCF 配额波动区间 (命中 key2 100% 200, 全挂时空 key 429 avg_dur=1332ms 快速返回)
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=45.5% vs R598 50.0% vs R597 50.0% vs R596 57.1% vs R595 66.7% vs R594 54.5% vs R593 69.2% vs R592 66.7% vs R591 44.4% vs R590 37.5% vs R589 37.5% → 波动区间内, 与 R545-R598 同一 NVCF 配额波动模式
