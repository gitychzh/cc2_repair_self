...
> 当前轮: **R901 (NOP 巡检轮/不改码 — cc2 主链路连续第 10 轮 100% 干净; 5 条 all_tiers_exhausted (502) + 26 条 bad fid 52e1ddb6 全属 hermes 线, JOIN 铁证, 非 cc2 范围)**
> cc4101-primary (主 nv_gw:40006) 实时 30min = **127/127 = 100% SR, 0 bad** (实拉);
> live DB now()≈2026-08-07 10:00 CST
> 上轮: R900 (NOP, 主链 126/126=100%)

## 本轮 (R901) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 10 轮 100% 干净, 无新错误类; bad 请求 + bad fid 全属 hermes 非 cc2)

### 依据 (live DB 30min 实拉, ≈2026-08-07 10:00 CST)

- 30min cc4101-primary (主 nv_gw:40006) = **127/127 全 200, 0 bad (100% SR)**。
  实拉 `WHERE caller='cc4101-primary' AND status!=200` → **0 条**。
- 30min 所有 bad = `all_tiers_exhausted ×5 (max 180s)`, 实拉按 caller 分组唯一 caller=**hermes**。
- per-key 分布: 主链各 key 走健康 fid 281478d0 (每 key 25-27); 同时有 26 条坏 fid 52e1ddb6
  出没 → **request_id JOIN 铁证**: 26 条 52e1ddb6 全部 caller=hermes, cc2 主链 0 泄漏。
- buffer (cc4101-primary): 全 attempt=1/5 成交, 8-10s 复盘, 0 重试 / 0 429 / 0 cooldown。
- fallback (cc2 线) 0 次 (130 total, 0 fb)。
...
| hermes 线 bad (502) | all_tiers_exhausted ×5, max 180s | ⚠️ 越界 |
| bad fid 52e1ddb6 | 26 条全 caller=hermes (JOIN 铁证), cc2 0 泄漏 | ✅ 隔离 |
| per-key (nv_tier_attempts) | 主链各 key 281478d0 24-27, 偶发远端瞬态，无雪崩 | ✅ |
...
- 52e1ddb6 归属: `nv_tier_attempts JOIN nv_requests` → 26 条全 caller=hermes, cc2 0 泄漏。

### 关键判断
cc2 主链路连续 10 轮 (R892 139/139, R893 153/153, R894 143/143, R895 137/137, R896 134/134,
R897 126/126, R898 125/125, R899 124/124, R900 126/126, **R901 127/127**) 100% SR 干净。
bad 请求 + bad fid 52e1ddb6 100% 属 hermes caller 活动, JOIN 铁证未进 cc2 主链候选池。
