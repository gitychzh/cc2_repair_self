# R632 — NOP 巡检轮 (2026-08-03 14:10 CST)

## 基线 (R632 实测, 05:36-06:06 UTC 窗口)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本, 同 R626-R631)
- dsv4p_nv 30min (hermes caller): 19 req, 14×200 + 3×429 + 2×502 (SR=73.7%)
  - vs R631 76.2% / R630 80.8% / R629 86.7% / R628 90.3% / R627 90.3% / R626 88.5%
    → **本轮略降 76.2%→73.7%, 仍在 R617-R631 正常波动区间下沿 (73-91%), 小样本 19req 波动, 趋势未恶化**
  - per-key: k2=14×200 全 first-attempt 命中
  - per-egress-IP: 203.10.96.139 14×200 / 5 fail (单 IP 主力, 同 R630/R631)
  - finish_reason: tool_calls×13 + stop×1 (健康, 无 zombie)
  - 200 avg_dur=9797ms (健康, vs R631 10166ms 略快)
- 错误分类 1 类 (非新错误):
  - `all_tiers_exhausted` ×5 (avg_dur 15050ms: 429→1462ms 快速ABORT; 502→35433ms 长ABORT)
    → R624→R632: 3→3→3→3→3→4→5→5→5 (持平 R630/R631, 仍在 3-5 正常区间)
- 30min nv_tier_attempts: 0 行 (hermes caller 不走 buffer, KeyManager 层 ABORT)
- cc_requests stream_total_deadline 6h: 0 (deadline 链健康)

## 持续观察点 (沿用 R631, 不改码) — RemoteDisconnected 34-35s 长卡顿
- 本轮 2×502 duration≈35433ms, 同 R626-R631 (34646/35433/35433/35433/35433/35433ms) 模式
- 根因不变: NVCF 上游慢响应 (34s+ 主动 FIN) + peek-retry 单 key 语义不轮转, 非 timeout 配置
- 被 cc4101 fallback ms_gw 兜住, 用户侧无感
- 改 nvcf_conn.py read timeout = 高风险 (影响全 key 全 model), 非本轮该动

## 本轮改动
- 无 (NOP). 根因不变: NVCF 配额型 + 上游慢响应, 非 nv_gw 侧可改.

## 依据
- cc2 0 流量 → 无评估样本, 铁律1 cc2 视角不满足 (同 R626-R631)
- dsv4p_nv SR=73.7% 略降 R631 76.2%, 仍在正常波动区间下沿 (73-91%), 小样本波动无恶化
- all_tiers_exhausted ×5 持平 R630/R631 (R624-R632: 3→3→3→3→3→4→5→5→5), 无退化
- KeyManager 指数退避正确, 429 ABORT avg 1462ms (快速, 同 R631)
