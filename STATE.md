# R472 — NOP 巡检轮 (2026-08-03 03:35 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮.
- 数据窗口 18:50-19:20 UTC (03:34 CST 注入, 与 R470/R471 同窗口).
- cc2 (cc4101-primary) 30min 仍 0 req (cc2 session 间歇空闲, 无评估样本).
- dsv4p_nv 全 caller 30min SR=44.4% (4/9), 与 R467-R471 六轮锁定.
- 错误: all_tiers_exhausted ×5 (唯一类型, 模式与 R268-R471 一致, 无新错误).
- nv_tier_attempts 30min 0 行 → 429 在 tier 层前被拒 (空 IP).
- 6h stream_total_deadline = 0 (deadline 链无越界).
- nv_gw Up 13h, cc4101 Up 3h (本轮未重启).
- 连续 19 轮无 502 (R454-R472), 单点 502 模式似已消退.
- 配置实测确认与 R470/R471 完全一致, 无漂移.

## 链路数据 (03:34 CST 注入)
- 全 caller 30min: 4×200 + 5×429 = SR 44.4% (9req, 与 R470/R471 一致)
  - hermes|dsv4p_nv: 3×200 + 5×429
  - openclaw|dsv4p_nv: 1×200
- cc4101-primary 30min: 0 rows (cc2 0 流量)
- 错误分类: all_tiers_exhausted ×5 (唯一类型, avg_dur 1293ms)
- per-key: nv_key_idx=2 3×200(13222ms); nv_key_idx=3 1×200(2904ms); NULL key 5×429
- per-egress-IP: 203.10.96.139 3×100%; 134.195.101.194 1×100%; 空 IP 5×失败
- 200 延迟: avg 10643ms, max 19084ms, min 2904ms, avg_ttfb 10437ms
- finish_reason: stop×2, tool_calls×2 (无 zombie, 全部正常结束)
- fallback: f×9 (ms_gw 未触发, dsv4p_nv 自恢复足够)
- nv_tier_attempts: 0 行 (429 未进入 tier 尝试, 空 IP)
- per-min 趋势: 19:05(200×2)/19:06(200×2)/19:10(429)/19:15(429)/19:20(429)/19:25(429)/19:30(429)
  - 19:05-19:06 连续 200 恢复明确, 其余离散 429 (每5min1次) 非集中爆发
- buffer/wait 日志: 30min 无 BUFFER-/WAIT- 行 (cc4101-primary 0 req, 无 buffer 触发样本)

## keymgr 行为 (沿用 R470 观察, 本轮未重复拉日志)
...
## 历史波动区间 (R437-R472)
R437=85.0 → ... → R467=44.4 → R468=44.4 → R469=44.4 → R470=44.4 → R471=44.4 → R472=44.4% (六轮锁定 44.4%)

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型仅 all_tiers_exhausted ×5, 模式与 R268-R471 一致, 无新错误.
- dsv4p_nv SR=44.4% 与 R467-R471 完全一致, 六轮锁定, 仍处 46.2%-85% 历史波动区间, 属稳态行为.
- 5×429/30min ≈ 10/h 略高于 5/h 阈值, 但 per-min 趋势呈"19:05-19:06 连续 200 + 其余离散 429"恢复模式, 非集中爆发, 整体可接受.
- 本轮无 502 (R454-R472 连续 19 轮无 502, 单点模式似已消退, 继续观察).
- fallback 未触发 (ms_gw 已恢复但 dsv4p_nv 自恢复足够, 无需 fallback).
- 0 restart → 无需 py_compile / curl 复测.
- 配置实测与 R470/R471 完全一致, 无配置漂移.
...
- 留意 502 是否再现 (R454-R472 连续 19 轮无 502, 再现 >=3/h 才介入).
