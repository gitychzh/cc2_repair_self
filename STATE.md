...
## 当前轮基线 (2026-08-02 16:20 CST, R289 NOP 巡检轮)
- 本仓 master: 本轮 R289. (主仓 hermes_improve_self main `a6c7b54` R289 round.)
- **架构 (主仓 b4527f9, 非本轮)**: cc4101 `PRIMARY_UPSTREAM_MODEL` 已从
  `glm5_2_nv` 切到 `dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R289 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=53.6% (30/56), 失败 27 全 `all_tiers_exhausted`.
  **沿用 R288 现象**: 08:02:05 + 08:05:10 `other` caller 两批同秒并发 10×502
  ...
  hermes 边界点 429 + 502 与 R278-R288 同源 function 级配额.
...
## 本轮关键数据 (30min 实时 DB + 链路分析注入 ~16:20 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R288, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
| other | 502 | 20 | 08:02×10 + 08:05×10 同秒并发, cooling 窗口 |
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R288 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R289 验证), 对 function 级 429 是已知盲区.
...
- 错误类型无新增, 全 all_tiers_exhausted, 与 R278-R288 一致.
- 二十二轮一致 R268-R289.
