# R359: NOP 巡检轮 (cc2 0req, dsv4p_nv SR=0.0% 0/6, all_tiers_exhausted×6, 根因不变)

**时间**: 2026-08-02 21:05 CST
**方向**: R-nvonly (ms_gw fallback 已恢复, 不主动禁用)
**改动**: 0 改动 0 restart (NOP 巡检轮)
**结论**: 链路空闲健康, 错误类型无新增, 八十二轮一致 R268-R359.

## 本轮数据 (30min 链路分析注入 ~20:52 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R358, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).

### 2. dsv4p_nv 30min 全 caller (hermes) SR=0.0% (0/6)
| caller | model | status | count |
|---|---|---|---|
| hermes | dsv4p_nv | 429 | 6 |

per-key (dsv4p): 空 key → 6×429 (avg_dur 1463).
per-egress: 空 IP → 6×429.
分钟趋势: 12:25-12:50 每 5min 单发 1×429 (function 配额周期耗尽, 6 波).
fallback f×6 (hermes caller fallback ms_gw 成功兜底).
延迟 (200): 无 200 样本.

### 3. 错误分类 (DB 实测)
- 6 错误: 6× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 1463).
- 6× 429: NVCF function 级配额周期单发, 5 key 同 function → all_tiers_exhausted.
- 本轮 **无 NVStream_IncompleteRead** (R353 已清零, 周期性低频偶发).
- 与 R268-R358 错误类型集合一致 (无新增).
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R358 分析)
...
- 单波 429 证明是 NVCF function 配额周期自恢复 (5min 周期), 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.
- avg_dur 1463s 是 all_tiers_exhausted 累积 KEYMGR 指数退避等待时长.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R358 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=0.0% (0/6), 较 R358 14.3% (1/7) 略低 (样本极小自然变动, 无 200 样本).
- 错误类型无新增, 与 R268-R358 一致 (八十二轮一致).
- 八十二轮一致 R268-R359.
