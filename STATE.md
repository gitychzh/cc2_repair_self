...
> 当前轮: **R950 (NOP 巡检轮/不改码 — cc2 主链路连续第 58 轮 100% 干净; 坏请求 all_tiers_exhausted ×2 + stream_absolute_cap ×1 全属 hermes 线, caller 列 + host_machine 双重归属证明非 cc2 范围; fallback 0 次; zombie_empty_completion 本轮窗口未现)**
> cc4101-primary (主 nv_gw:40006) 实时 30min = **121/121 = 100% SR, 0 bad** (live 实拉);
> cc4101-primary 专属错误 30min = **0 rows** (121 request 全 200);
> 容器: nv_gw Up 8h, cc4101 Up 8h, nv_gw_stable Up 5d(并存), dsv4p_nv40066 Up 2d, dsvf0731_nv40666 Up 18h
> 上轮: R949 (NOP, 主链 121/121=100%)

## 本轮 (R950) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链路连续 57 轮 100% 干净, 主专属错误 0 行; bad 请求全属 hermes 非 cc2; zombie_empty_completion 本轮窗口 0)
...
- 30min 所有 bad (非 200) = **caller=hermes all ×3** + **host_machine=opc2sname-dsv4f40666**:
  `all_tiers_exhausted ×2` + `stream_absolute_cap ×1` (502×3, 全部坏 fid 52e1ddb6, 越界泄漏源容器 40666)。
- bad 请求 100% 属 hermes (caller 列 + host_machine 双重归属, host-separated), 0 个属于 cc2 主链。
- zombie_empty_completion 本轮窗口未现 (R941 首现 1 → R948 连续 8 轮 1 → R949 0 → R950 0)。
- fallback (cc_requests 30min) = **0 次** (121 req, fb=0, SR=100.0%)。
- nv_tier_attempts 30min: pexec_success (k0:23/k1:25/k2:25/k3:23/k4:25) + NVCFPexecRemoteDisconnected (k0:3/k1:3/k2:6/k3:4/k4:3) + NVCFPexecTimeout (k4:2) + empty_200 (k1:1)。瞬态错误被多 tier round-robin + buffer 重试吸收, 全部 resolve 为 200。
- buffer 日志: cc4101-primary 全 attempt=1 成功 (elapsed 6~9s, verdict=success_tool_call), 无 WAIT-/KEYMGR- 错误。
...
1. glm5_2_nv 62.5% 疲劳 → R829/R833 fail-fast + cc4101 动态 primary → dsv4f0731_nv (now real model dsvf0731_nv)
2. dsv4f0731_nv 一次成功 (主链已用健康 fid 281478d0), 用户无感知
3. 多 tier round-robin + func_health fid 健康选择自适应吸收底层跨 key/fid 瞬态失败
4. 坏 fid 52e1ddb6 (hermes 宿主 40666) 与主链健康 fid 281478d0 容器+候选池双层隔离
