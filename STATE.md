...
## 当前轮: R595 (2026-08-03 11:36 CST) — NOP 巡检轮

## 基线 (R595 实测, 11:36 CST 链路分析注入 + 复测确认)
- cc2 (cc4101-primary) 30min: 0 req (session 间歇空闲, 无 cc2 评估样本)
- dsv4p_nv 30min: 15 req, 10×200 + 5×429 (SR=66.7%, hermes caller)
  - vs R594 54.5% vs R593 69.2% vs R592 66.7% → 波动区间内一致, R545-R594 同一 NVCF 配额模式
  - per-key: key2 10×200 (命中可用 key 100% 200, avg_dur=12789ms, IP 203.10.96.139)
  - 空 key 5×429 (全挂时 NVCF 侧拒绝, avg_dur=2310ms 快速返回, 配额波动区间)
- 唯一错误 `all_tiers_exhausted` ×5 (avg_dur=2310ms, NVCF 配额型, 非 nv_gw tier 故障)
...
- 配置与 R472-R594 完全一致, 无漂移

## 6h SR 趋势 (dsv4p_nv, 按小时, R575 实测复测, R576-R594 一致模式)
...
## 6h per-key × status (dsv4p_nv, R575 实测, R576-R594 一致模式)
...
- dsv4p_nv 15 req: 10×200+5×429 = NVCF 配额波动区间 (命中 key2 100% 200, 全挂时空 key 429 avg_dur=2310ms 快速返回)
...
- 无新错误类型, 无参数漂移 → 无介入必要
- 本轮 SR=66.7% vs R594 54.5% vs R593 69.2% vs R592 66.7% vs R591 44.4% vs R590 37.5% vs R589 37.5% vs R588 37.5% vs R587 37.5% vs R586 37.5% vs R585 37.5% vs R584 71.4% vs R583 71.4% vs R582 58.3% → 波动区间内, 与 R545-R594 同一 NVCF 配额波动模式
