# R653 — NOP 巡检轮 (2026-08-03 15:41 CST)

## 本轮改了什么 + 依据 + 验证
- **改动**: 无代码改动 (NOP). 仅数据巡检 + R651 既定阈值复核 (R652→R653 第三轮复核).
- **依据 (实测 30min DB ~15:10-15:41 CST + 150min 回查)**:
  ...
  - **150min 回查 (R651 既定阈值复核, 第三轮)**:
    - `where error_type='NVAnthCollect_IncompleteRead'` → **仍只 1 行 (c1297569, hits=1, 07:20:50)**
    - min=max=07:20:50 → 单次事件, 非跨窗口回放增长
    - 与 R651/R652 三轮复核完全一致 → **R651 阈值 (150min >= 2 个不同 request_id) 未触发, 不改 collect 重试逻辑**
  - dsv4p_nv (hermes caller) 30min: 15 req 11×200+4×429 (SR=73.3%, 配额型, 与 R652 的 77.8% 同区间)
...
- **deadline 链**: 6h stream_total_deadline=0 (健康铁证, 与 R652 一致)
- **验证**:
  - /health ok: nv_num_keys=5, nv_default_model=glm5_2_nv, nvcf_pexec_models=[kimi_nv,dsv4p_nv,glm5_2_nv]
  - 容器: nv_gw Up 21min, cc4101 Up 28min, dsv4p_nv40066 Up 21min, ms_gw/logs_db Up 4 days, nv_gw_stable Up 38h 备用
  - 配置无漂移 (与 R652 快照一致): nv_gw NVU_DISABLE_MS_FALLBACK=1, buffer 5key×90s=450s, TIER_COOLDOWN=180; cc4101 PRIMARY=glm5_2_nv, FALLBACK=http://dsv4p_nv40066:40066, STREAM_TOTAL=470

## 基线 (R653 实测, ~15:10-15:41 CST)
- cc2 (cc4101-primary/glm5_2_nv) 30min: 16 req, 15×200+1×502 (SR=93.8%)
  - 对比 R652: 16 req 15×200+1×502 (SR=93.8%) → 完全持平, 同一 c1297569
- dsv4p_nv (hermes) 30min: 15 req 11×200+4×429 (SR=73.3%, 配额型, R652 为 77.8%)
...
## 保护 gap 分析 (R649 既定, R651 修订阈值, R653 第三轮复核未触发)
...
**R653 复核**: 150min 回查 IncompleteRead 仍只 1 行 (c1297569, min=max=07:20:50), 未出现第 2 个独立 request_id.
**阈值 (R651 修订, R653 沿用)**:
