...
## 当前轮基线 (2026-08-02 12:39 CST, R-nvonly-post223 NOP 巡检轮)
- 主仓 git HEAD: f65590a (post219); 本仓 master: 本轮 post223.
  (注: 本仓 STATE.md 走 master, 主仓 hermes_improve_self main 收 round 文件.)
- **本轮 R-nvonly-post223 (hm2_cc2)**: NOP 巡检轮. cc2 30min 0 req (session 轮前无流量产生, 无数据可判 SR).
  ...
  0 改动, 0 重启.
  hermes 打 dsv4p_nv 19req (15×200/4×429, SR=78.9%, all_tiers_exhausted ×4, 配额限流, key2 单 key 扛 15×200).
  **与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv).
  glm5_2_nv 连续 post100-post223 (122 轮) 无 dsv4p 故障扩散.
...
|--------|--------|-----|-----|-----|
| hermes | dsv4p_nv | 15 | 4 | 78.9% (19req) |

hermes→dsv4p_nv SR=78.9% (19req): 15×200 + 4×429, all_tiers_exhausted ×4 (avg_dur 3663ms, NVCF 配额限流).
per-key: key2 扛 15×200 (单 key 健康), 4×429 来自无 key 映射 (empty key).
per-egress: 203.10.96.139 扛 15×200 (100% SR), 4×429 来自无 egress (空 IP).
**与 cc2 无关** (cc2 走 glm5_2_nv, 不打 dsv4p_nv).
...
| 30min 全 caller | hermes 19req dsv4p_nv (15×200/4×429 限流), cc2 0 req ✓ |
| 配置 | NVU_DISABLE_MS_FALLBACK=0 (fallback 已恢复), FALLBACK_UPSTREAM=ms_gw:40007 ✓ |

## 参数快照 (2026-08-02 12:39 CST, 无变化, 同 post219)
