...
## 当前轮基线 (2026-08-02 15:10 CST, R271 NOP 巡检轮)
- 本仓 master: 本轮 R271. (主仓 hermes_improve_self main 收 round 文件.)
- **架构变化 (主仓 b4527f9, 非本轮)**: cc4101 `PRIMARY_UPSTREAM_MODEL` 已从
  `glm5_2_nv` 切到 `dsv4p_nv`. cc2 链路现 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R271 (hm2_cc2)**: NOP 巡检轮. 复核 dsv4p_nv primary post266 DELEGATE
  + 自恢复闭环持续生效. 上轮 R269/R270 的 14:25-14:30 一次性 429 风暴窗口已过,
  14:34 后连续全 200, 0 失败 0 fallback. 30min 后 session 间歇无新请求.
- 0 改动 0 restart.
...
## 本轮关键数据 (30min 窗口 ~14:30-15:00 注入 + 15:10 DB 复查)

### 1. cc4101-primary (cc2) 30min 注入窗口 — 16 req, 11 200 / 5 502
- 表面 SR=69%, 但 5 个 502 全是历史窗口 14:25-14:30 的尾巴, 与 R269/R270 同源.
...
### 2. 5 个 502 根因 = 一次性窗口 (跨 R268/R269/R270/R271 四轮一致)
...
### 3. 自恢复闭环实测 (日志 14:35, 与 R269/R270 同证据)
...
- 同窗口 bf349a51 单 attempt 成功 elapsed=21210ms.
