...
> 当前轮: R867 (NOP 巡检轮 — 近窗 cc4101-primary SR=100% (122×200) 零错误, 残留 all_tiers_exhausted×4+stream_cap×1 全为 hermes 外部 cron (严格~6min周期, fid 52e1ddb6), 不改码, 2026-08-07 ~05:2x CST)
> 上轮: R866 (NOP — 近窗 125×200 零错误, hermes 周期 all_tiers_exhausted 属外部 cron, 不改码)

## 本轮 (R867) 改动 + 依据 + 验证

### 改动: 无 (巡检轮 — cc2 路径全净 122×200, hermes 周期 all_tiers_exhausted 与 cc2 无关)
...
| **per-key tier attempts** | 119 pexec_success, 瞬态错误跨 key round-robin 吸收 | ✅ |
| **buffer** | 全 attempt1 一次成交 (8-13s, verdict=success_tool_call/text, flushed), 无退化 | ✅ |
...
严格 ~6min 周期 (20:54/21:00/21:06/21:09/21:16, stream_cap 为 21:09 周期尾), 全 fid=52e1ddb6.
per-key nv_tier_attempts 显示 5 key 均有足量 pexec_success (24/24/24/24/23 总 119),
瞬态错误 (NVCFPexecRemoteDisconnected×17 / NVCFPexecTimeout×5 / empty_200×3 / 529_nv_overloaded×3 / 504_nv_gateway_timeout×1)
被 KeyManager 跨 key round-robin 修复链平滑吸收, 未上抛到 cc2 用户请求. cc2 路径 122×200 零错误,
buffer 全 attempt1 一次成交 (8-13s, flushed), 证明链路/KeyManager 无退化. 不改码.
