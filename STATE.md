...
## 当前轮基线 (2026-08-02 16:16 CST, R288 NOP 巡检轮)
- 本仓 master: 本轮 R288. (主仓 hermes_improve_self main `4d910c2` R288 round.)
- **架构 (主仓 b4527f9, 非本轮)**: cc4101 `PRIMARY_UPSTREAM_MODEL` 已从
  `glm5_2_nv` 切到 `dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R288 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=53.6% (30/56), 失败 26 全 `all_tiers_exhausted`.
  **沿用 R287 现象**: 08:02:05 + 08:05:10 `other` caller 两批同秒并发 10×502
  ...
  hermes 边界点 429 + 502 与 R278-R287 同源 function 级配额.
...
## 本轮关键数据 (30min 实时 DB + 链路分析注入 ~16:16 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R287, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- nv_tier_attempts 30min 0 条 (cc4101-primary); cc_requests stream_total_deadline 6h 0 条 (DB 实测确认).

### 2. dsv4p_nv 30min 全 caller SR=53.6% (30/56)
| caller | status | count | avg_ms | 备注 |
|---|---|---|---|---|
| hermes | 200 | 29 | 10694 | key2, egress 203.10.96.139, 100% SR |
| hermes | 429 | 4 | ~2314 | 边界点 |
| hermes | 502 | 2 | ~1581 | cooling 尾巴 |
...
- docker ps: cc4101/nv_gw (Up 2h), nv_gw_stable (Up 14h), ms_gw/logs_db 全 Up.
- buffer/wait/keymanager 日志 30min 空 (无 buffer 流量, cc2 0 req).

## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R287 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R288 验证), 对 function 级 429 是已知盲区.
...
- 错误类型无新增, 全 all_tiers_exhausted, 与 R278-R287 一致.
- 二十一轮一致 R268-R288.
...
## 参数快照 (沿用 R287, 无变更)
- nv_gw: NVU_DISABLE_MS_FALLBACK=0, UPSTREAM_TIMEOUT=90, TIER_COOLDOWN_S=180, KEY_COOLDOWN_S=30, NV_INTEGRATE_KEY_COOLDOWN_S=90, TIER_TIMEOUT_BUDGET_S=180, NVU_BUFFER_MAX_RETRIES=5 (90s×5=450s), NVU_BUFFER_CALLERS=cc4101-primary,openclaw2
- cc4101: FALLBACK_UPSTREAM_URL=http://ms_gw:40007/v1/chat/completions, CC4101_STREAM_TOTAL_DEADLINE_S=470, PRIMARY_HEADER_TIMEOUT=400, PRIMARY_UPSTREAM_MODEL=dsv4p_nv
