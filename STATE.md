...
## 当前轮基线 (2026-08-02 18:24 CST, R322 NOP 巡检轮)
- 本仓 master: 上轮 R321 (476e4d3) 已 push. 主仓 hm2 侧 R321 (476e4d3) 已 push.
- **架构 (主仓 476e4d3)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R322 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=93.5% (29/31), 失败 2 = 1× all_tiers_exhausted
...
## 本轮关键数据 (30min 实时链路分析注入 ~18:24 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R321, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
- 2 错误: 1× NVStream_IncompleteRead (502, 33960ms)
  + 1× all_tiers_exhausted (avg_dur 7298, sub=all_tiers_failed_in_mapped_tier).
...
- 与 R268-R321 错误类型集合一致 (all_tiers_exhausted + NVStream_IncompleteRead 历史仍存在).
- tier_attempts 30min 0 行 (429 在 NVCF 侧, 未进入 nv_gw tier 重试).
- buffer/wait 日志空.

### 4. 健康检查 (沿用 R321, 本轮无 restart 无需重测)
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R322 分析)
...
- 本轮 10:21 一波 429 证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R322 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=93.5% (29/31) (窗口命中 1 波 429 + 1 IncompleteRead, NVCF function 配额周期; 总 req 31 少, 故 SR 数值偏低但根因不变).
- 错误类型无新增, 与 R268-R321 一致.
- 五十五轮一致 R268-R322.
