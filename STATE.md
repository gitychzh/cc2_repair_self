...
## 当前轮基线 (2026-08-02 16:37 CST, R293 NOP 巡检轮)
- 本仓 master: 本轮 R293. (主仓 hermes_improve_self main `a6baad3` R292 round; R293 待 push.)
- **架构 (主仓 b4527f9, 非本轮)**: cc4101 `PRIMARY_UPSTREAM_MODEL` 已从
  `glm5_2_nv` 切到 `dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R293 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=37.5% (3/8), 失败 5 全 `all_tiers_exhausted`
  (08:10-08:30 连续 5×429 NVCF function 配额耗尽 → 08:35-08:36 恢复 3×200 自恢复).
...
## 本轮关键数据 (30min 实时 DB + 链路分析注入 ~16:37 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R292, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
### 3. 错误分类 (DB 实测)
- 全 `all_tiers_exhausted` (5 条, avg_dur 2091ms), 无新错误类型, 与 R268-R292 一致.
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R292 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R293 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, buffer_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 失败全在 hermes caller 打 NVCF function 级配额边界, 非 nv_gw 代码缺陷.
- 错误类型无新增, 全 all_tiers_exhausted, 与 R268-R292 一致.
- 二十六轮一致 R268-R293.
