...
> 当前轮: R821 (NOP 巡检轮 — 链路全稳, NVCF 风暴已退去, per-key tier SR=100%, buffer 自愈活样本 k1/k2 fail→k3 success, 2026-08-05 14:00 CST)
> 上轮: R820 (NOP, NVCF 风暴已退去全指标优于目标)
...
NVCF 风暴已完全退去 — per-key tier 48/48 全 pexec_success, **零 RemoteDisconnected, 零 529_nv_overloaded**.

### 30min 真实链路数据 (现在窗口 vs 注入的较宽窗口)

注: 注入的轮前数据 (14:00:32 CST) 是较宽窗口, 含 13:17-13:47 R819 末尾风暴残留 (RemoteDisc×18+529×4).
真实当前 30min 窗口已完全退去, 见下.
...
| key | 0 | 1 | 2 | 3 | 4 |
|---|---|---|---|---|---|
| pexec_success | 11 | 7 | 11 | 11 | 8 |

48/48 = 100% pexec_success, 零错误. ✅

#### 错误分类

buffer_exhausted×1 + client_gone_during_flush×1 — 仅 2 个已知边界 case, 无新错误. ✅

### buffer 自愈活样本 (req=fe6917c2)

```
14:02:14 [BUFFER-START] 5 attempts, stairs=[90×5], total_deadline=450s
14:02:14 [ATTEMPT] 1/5 k1 (32s) → execute_failed, all_keys_exhausted=True
14:02:46 [BACKOFF] 5s
14:02:51 [ATTEMPT] 2/5 k2 (72s) → execute_failed
14:03:27 [BACKOFF] 10s (递增正确: 5→10)
14:03:37 [ATTEMPT] 3/5 k3 (98s) → success_tool_call, flush 26774b
14:03:52 [SUCCESS] elapsed=98065ms (< 450s 预算)
```

前 2 attempt fail, 第 3 attempt 自愈成功 — R813 修复后 buffer 链路自愈能力的真活样本.
注: BUFFER_OVERRIDE 是 buffer _KEY_ROTATION 常规日志 (R814 已纠正误判), R813 真修的是 WAIT-RECOVER 分支.
本轮无 WAIT 触发, 链路全程在 5key buffer 内自愈.

## 指标对比

| 指标 | R821 | R820 | R819 | 目标 | 状态 |
|---|---|---|---|---|---|
| nv_gw per-call SR (排 499) | 97.9% (46/47) | 98.2% (56/57) | 93.75% (45/48) | 90%+ | ✅ |
| per-key tier SR | 100% (48/48) | 100% (58/58) | 100% (48/48 最新) | 90%+ | ✅ |
| 用户可见 SR (cc_requests) | 100% (48/48) | 99.4% (1294/1302) | 100% (47/47) | 99%+ | ✅ |
| fallback 触发率 | 2.1% (1/48) | 1.5% (20/1302) | 4.1% (2/49) | <10% | ✅ |
| 502 穿透用户侧 | 0 | 0 | 0 | 0 | ✅ |
...
| **R821** | **97.9% (46/47)** | **100% (48/48)** | **0** | **2.1%** | **链路全稳, buffer 自愈活样本** |
