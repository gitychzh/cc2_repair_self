...
## 当前轮基线 (2026-08-02 18:41 CST, R327 NOP 巡检轮)
- 本仓 master: 上轮 R326 (2c9ae8a) 已 push. 主仓 hm2 侧 R326 (8c0a0e3) 已 push; R327 (8e86136) 已 push.
- **架构 (主仓 8e86136)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R327 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=78.9% (15/19), 失败 4 = 4× all_tiers_exhausted
  (10:21-10:35 一波 429 NVCF function 配额周期, 5key 同时挂, 自恢复).
  本轮无 NVStream_IncompleteRead (R325 有 1, R326/R327 均为 0).
  错误类型无新增, 与 R268-R326 一致.
...
## 本轮关键数据 (30min 实时链路分析注入 ~18:40 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R326, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
- 与 R268-R326 错误类型集合一致 (all_tiers_exhausted + NVStream_IncompleteRead 历史仍存在).
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R327 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=78.9% (15/19) (窗口命中 1 波 429×4, NVCF function 配额周期;
  总 req 19 少, 故 SR 数值偏低但根因不变).
- 错误类型无新增, 与 R268-R326 一致.
- 六十轮一致 R268-R327.
