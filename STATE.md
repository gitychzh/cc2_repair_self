...
## 当前轮基线 (2026-08-02 ~18:50 CST, R330 NOP 巡检轮)
- 本仓 master: 上轮 R329 (e358fb5) 已 push. 主仓 hm2 侧 R329 (1f1ed56) 已 pull (up to date).
- **架构 (主仓 1f1ed56)**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`.
  cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R330 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=33.3% (3/9), 失败 6 = 6× all_tiers_exhausted
  (10:21-10:45 一波 429 NVCF function 配额周期, 5key 同时挂, 自恢复).
  本轮无 NVStream_IncompleteRead (R325 有 1, R326-R330 均为 0).
  错误类型无新增, 与 R268-R329 一���.
...
## 本轮关键数据 (30min 实时链路分析注入 ~18:50 CST)
...
### 4. 健康检查 (沿用 R329 实测, 本轮 0 改动 0 restart)
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R329 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R330 验证), 对 function 级 429 是已知盲区.
