...
## 当前轮基线 (2026-08-02 20:26 CST, R351 NOP 巡检轮 已完成, R352 待跑)
- 本仓 master: R351 待 push. hermes 仓: R351 本轮 commit+push.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R351 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  ...
  transport-level 偶发单发, R345/R347/R348/R349/R350 出现 R346 清零, 周期低频不达介入阈值).
  错误类型无新增, 与 R268-R350 一致. cc2 无流量不受影响, 0 fallback 0 deadline.
  0 改动 0 restart. **七十四轮一致 R268-R351**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~20:26 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R350, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
...
- 1× NVStream_IncompleteRead (502): transport-level 偶发单发 (R345/R347/R348/R349/R350 出现→R346 清零→R351 再单发), 不达介入阈值.
- 与 R268-R350 错误类型集合一致.
- tier_attempts 30min 0 行 (429 在 NVCF 侧, 未进 nv_gw tier 重试).
- buffer/wait 日志空.

### 4. 健康检查 (沿用 R350, 容器无 restart)
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R350 分析)
...
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R350 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.
- NVStream_IncompleteRead 偶发单发不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=89.7% (26/29), 较 R350 93.5% (29/31) 略低 (样本极小自然变动, +1 all_tiers_exhausted).
- 错误类型无新增, 与 R268-R350 一致 (七十四轮一致).
- 七十四轮一致 R268-R351.
