...
> 当前轮: **R1068 (NOP 巡检轮/不改码 — cc2 主链 114/114=100% SR, 0 bad; fallback 0; 唯一 bads 均 hermes 越界宿主非 cc2 范围; 连续多轮达完全健康基线)**
> cc4101-primary (主 nv_gw:40006) 实测 30min = **114/114 = 100% SR, 0 bad**;
> dsv4f0731_nv 整体 SR=98.1% (153/156) — 3 个 502 全属 hermes 越界宿主, 非主链;
> 30min cc_requests = **114/114 = 100%, fallback 0 次 / 0.0%** (DB 复核);
> 错误分类: zombie_empty_completion×3 (全 hermes 越界宿主, 非 cc2 范围);
> 错误分类: zombie_empty_completion×3 (全 hermes 越界宿主, 非 cc2 范围);
> buffer 日志无 fail/WAIT/KEYMGR;
> per-key 全 5 key 正常 pexec_success, 仅单条 RemoteDisconnected 噪声;
> 容器 (/health 复核): nv_gw Up 21h, cc4101 Up 16h, /health 40006/4101 全 200
> 上轮: R1067 (NOP, 主链 111/111=100%)

## 本轮 (R1068) 改动 + 依据 + 验证

### 改动: 无 (NOP。cc2 主链 114/114=100% 0 bad, 无新错误, 无参数可调; 唯一 bads 均为 hermes 越界宿主, 非 cc2 范围)

### 依据 (注入轮前链路分析 19:40 CST + DB/容器复核 2026-08-07)

- **30min cc4101-primary (主 nv_gw:40006) = 114/114 = 100% SR, 0 bad**。
- dsv4f0731_nv 整体 SR=98.1% (153/156) — 3 个 502 全属 hermes 越界宿主 (非主链)。
- 30min cc_requests = **114 ok / 114 = 100% SR, fallback 0 次 / 0.0%** (DB 复核)。
- 错误分类: zombie_empty_completion×3 (全 hermes 越界宿主, 非 cc2 范围)。
- per-key: 全部 5 key 正常 pexec_success, 仅单条 RemoteDisconnected 噪声 (每次 1, 常态)。
- buffer 日志无 fail/WAIT/KEYMGR。
- /health 实测: 40006/4101 全 200; 容器 nv_gw Up 21h, cc4101 Up 16h, nv_gw_stable Up 5d.

### 本轮数据

| 指标 | 值 | 状态 |
|---|---|---|
| 主 nv_gw(40006) cc4101-primary | **114/114 = 100% SR, 0 bad** | ✅ |
| dsv4f0731_nv 整体 | 153/156 = 98.1% (3×502 全 hermes 越界宿主) | ✅(非主链) |
| 30min cc_requests | 114/114 = 100% SR, fallback 0 次 (0.0%) | ✅ |
| hermes (越界宿主, 非 cc2 范围) | zombie_empty_completion×3 | ⚠️(非主链) |
| per-key | 全 5 key pexec_success, 仅单条 RemoteDisconnected 噪声 | ✅ |
...
## 参数快照 (2026-08-07, 与上轮 R1067 一致, 未动)
