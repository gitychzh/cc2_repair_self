...
## 当前轮基线 (2026-08-02 21:58 CST, R375 已完成, R376 待跑)
- 本仓 master: R374 已 push (bf2383a). hermes 仓: R374 待 push.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R375 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller SR=27.3% (3/11), 失败 8 = 8× all_tiers_exhausted
  (细分 5×429 avg 1766ms NVCF dsv4p function 配额瞬时空位 + 3×502 avg 1ms NV-TIER-SKIP 全 cooling 瞬拒, 均 0ms/1ms 快速失败非缓冲 caller)
  ...
  **cc2 是缓冲 caller (NVU_BUFFER_CALLERS), 走 buffer 5key 轮转路径, 不走 NV-TIER-SKIP, 不受影响.**
  错误类型无新增 (dsv4p 仍 all_tiers_exhausted, glm5_2_nv stream_first_byte_timeout, 九十八轮一致 R268-R375).
  cc2 无流量不受影响, 0 fallback 0 deadline. 0 改动 0 restart. **九十八轮一致 R268-R375**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~21:57 CST)
...
per-key (dsv4p): key2 → 3×200 (9700); 空 key → 5×429 (1766) + 3×502 (1).
per-egress: 203.10.96.139 → 3× (100); 空 → 8× (0).
finish_reason (200): tool_calls×2, stop×1 (无 zombie).
分钟趋势: 13:30 429×1, 13:33 502×3 (1ms NV-TIER-SKIP), 13:35 429×1, 13:40 200×1, 13:41 200×2, 13:45 429×1, 13:50 429×1, 13:55 429×1.
延迟 (200): avg_dur 9700, max 11546, min 7774, avg_ttfb 8289, avg_in 0, avg_out 0.
fallback f×14 (全部 false, 0 fallback).

### 3. 错误分类 (30min)
- 8 dsv4p 错误: 8× all_tiers_exhausted (avg 1104ms) — 细分: 5×429 (1766ms, NVCF dsv4p function 配额瞬时空位) + 3×502 (1ms, NV-TIER-SKIP 全 cooling 瞬拒).
