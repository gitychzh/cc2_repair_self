...
## 当前轮基线 (2026-08-02 19:43 CST, R342 NOP 巡检轮)
- 本仓 master: 上轮 R341 (39ecc2a) 已 push. 本轮 R342.
- **架构**: cc4101 `PRIMARY_UPSTREAM_MODEL=dsv4p_nv`. cc2 链路 = cc4101(dsv4p_nv) → nv_gw → NVCF.
- **本轮 R342 (hm2_cc2)**: NOP 巡检轮. cc2 (cc4101-primary) 30min 0 req (session 间歇空闲).
  dsv4p_nv 30min 全 caller (hermes) SR=66.7% (12/18), 失败 6 = 6× all_tiers_exhausted
  (NVCF function 级 429 配额周期多波). 本轮无 NVStream_IncompleteRead/502 transport hang.
  错误类型无新增, 与 R268-R341 一致. cc2 无流量不受影响, 0 fallback 0 deadline. 0 改动 0 restart.
  **六十四轮一致 R268-R342**.
...
## 本轮关键数据 (30min 实时链路分析注入 ~19:42 CST)

### 1. cc2 (cc4101-primary) 30min 0 req
- 同 R275-R341, session 间歇空闲, 链路空闲健康. 0 fallback 0 deadline.
- buffer/wait 日志 (BUFFER-/WAIT-) 30min 空 (无 buffer 流量).
- 30min caller×model×status 总览: hermes|dsv4p_nv|200×12, 429×5, 502×1.

### 2. dsv4p_nv 30min 全 caller SR=66.7% (12/18)
| caller | model | status | count |
|---|---|---|---|
| hermes | dsv4p_nv | 200 | 12 |
| hermes | dsv4p_nv | 429 | 5 |
| hermes | dsv4p_nv | 502 | 1 |

per-key (dsv4p): key2 → 12×200 (avg_dur 10851); 空 key → 5×429 (1776) + 1×502 (34101, transport-level hang).
per-egress: 203.10.96.139 → 12×100%; 空 IP → 6×0%.
finish_reason (200): tool_calls×12 (无 zombie).
分钟趋势: 11:15-11:27 持续 12×200; 11:16/11:20/11:25/11:26/11:27/11:30/11:35/11:40 多波 429 + 11:27 1×502.
延迟 (200): avg_dur 10851, max 19630, min 4856, avg_ttfb 10474.
fallback 0/18.

### 3. 错误分类 (DB 实测)
- 6 错误: 6× all_tiers_exhausted (sub=all_tiers_failed_in_mapped_tier, avg_dur 7164).
- 429 (5×): NVCF function 级配额周期, 多波, key2 同时挂 → all_tiers_exhausted.
- 502 (1×, dur 34101ms): 同 R336-R341 RemoteDisconnected transport-level hang (单发, 偶发).
- 本轮无 NVStream_IncompleteRead.
- 与 R268-R341 错误类型集合一致.
...
## 根因: buffer 对 function 级 429 无保护 (设计盲区, 非代码缺陷, 沿用 R278-R341 分析)
...
- 多波 429 证明是 NVCF function 配额周期自恢复, 非 nv_gw 代码缺陷.
- KEYMGR 指数退避 (120→180→480s) 正常工作, 429 后 key 进入冷却, 配额恢复后 ProbeWorker 探测唤醒.
- 502 RemoteDisconnected transport hang: 同 R336-R341, NVCF 单 function 偶发 transport 级断连, peer 也降级时返回 502 (设计). 偶发不达介入阈值.

### 结论
- **非 nv_gw 代码缺陷, 无需本轮改码**. NVCF function 级配额是上游硬限制.
- buffer 5key 轮转对 key/IP 级 429 仍有效 (R268-R341 验证), 对 function 级 429 是已知盲区.
- 当前 cc2 流量极低, all_tiers_exhausted 罕见且自恢复, 不达介入阈值.

## 判稳
- **NOP 巡检轮**. cc2 primary 0 req, 链路空闲健康, 0 fallback 0 deadline.
- dsv4p_nv 本轮 SR=66.7% (12/18), 与 R341 同窗口数据漂移 (18/23 → 12/18, 窗口滚动自然变动, 429 波次增多).
- 错误类型无新增, 与 R268-R341 一致.
- 六十四轮一致 R268-R342.
