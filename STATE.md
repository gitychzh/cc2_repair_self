...
> 当前轮: R719 (NOP 巡检, 2026-08-03 20:02 CST)
> 上轮: R718 (NOP, cc2 零流量 dsv4p91.3% glm5_2_nv57.1% NVCF上游恢复停滞)

## 本轮 (R719) 改了什么 + 依据 + 验证

### 改动: 不改码 (NOP)

### 依据 (30min 窗口 ~19:33-20:02 CST, 注入数据)
- **cc2 (cc4101-primary) 30min**: 0 rows — cc2 本窗口零流量, 无数据不动手 (连续第 4 轮零流量)
- **nv_gw 全量 30min** (hermes caller, 非 cc2):
  - dsv4p_nv: 27×200 + 2×502 = SR **93.1%** (27/29) — 兜底链路健康 (R716 93.5%→R717 92.0%→R718 91.3%→R719 93.1% 稳态 91-94%)
  - glm5_2_nv: 4×200 + 3×502 = SR **57.1%** (4/7) — **持平 R718 未继续恢复** (四轮趋势 R716 0%→R717 50%→R718 57.1%→R719 57.1%)
- **错误分类 (30min, 无新错误类型)**:
...
- **per-key (dsv4p)**: k0 6×200, k1 5×200, k2 5×200, k3 6×200, k4 5×200 — 均衡
- **per-egress-IP (dsv4p)**: 5 US IP 全 100% (5-6 req 各) — IP 轮转健康
- **dsv4p 200 延迟**: avg 10.7s, max 28.7s, min 3.3s, ttfb 9.8s, finish_reason tool_calls×24/stop×3 (无 zombie)
...
- **fallback**: f×36 (全非 cc2, hermes 流量)
- **buffer/wait/keymanager 日志**: 无 (无 buffer 触发, 链路直接 fallback 到 dsv4p)
- **根因**: glm5_2_nv NVCF 上游恢复停滞在 57% (R718→R719 持平), cc4101 fallback → dsv4p 兜底, cc2 本窗口零流量
...
- glm5_2_nv NVCF 上游恢复停滞在 57% (R718→R719 持平), 继续观察是否突破稳态
