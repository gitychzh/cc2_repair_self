# R498 — NOP 巡检轮 (2026-08-03 05:14 CST)

## 摘要
- 0 改动 0 restart. NOP 巡检轮 (连续低谷窗口延续, R475-R497 模式延续).
- cc2 (cc4101-primary) 30min 0 req (session 间歇空闲, 无评估样本, 铁律1 不满足 → 不动码).
- dsv4p_nv 全 caller 30min SR=22.2% (2/9: 2×200 + 6×429 + 1×502), 与 R496(33.3%)/R497(20.0%) 同低谷窗口一致.
- 错误: all_tiers_exhausted ×6 (历史一致, R268 起 190+ 轮) + zombie_empty_completion ×1 (R2311 主动防御, 非新故障).
- nv_tier_attempts 30min 0 行 (429 在 tier 层前被 KeyManager 全局冷却拦截, 与 R470-R497 一致).
- 配置实测确认与 R475-R497 完全一致, 无漂移.
...
- 错误分类: all_tiers_exhausted ×6 (sub=all_tiers_failed_in_mapped_tier, avg_dur=2045ms), zombie_empty_completion ×1 (avg_dur=1881ms)
- per-min: 20:40|429, 20:45|429, 20:50|429, 20:55|429, 21:00|429, 21:04|502, 21:05|200×2, 21:06|429 (离散, 非爆发)
...
## 历史波动区间 (R437-R498)
R437=85.0 → ... → R467-R472=44.4 → R473-R481=0.0% → R482-R487=14.3% → R488-R489=40.0% → R490-R498=20-33.3% (30min 低谷, 6h 视角 69.6%)

## 判稳
- cc2 0 流量 → 无评估样本, 改前无数据 (铁律1 不满足), 不动码.
- 错误类型 all_tiers_exhausted ×6 + zombie_empty_completion ×1, 模式与 R268-R497 一致 (190+ 轮), 无新错误.
- dsv4p_nv 30min SR=22.2% (vs R497 20.0% 持平) 仍是低谷窗口, 属稳态周期性行为, 根因在 NVCF dsv4p 配额/IP 漂移, 非 nv_gw 侧可修复.
- fallback 兜底正常 (cc4101 层走 ms_gw glm5_2_ms, 链路有保障).
- 0 restart → 无需 py_compile / curl 复测.
- 配置实测与 R475-R497 完全一致, 无配置漂移.
...
- 留意 502 是否再现 (R476/R480-R497 记 6h 低频 zombie 502, 再现 >=3/h 才介入).
